// Korean (한국어) — the source dictionary. Adding a key here makes it
// required in every other language file via `defineLocale` typing.

const ko = {
  // Search
  search_placeholder: '검색어를 입력하세요...',
  search_close: '닫기',
  search_results_count: '개의 결과',
  search_empty: '검색어를 입력하면 결과가 표시됩니다',
  search_no_results: '검색 결과가 없습니다',
  search_no_results_hint: '검색어의 철자나 다른 표현을 시도해 보세요.',
  search_clear_filter: '필터를 지우고 다시 검색',
  search_browse_paths: '학습 경로 둘러보기',
  search_browse_archive: '전체 글 보기',
  search_loading: '검색 인덱스 로딩 중...',
  search_error: '검색 인덱스를 불러올 수 없습니다. 다시 시도해주세요.',
  search_select: '선택',
  search_navigate: '이동',
  search_trigger_hint: '문서 검색',
  search_filter_tag: '태그:',
  search_filter_series: '시리즈:',

  // 404
  page_not_found: '페이지를 찾을 수 없습니다',
  page_not_found_description: '요청하신 페이지가 존재하지 않거나 이동되었을 수 있습니다.',
  back_to_home: '홈으로 돌아가기',
  view_blog: '블로그 보기',
  search: '검색하기',

  // Pagination
  prev: '이전',
  next: '다음',
  prev_page: '이전 페이지',
  next_page: '다음 페이지',
  page_of: '페이지',

  // Blog
  toc: '목차 (On this page)',
  toc_open_close: '목차 열기/닫기',
  recent_posts: '최근 글',
  latest_posts: '최신 글',
  recently_updated: '최근 업데이트',
  updated: '업데이트',
  no_recent_updates: '아직 최근 업데이트된 글이 없습니다',
  view_all: '전체 보기 →',
  reading_time_min: '분 읽기',

  // Shortcuts
  keyboard_shortcuts: '키보드 단축키',
  shortcut_open_search: '검색 열기',
  shortcut_help: '이 도움말 보기',
  shortcut_close_modal: '모달 닫기',

  // Settings
  settings_saved: '설정이 저장되었습니다',

  // Share
  share_twitter: 'Twitter/X에 공유',
  share_linkedin: 'LinkedIn에 공유',
  share_copy_link: '링크 복사',
  share_copied: '복사됨!',

  // Related Posts
  related_posts: '관련 글',

  // Accessibility
  skip_to_content: '본문으로 건너뛰기',

  // Giscus
  giscus_setup_required: '댓글 기능을 사용하려면 Giscus 설정이 필요합니다.',
  giscus_setup_link: 'giscus.app에서 설정하기 →',
};

export default ko;
