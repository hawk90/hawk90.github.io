...
> **주의** — `scheduler-locking on` 상태로 `continue`하면 정지 상태인 다른 스레드 때문에 데드락에 빠질 수 있습니다. 디버깅 끝나면 `off`로 되돌립니다.
...
제약: x86 Linux만, 일부 CPU 이벤트(예: RDTSC, 일부 SIMD)는 기록 안 됨, 멀티 코어 동시성은 단일 코어 시뮬로만. 그래도 *재현 불가 버그*에는 거의 유일한 무기입니다.

LLDB에는 동등한 기능이 없고, macOS에서는 `chronicle` 같은 실험적 도구가 있긴 하지만 안정성은 rr이 압도적입니다.
