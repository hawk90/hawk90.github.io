...
[Ch 1](/blog/tools/gnu-make/chapter01-intro)부터 [Ch 6](/blog/tools/gnu-make/chapter06-conditionals)까지의 도구를 한 자리에 모읍니다. 가공의 예제가 아니라 *실제 프로젝트에 그대로 옮겨 쓸 수 있는* Makefile입니다.
...
- order-only `| $(BUILDDIR)`로 디렉터리 보장 ([Ch 2](/blog/tools/gnu-make/chapter02-rules#order-only-의존성--있기만-하면-ok)).
- `-MMD -MP`로 헤더 의존성 자동 추적 ([Ch 6](/blog/tools/gnu-make/chapter06-conditionals#자동-의존성-생성--실무-표준-패턴)).
...
다음 단계로 [CMake 시리즈](/blog/tools/cmake/chapter01-intro)를 권합니다. CMake는 *Make/Ninja를 자동 생성*하는 메타 빌드 시스템으로, 크로스 플랫폼·IDE 통합·외부 라이브러리 탐색을 단번에 해결합니다.
