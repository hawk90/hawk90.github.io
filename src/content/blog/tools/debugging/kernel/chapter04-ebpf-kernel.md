...
```
사용자
  ↓ (bpftrace 한 줄 또는 C 코드)
  컴파일러 → BPF 바이트코드
  ↓
커널 verifier (안전성 검증: 무한 루프 없음, 메모리 접근 안전, ...)
  ↓
JIT (네이티브 x86/ARM 코드로)
  ↓
hook (kprobe, tracepoint, perf event, XDP, ...) → 실행
  ↓
사용자 측에 결과 (perf buffer, map, ...)
```
...
- [strace-tracing 시리즈](/blog/tools/strace-tracing/chapter05-ebpf-overview)
