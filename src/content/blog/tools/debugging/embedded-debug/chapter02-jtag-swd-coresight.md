...
JTAG의 상태 머신과 달리 *패킷 기반*. 패킷 한 단위.

```
Host → Target (Request, 8 bits):
  [Start=1][APnDP][RnW][A2][A3][Parity][Stop=0][Park=1]
  
[Turnaround, 1 clock]

Target → Host (ACK, 3 bits):
  001 = OK
  010 = WAIT
  100 = FAULT

[Data, 32 bits + parity, direction depends on RnW]

[Turnaround, 1 clock if needed]
```
...
```
        ...
        ▼      ▼      ▼      ▼      ▼      ▼
     ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌─────┐
     │CPU │ │FPB │ │DWT │ │ITM │ │TPIU│ │ ETM │
     └────┘ └────┘ └────┘ └────┘ └────┘ └─────┘
                           (SWO/TRACE 핀으로 trace 출력)
```
