...
Cortex-M의 NVIC는 *우선순위 레지스터에 8 비트의 자리*를 두지만 *실제 구현 비트 수는 칩마다 다릅니다*. NVIC IPR 레지스터의 *유효 비트는 MSB 쪽*에 위치한다.

| 코어 | 구현 비트 | 단계 | 예 |
|------|----------|------|-----|
| Cortex-M3/M4 (STM32F4 등) | 4 비트 | 16 단계 | priority 0 → `0x00`, priority 5 → `0x50` (`5 << 4`), priority 15 → `0xF0` |
| Cortex-M0/M0+ | 2 비트 | 4 단계 | priority 0 → `0x00`, priority 1 → `0x40`, priority 3 → `0xC0` |
...
`configPRIO_BITS`(FreeRTOSConfig.h):
...
값이 틀리면 `configMAX_SYSCALL_INTERRUPT_PRIORITY` 계산이 깨져 ISR이 *silent하게 잘못된 우선순위*로 동작합니다.
...
| configMAX_SYSCALL_INTERRUPT_PRIORITY 이상 우선순위 ISR이 RTOS API 호출 → assert | 우선순위 정책 위반 | ISR 우선순위 하향 |
...
- [Practical RTOS Internals](/blog/embedded/rtos/practical-internals/00-preface) — ISR 입출구 코드
- [ESP32-C3 Mastering Ch 10: FreeRTOS](/blog/embedded/riscv/esp32-c3-mastering/chapter10-freertos)
- [Modern Embedded Recipes](/blog/embedded/modern-recipes/00-preface) — ISR 패턴
