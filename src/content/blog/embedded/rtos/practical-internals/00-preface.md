...
slug: "embedded/rtos/practical-internals/00-preface"
...
### Part 2: Scheduler & Context Switch (11개)
...
| 2-11 | RTOS Tracing과 Observability | Tracealyzer, SystemView, ITM/ETM |

---

### Part 3: IPC & Synchronization Internals (11개)
...
| 3-11 | Stream Buffer와 Message Buffer | FreeRTOS 10+ lock-free SPSC IPC |

---

### Part 4: Memory & Advanced Topics (13개)
...
| 4-11 | TrustZone과 TF-M 통합 | Cortex-M33+ 보안 격리, PSA |
| 4-12 | AMP와 OpenAMP | Heterogeneous SoC, RPMsg, remoteproc |
| 4-13 | C++ in RTOS | RAII, std::thread thunks, coroutine |

---

### Part 5: RTOS Source Analysis & Comparison (7개)
...
| 5-06 | Apache NuttX 분석 | POSIX RTOS, PX4·NASA Ingenuity |
| 5-07 | PREEMPT_RT Linux 비교 | Mainline RT (6.12), Xenomai 4 |
...
| RTOS | 버전 | 특징 |
|------|-----|------|
| FreeRTOS | 11.2 | 가장 널리 사용, 단순한 구조, AWS IoT 통합 |
| Zephyr | 4.0 LTS | Linux Foundation, 풍부한 드라이버, devicetree |
| RT-Thread | 5.x | 경량 RTOS, 중국 생태계, 다양한 컴포넌트 |
| Eclipse ThreadX | 6.4 | 구 Azure RTOS — 2024-01 Eclipse Foundation 기증 후 오픈소스화 |
| Apache NuttX | 12.x | POSIX-compliant, PX4·NASA Ingenuity |
| Linux (PREEMPT_RT) | 6.12+ | 2024-09 mainline merge — soft RTOS 대안 |

> **참고**: Arm mbed OS는 2026년 7월 EOL로 신규 프로젝트에서 제외됩니다.

## 학습 로드맵

- **RTOS 입문자** — Part 1 (기초) → Part 2 (스케줄러) → Part 5-01 (FreeRTOS 분석)
- **RTOS 내부 이해** — Part 2 (스케줄러) → Part 3 (IPC) → Part 4 (메모리)
- **RTOS 개발자/포팅** — Part 2 심화 → Part 4 심화 → Part 5 (소스 분석, 포팅)
