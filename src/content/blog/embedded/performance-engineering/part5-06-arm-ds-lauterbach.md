---
title: "5-06: ARM DS·Lauterbach — Hardware Trace 도구"
...
```text
ARM DS
├── DS-5 (legacy 이름, 현재는 ARM DS로 통합)
├── Streamline Performance Analyzer
├── DSTREAM / DSTREAM-PT probe 지원
└── Cortex-A·R·M 전 코어 디버깅
```

Streamline의 capture 흐름은 다음과 같습니다.

```text
1. Target에 gator daemon 설치 (Linux)
   또는 ETM unit 활성화 (bare-metal)
...
├── Instruction trace (PC 흐름)
├── Data trace (load/store 주소·값)
...
```text
ETM 출력 포맷
├── Branch packet     — 분기 발생 시 PC
├── Address packet    — 분기 목적지
├── Cycle count       — 정확한 cycle 단위 timestamp
└── Data packet       — load/store 값 (옵션)
```
...
```text
FreeRTOS, ThreadX, Zephyr, AUTOSAR OS, QNX, INTEGRITY
```
...
```text
DO-178C (항공)     — Modified Condition/Decision Coverage 요구
ISO 26262 (자동차)  — Branch Coverage 요구
IEC 62304 (의료)    — Statement Coverage 요구
```
...
```text
ARM Development Studio       Ultimate Edition  연 $3,000-5,000
ARM Streamline (포함)
...
```text
ETM이 초당 10 Gbit를 출력하는데 probe buffer가 1 GB면 0.8초만 기록 가능
```
...
```text
gator daemon은 sampling 기반이며 1-3% CPU 사용
```
