...
```text
[1] Direct Attach (CXL 1.1/2.0)

  Host CPU ────CXL link────► CXL Type 3 Memory Device
                              (Samsung CMM-D, SK Hynix Niagara)

[2] Single Switch (CXL 2.0 pooling)

  Host CPU ────CXL link────► CXL Switch ──┬─► Memory Device A
                                          ├─► Memory Device B
                                          └─► Memory Device C

[3] Multi-Host Pool (CXL 2.0 multi-LD / CXL 3.0 fabric)

  Host A ───┐
  Host B ───┼──► CXL Switch ──► Memory Device (Multi-Logical Device)
  Host C ───┘                   (각 Host에 logical slice 할당)
```
