...
N — CPU 수
α — contention (serial fraction)
β — coherency (cache invalidation 비용)
```

α만 있으면 Amdahl처럼 점근선에 수렴합니다. β가 있으면 어느 N부터는 throughput이 오히려 감소하는 곡선이 됩니다. 우리 측정값이 정확히 USL β > 0 곡선이었습니다.
