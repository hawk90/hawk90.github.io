...
slug: "embedded/automotive/misra-c/chapter11-iso26262-audit-walkthrough"
...
**EPB-CS-001 Coding Standard 구조:**

**1. Base Standard** — MISRA C:2012 Amendment 2 (2020).
...
하지만 ASIL D 적용으로 *보수적 분류* TCL3 처리. ASIL D는 *Qualification Method = "Validation of the software tool"*을 요구 → Validation Suite 적용.
...
**Description** — 다음 조건을 모두 만족하면 0 나눗셈 검사를 생략할 수 있다.
...
**Justification** — 실시간 제어 루프에서 *모든 나눗셈*에 conditional 검사 삽입 시 worst-case 실행 시간이 5~8% 증가. 100ms 보장(TSR-EPB-001) 마진 잠식. 검사를 *입력 경계*에서 한 번만 수행하는 패턴이 더 안전.
...
**Reason for deviation** — STM32F767 DMA1 Controller는 *고정 MMIO 주소* `0x40020000`에 매핑된다. 포인터-정수 변환 없이는 *주소를 코드에 표현할 방법이 없다*.

**Alternative Considered:**

- Vendor HAL 사용 — HAL은 MISRA 면제 영역(Permit-EXT-002)이며, 더 큰 wrapper API 추가는 본 함수의 *단순성*을 해친다.
- Linker symbol 정의 — `__dma_base`를 `.ld` 파일에서. 분석 도구와 호환되지 않아 채택 X.
