...
- 임베디드 Linux는 "PC Linux를 줄인 것"이 아니다. 부트 시퀀스, 스토리지 모델, 업데이트 전략이 다르다.
- 직접 빌드(make + crosstool-NG + buildroot script)는 *작은 프로젝트*까지만 합리적이다.
- 여러 보드·여러 제품 라인·여러 배포판 변형을 한 코드베이스로 관리하려면 *메타 빌드 시스템*이 필요하다 — 이것이 Yocto/OpenEmbedded의 존재 이유.

## 정리

- 이 시리즈는 Yocto를 *왜 그렇게 만들었는가*에 초점을 둔다.
