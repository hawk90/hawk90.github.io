...
slug: "embedded/aerospace-standards/do-178c/chapter11-tool-qualification-do330"
...
|------------|-------------|----------|
| **TC-1** (사용) | 도구 출력이 *직접 EOC에 포함* 또는 verification에 의존 | Compiler, code generator, linker, automated test tool |
| **TC-2** (검증) | 도구가 *verification 결과 제공*, 도구 실수가 *verification miss* | Static analyzer, MC/DC tool, test framework, simulator |
| **TC-3** (개발) | 도구가 *개발 보조*, 도구 실수 영향 *작음* | IDE, formatter, debugger, diff tool, profiler |
...
| Tool Class | DAL A | DAL B | DAL C | DAL D |
|-----------|-------|-------|-------|-------|
...
| TQL | 의무 |
|-----|------|
| **TQL-1** | Tool 자체에 DO-178C DAL A 동등 모든 obj 적용. Tool requirements/design/code/test 모두 + Coverage 100% MC/DC + Independence |
| **TQL-2** | TQL-1 minus 일부 design obj — HLR + LLR + Coverage, 약간 완화 |
| **TQL-3** | TQL-2 minus more — HLR 위주, LLR 부분 면제 |
| **TQL-4** | 가벼운 qualification — Tool requirements 정의, 사용법 검증, test로 충분, code 자체 검증 안 함 |
| **TQL-5** | 가장 가벼움 — 사용 환경 정의, 알려진 한계 문서화, 가벼운 test |
...
3. Tool Validation Suite (TVS) — 1000+ test cases covering MISRA rules, expected outputs, pass/fail criteria
