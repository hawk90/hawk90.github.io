...
slug: "embedded/aerospace-standards/do-178c/chapter02-dal-and-objectives"
...
| Failure Effect | DAL | 결과 | 예 |
|----------------|-----|------|-----|
| Catastrophic | A | 항공기 비행 불능, 다수 사망 (정상 또는 단일 failure) | FBW (Fly-by-wire) 비행 제어 |
| Hazardous | B | 운영 한계 큰 감소, 안전 마진 손실, 승무원 stress 증가, 부상 가능 | Autopilot, primary navigation |
| Major | C | 운영 한계 감소, 절차 증가, 가벼운 부상 가능 | FMS, secondary navigation, EICAS |
| Minor | D | 운영 capacity 약간 감소, 불편, 부상 없음 | 일부 통신 redundancy, 데이터 로깅 |
| No Effect | E | 운영·안전·승무원에 영향 없음 (DO-178C 면제) | 좌석 entertainment, 비행 후 분석 SW |
...
| 역할 | 컴퓨터 |
|------|--------|
| Primary | ELAC (Elevator and Aileron Computer) |
...
| SW | DAL | 근거 |
|----|-----|------|
| ELAC | A | Catastrophic if both ELACs fail |
| SEC | A | Catastrophic redundancy loss |
| FAC | A | last electronic backup |
| Display (PFD) | B | Hazardous if crew unaware of state |
| FMS | C | Major — secondary navigation backup |
| Cabin SW | E | No effect on flight |

같은 항공기에 *DAL A부터 E까지 다 있다*. 각 SW가 *별도 인증 packet*.

### Multiple Computer Architecture로 등급 낮추기

**원래 시스템** — DAL A 요구, single computer with critical function.

**Redundancy 추가** — 3 computers, 2-out-of-3 voting:
...
| 옵션 | 인증 비용 |
|------|----------|
| DAL A 단일 | 1× (코드 100% DAL A) |
| DAL B × 3 redundancy | 3 × DAL B ≈ DAL A의 70 % × 3 = 210 % |

→ 단일 DAL A가 비용·복잡도 낮음 *대부분의 경우*. 시스템 설계로 DAL 낮추는 것은 *DAL A 인증 자원 부족 시*에만.
...
| A-2 | Software Development Process | 15 |
| A-3 | Verification of Outputs (Requirements) | 15 |
| A-4 | Verification of Outputs (Design) | 14 |
| A-5 | Verification of Outputs (Code) | 18 |
| A-6 | Testing of Outputs | 9 |
| A-7 | Verification of Verification Process | 9 |
| A-8 | Software Configuration Management | 6 |
| A-9 | Software Quality Assurance | 5 |
| A-10 | Certification Liaison | 3 |
| | **Total (논리적 분류)** | **101** |
| | **DAL A 적용 (의무)** | **71** |
...
각 partition이 *시간 budget*과 *memory region*을 *강제로 분리*합니다. 한 partition의 crash가 *다른 partition에 영향 없습니다*.
...
| Level | 대상 | 환경 |
|-------|------|------|
| Hardware-Software Integration Test (Highest) | 모든 인터페이스 | 실제 target, 보통 HIL lab |
| Software Integration Test | 통합된 SW (단위 X) | Host 또는 target |
| Low-Level (Software Unit) Test | 각 함수·모듈 | Host 또는 target — MC/DC 측정 source |
...
| Option | 구성 | 비용 | 인증 기간 |
|--------|------|------|----------|
| 1 — 단일 DAL A | 50 KLoC × \$100/LoC | \$5,000,000 | 3 year |
| 2 — 2× DAL B redundancy (2-of-2 dissimilar) | 2 × 50 KLoC × \$60 + dissimilarity 추가 | \$7,500,000 | 2.5 year (병행) |
| 3 — 3× DAL C (2-out-of-3 voting) | 3 × 50 KLoC × \$35 + voting logic DAL A (\$200 K) | \$5,450,000 | 2 year (병행) |
