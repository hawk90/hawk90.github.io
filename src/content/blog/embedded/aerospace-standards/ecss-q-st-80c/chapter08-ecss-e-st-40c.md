...
slug: "embedded/aerospace-standards/ecss-q-st-80c/chapter08-ecss-e-st-40c"
...
| 표준 | 역할 |
|------|------|
| ECSS-E-ST-40C | SW를 *어떻게 만드는가* (Engineering) — V-model lifecycle 정의 |
| ECSS-Q-ST-80C | 만든 SW가 *어떻게 좋은가* (Quality) — 각 단계의 quality 보증 |
...
| Review | After | Output | Approval | Major decision |
|--------|-------|--------|----------|---------------|
| **SRR** — System Requirements Review | Phase 1 (System Req 확정) | Functional Baseline (FBL) | Customer + Project Manager | — |
| **PDR** — Preliminary Design Review | Phase 3 (Architectural Design) | Allocated Baseline (ABL) | Customer + ESA / 정부 | — |
| **CDR** — Critical Design Review | Phase 4 (Detailed Design) | Design Baseline (DBL) | Customer + ISVV | Design freeze |
| **QR** — Qualification Review | Phase 8 (Validation) | Product Baseline (PBL) | Customer + ISVV | "Ready for delivery?" |
| **AR** — Acceptance Review | Phase 9 (Delivery) | Operational Baseline (OBL) | Final customer signature | "Ready for operation?" |
...
**Review** — SRR (System Requirements Review): System-level (모든 stakeholder) / SW allocation 검증 / Feasibility 평가
...
CDR은 *design freeze*. 이후 변경은 CCB 거쳐야 하며 큰 영향이 있습니다.
...
**ECSS와 Agile 조합 (hybrid)**

| Phase 범위 | 운영 방식 |
|------------|-----------|
| Phase 2 – 4 (Requirements + Design) | iterative sprint |
| Phase 5 – 7 (Coding + Testing) | continuous integration |
| Gate reviews (SRR / PDR / CDR) | waterfall 준수 (단단한 gate) |

Hybrid approach는 *신생 우주 회사*나 *small mission*에서 흔히 시도됩니다. 각 조직의 구체적 process는 *조직 발표*만 인용.
