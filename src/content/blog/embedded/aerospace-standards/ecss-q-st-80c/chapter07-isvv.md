...
```
Criticality A   ISVV 외부 의무 (일반)
Criticality B   ISVV 강한 권장
Criticality C   프로젝트 결정
Criticality D   자체 가능
```
...
```
A. Completeness
   - 모든 system req가 HLR에 derive?
   - 누락된 functional / non-functional req?
...
Owner Response:
  [개발 조직의 응답 — accept / discuss / reject + justification]
```
...
B. Test Execution (Independent)
   ...
   - 다른 environment 또는 같은 environment 재실행
...
Key Strengths
Key Improvement Areas (다음 mission용)
