...
**DO-333 (2011, RTCA)** — Formal Methods Supplement to DO-178C.
...
**공통** — Code·model의 *property*를 *수학적으로* verify. Testing 대비 *all-paths* 분석.

**인증 위치** — Testing·analysis의 *대체 또는 보완*. 특히 *robustness·timing·memory* property.

DO-333 — *수학적 verification*의 인증 frame.

## 1. Abstract Interpretation

**Abstract interpretation** — Sound static analysis. All-paths over-approximation. False positive 가능, false negative 없음.
...
**Astrée (AbsInt)** — C·C++ static analyzer. Sound abstract interpretation.

**Airbus 도입** — A380·A340·A350 fly-by-wire SW. RUNTIME ERROR 0 보장.
...
**효과** — Testing 대비 *비용 절감 + 강한 보장*. Real flight 사고 zero (관련 SW).
...
**Polyspace Code Prover (MathWorks)** — Similar to Astrée. Sound analysis.

**Polyspace 변형**:

- **Bug Finder** — heuristic (faster, less precise)
- **Code Prover** — formal (slower, sound)

**사용처** — Boeing·Airbus·자동차. ISO 26262·DO-178C. Tool qualified TQL-4.
...
**Model checking** — Finite-state model의 property를 exhaustive check. Temporal logic property (LTL·CTL).
...
**Theorem proving** — 수학적 axiom·rule 기반 *정리 증명*. Mechanized prover.
...
**SPARK** — Ada subset for formal verification. AdaCore (commercial) + open source.
...
**Frama-C (CEA·INRIA)** — C analysis framework.
...
Open source + commercial 지원. 사용처 — Aerospace (CEA·Airbus 부분), Critical OS·driver.
...
**Coverage** — Formal method가 *all-paths* 분석 → Object code coverage 대체 가능 (Level A). 단 PSAC에 *명시 + agreed with FAA*.

DO-333 사용 시 PSAC·SVP 명시 → FAA·EASA agreed.

## Formal Method 한계

- **좁은 domain** — 제어·protocol·crypto 가능. UI·application 어려움.
- **Property 정의 어려움** — "안전하다" 정량 어려움. Specification language 학습.
- **Tool 성숙도** — Astrée·Polyspace·SPARK는 mature. Coq·Isabelle은 niche.
- **비용** — Engineer 학습 (수개월~년), Tool license, Iteration (false positive 해결).
- **State explosion (model checking)** — 상태 수 폭증. Abstraction 필요.
...
- **Layer 1 — Application code** — Testing + coverage (traditional)
- **Layer 2 — Critical algorithm** — Formal method (abstract interpretation 또는 proof) + Testing 보조
- **Layer 3 — Mathematical core** — Theorem prover (Coq·SPARK) + Code generation
- **Layer 4 — Protocol·concurrency** — Model checking (SPIN·UPPAAL)
- **Layer 5 — System integration** — HIL test (formal 불가)
...
- **Cassini (1997 launch)** — Manual proof + extensive testing
- **Mars Pathfinder (1997)** — Priority inversion bug catch — model checking 활용
- **Mars Curiosity (2012)** — Lockheed JPL 협력. SPIN·model checking for autonomy.
- **SpaceX Crew Dragon (2020)** — C++ heavy + 자체 formal·static
- **SLS·Orion (current)** — NASA NPR 7150.2 Class A. Formal method limited.
...
**Property Verification Trace** — HLR ↔ formal property ↔ proof.

**Tool Qualification** — DO-330 (Ch 9). Astrée·Polyspace 등 vendor kit.
...
**학계** — KAIST formal methods 연구. SoongSil·POSTECH 일부.

**방사청** — Formal method 직접 요구 없음. 자체 verification 강조.
...
| Unit test | medium | medium |
| System test | high | medium |
| MC/DC coverage | high | high (specific) |
| Object coverage | very high | high |
| Abstract interpretation | high | very high (specific) |
| Model checking | very high | very high (specific) |
| Theorem proving | extreme | ultimate (limited scope) |
...
"Formal method로 모든 verification 대체" → 비현실적 → Application·UI 불가.

→ Hybrid — 핵심에만.

> ⚠️ False positive 무절제

Polyspace 결과 — 1000 alarms → 모두 무시 → 진짜 결함 묻힘.

→ Each alarm — *review·dismiss·fix·waive*.

> ⚠️ Tool qualification 누락

SPARK·Astrée·Polyspace 사용 → DO-330 qualification 미실시 → Audit fail.

→ Vendor kit + configuration.

> ⚠️ Property 잘못 정의

"안전성 증명" — vague property → 무엇을 prove? unclear.
