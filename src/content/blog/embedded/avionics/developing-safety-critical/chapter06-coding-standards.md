...
slug: "embedded/avionics/developing-safety-critical/chapter06-coding-standards"
...
**Plans (5)** — PSAC·SDP·SVP·SCMP·SQAP.
...
MISRA C 2023 (구 MISRA C:2012 Amd 4) — Origin: 영국 자동차 협회 (1998). 자동차 + 항공 + 의료 + 산업 채택.

**Rule 분류** (Total 175+ rules):
...
CERT C Coding Standard — Origin: CERT/CC (Carnegie Mellon SEI). *Security* vulnerability 방지.
...
JSF (Joint Strike Fighter) C++ Coding Standard — Origin: Lockheed Martin F-35 program (2005). Major C++ subset for safety-critical.
...
**AUTOSAR C++14 (2017)** — 자동차 SW (ISO 26262). ASIL-D 충족. C++14 + 안전 subset.

**MISRA C++ 2023** — AUTOSAR C++14 + MISRA C++ 2008 통합. JSF C++의 modern 후속. C++17 일부 허용.
...
**Functions** — `snake_case` 또는 `camelCase`. Prefix module/system: `avionics_compute_attitude()` 또는 `AvionicsComputeAttitude()`.

**Constants·macros** — `UPPER_SNAKE_CASE`. 예: `MAX_BUFFER_SIZE`, `PI_FLOAT`.
...
**방사청 SW 신뢰성시험 (한국 방산)** — 필수: *코딩 표준 명시 + 준수 evidence*.
...
1. **Language** — C++17 (`-std=c++17`), GCC 11.3.0 cross-compiler
2. **Subset** — MISRA C++ 2023 + AUTOSAR C++14 (no conflict)
3. **Forbidden**:
...
5. **Naming** — `namespace::ClassName::method_name()`, `variable_name`, `CONSTANT_NAME`
6. **Comment** — Doxygen header per file·function. Traceability tag (HLR·LLR·TC).
