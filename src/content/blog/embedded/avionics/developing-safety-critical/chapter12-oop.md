...
slug: "embedded/avionics/developing-safety-critical/chapter12-oop"
...
**DO-332 (2011, RTCA)** — Object-Oriented Technology and Related Techniques Supplement.

**다루는 언어** — C++, Java, Ada (with OO), C#. 실제 avionics — *C++ 중심*.
...
| 기능 | Risk | Mitigate |
|---|---|---|
| Inheritance | 복잡한 type hierarchy, Liskov 위반 시 unexpected behavior | 깊이 제한 (보통 ≤3), interface 위주 |
| Multiple Inheritance | Diamond problem, complexity | *금지* (interface inheritance OK) |
| Polymorphism | Runtime decision, unpredictable | Static dispatch 우선, dynamic은 limited use |
| Dynamic Dispatch (virtual function) | Coverage·analysis 어려움, vtable overhead | Coverage at each override, Substitution analysis |
| Type Substitution (LSP) | Sub-type이 base contract 위반 | Class invariant verification |
| Overloading | Ambiguity, reader confusion | 제한적 사용, 명확 naming |
...
**DO-332 입장** — Exception 사용 가능, 단 *모든 path 분석*.

**실무** — *Exception 사용 안 함* (99%):

- Compile `-fno-exceptions`
- Return code·status object 사용

**예외 case** — Throw/catch *없음*. Operator new throw bad_alloc → 대체 `new(nothrow)`.
...
**DO-332 입장** — Template 가능, 단 *각 instantiation* verify.
...
**DO-178C·DO-332 입장** — *Init 이후 dynamic 회피*. Init 단계만 허용.
...
**대체** — custom allocator (pool 기반), 스토리지 별도 partition.
...
단점 — Abstraction 부족, Code duplication, Type safety 약함, Manual memory management.
...
단점 — 컴파일러 복잡, Coverage·analysis 어려움, 인증 history 짧음, C++ subset rule 학습.
...
**자동차 (인증 비교)** — AUTOSAR C++14 / MISRA C++ 2023, ISO 26262 ASIL-D.
...
**Virtual function** — Each override — coverage? → 모든 dynamic call site에서 가능한 모든 type.

**Template instantiation** — Each instantiation — separate coverage? → 각 type별 generated code 검증.

**Implicit constructor·destructor** — Compiler-generated code → Coverage tool이 catch.

**Operator overloading** — Each operator — verify.
...
**Object code coverage (Level A)** — C++ → object code 변환 시 다음이 발생.
...
→ Object code 증가 + Coverage 측정 어려움.
...
"`std::vector`·`std::string`·`std::function` — productive" → Heap allocation → Real-time·인증 부담.

→ Subset rule + static container.

> ⚠️ Exception "안전 fallback"

"`try`·`catch`로 fault handle" → Coverage·analysis 폭증 → Unpredictable timing.

→ `-fno-exceptions` + return code.

> ⚠️ RTTI 사용

`dynamic_cast`·`typeid` → Runtime overhead·complexity → Coverage 어려움.

→ `-fno-rtti` + design pattern.

> ⚠️ Template 무절제

Heavy template metaprogramming → Compile time 폭증 → Coverage·debug 어려움.
