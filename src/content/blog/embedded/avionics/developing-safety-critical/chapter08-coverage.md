...
```text
정의:
  각 *condition*이 *독립적으로* decision outcome 결정
  
요구:
  1. Statement coverage 충족
  2. Decision coverage 충족
  3. 각 condition: 다른 condition fix 상태에서
     해당 condition만 바꿔서 decision 변화 시연
...
예:
  ...
  Decision outcomes:
    A=T,B=T → decision=T
    A=T,B=F → decision=F   (B의 영향 확인)
    A=F,B=T → decision=F   (A의 영향 확인)
    A=F,B=F → decision=F
...
Multiple Condition Coverage (MCC):
  모든 조건 조합 (2^N) 모두 cover
  
  if (A && B && C) {
    2^3 = 8 combinations 모두
  }
  
MC/DC:
  각 condition independent 영향만
  N+1 cases (보통) 충분
...
Source coverage 100% ≠ Object coverage 100%
...
1. Dead code (제거):
   Unreachable code
   → 코드 수정 (제거 또는 reachable 만들기)
   
2. Defensive code (justify):
   "Should never happen" branch
   예: switch default·assert·panic
   → coverage waiver + justification
...
순서:
  ...
  3. Coverage 부족 → 
     a) 더 많은 requirement-based test
     b) 또는 더 많은 robustness test
     c) Dead code (제거)
     d) Defensive code (waive)
...
```text
Vector CAST report 100%
→ Audit OK
→ 실은 instrumentation 오류 또는 test가 trivial
```
