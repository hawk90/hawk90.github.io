...
A      Catastrophic         10⁻⁹/hr           Hull loss
B      Hazardous           10⁻⁷/hr           Serious injury
C      Major                10⁻⁵/hr           Significant
D      Minor                10⁻³/hr           Some
...
```text
Level A: 71 objectives
  with independence: 30
  without:          41
  
Level B: 69 objectives
  with independence: 18
...
Level E: 0 (no DO-178C objectives)
```
...
2. SW Development Process
   ...
   - Coding
   - Integration
   Output: Requirements, Design, Code, EOC

3. SW Verification Process
...
```text
1. PSAC (Plan for SW Aspects of Cert) — FAA에 제출
   Method·standard·plan 설명
...
3. SAS (Accomplishment Summary) — 마지막
   All evidence·deviation 설명
...
예 (Objective A-6.1):
  Activity: Executable Object Code (EOC)
...
Architectural mitigation:
  Higher DAL function → 별도 partition
  Lower DAL elsewhere
  
ARINC-653 partitioning:
  Time + space isolation
  각 partition별 *별도 DAL* 가능
...
```text
Development tool (TQL-5):
  Output가 EOC에 포함
  Compiler·assembler·linker
  Model code generator (Simulink·SCADE)
  
Verification tool (TQL-1 to 4):
  Verification activity 대체
...
```text
Level A·B 차이 = 12 objectives + 12 independence
실 cost — *more nuanced*
```
