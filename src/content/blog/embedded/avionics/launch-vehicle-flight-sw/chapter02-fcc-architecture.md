...
Lattice·Achronix·Intel FPGA — vendor 옵션 다양.
...
- **SEU (Single Event Upset)** — bit flip, recoverable. ECC, TMR voting으로 복구.
- **SEL (Single Event Latch-up)** — CMOS short, reset 필요. Latch-up immune chip 사용.
- **SEFI (Functional Interrupt)** — chip 자체 stuck. Watchdog reset.
- **TID (Total Ionizing Dose)** — Long-term degradation. LV mission 짧아 무관, 위성은 critical.

LV — *SEU·SEFI* 주로. *TID 무관*.

## TMR — Triple Modular Redundancy

- **Architecture** — 3 identical compute → voter → output
- **1-fail** — voter picks 2 majority → continue
- **2-fail** — undefined or fail-safe
- **Voter** — Hardware (FPGA) preferred, Software 도 가능
...
TMR보다 가벼움 (2x vs 3x cost). Modern automotive·일부 LV. 자동차 brake·airbag ECU 표준. LV에서도 채택 증가.
...
Famous incident — STS-1 (1981)에 4 PASS sync 실패로 mission delay.
...
"commercial Cortex-A53 — 비행에 OK" 라고 가정하고 SEU·SEL test를 안 하면 flight fail 사례가 나옵니다. radiation test·EDAC 필수.

> ⚠️ FPGA underestimate

"FPGA = optional, CPU로 가능"이라고 가정해도 마이크로초 latency 요구 시 CPU IRQ가 너무 느립니다. timing critical은 FPGA로 갑니다.

> ⚠️ Backplane 호환성

CompactPCI 모듈을 VPX backplane에 꽂으면 안 됩니다. 표준 명시.

> ⚠️ TMR voter 자체 redundancy 없음

Voter 하나가 single point of failure가 됩니다. voter도 redundant로 (또는 simple discrete logic으로) 구성합니다.
