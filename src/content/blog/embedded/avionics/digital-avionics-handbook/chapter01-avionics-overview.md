...
항공기 (Aircraft):
  Mission time     수 시간
  Environment      대기·온도 변화
  Comm             VHF/UHF·SATCOM
  Sensors          IMU + GPS + air data + radar
  Actuators        Control surface + engine
  인증             DO-178C, DO-254 (FAA·EASA)
  Redundancy       Triple·quad

발사체 (Launch Vehicle):
  Mission time     수 분~시간
  Environment      극한 vibration·thermal·radiation
  Comm             Telemetry (S-band·Ku) + LOS
  Sensors          IMU 우선 + GPS (가능 구간)
  Actuators        TVC + RCS + engine
  인증             RCC 319·범 NASA·KARI 자체
  Redundancy       Dual (보통) 또는 fault-tolerant

위성·우주선:
  Mission time     수 년~수십 년
  Environment      vacuum, radiation, thermal cycle
  Comm             CCSDS·DSN·통신 위성 link
  Sensors          IMU + star tracker + sun sensor + GPS
  Actuators        RCS·CMG·magnetic torquer
  인증             ECSS·NPR 7150.2·KARI
  Redundancy       Cold·hot standby
```
...
```text
Federated Avionics (전통):
  각 function = *별도 LRU* (Line Replaceable Unit)
    - Navigation box
    - Display box
    - Engine control box
    - Communication box
...
프로그램별 SW Lines of Code (typical):
  ...
  F-15 (1970s)            50K LOC
  F-16 (1980s)            150K LOC
  Boeing 747-400 (1990s)  500K LOC
  Boeing 777 (1990s)      1.5M LOC
  Boeing 787 (2010s)      ~6.5M LOC
  Airbus A380 (2010s)     ~10M LOC
  F-22 (2000s)            ~2M LOC
  F-35 (2010s)            ~25M LOC ← 최대
  
LV 비교:
  Apollo (1960s)          ~145K LOC
  Space Shuttle (1980s)   ~400K LOC
  KSLV-II 누리 (2020s)    수십만 LOC
```
...
이 시리즈에서 다루는 14 chapters:
  발사체·항공우주 *직접 가치 있는* 항목 선별
```
...
KARI (한국항공우주연구원):
  KSLV-II 누리, KOMPSAT, KPLO 다누리
  
KAI (한국항공우주산업):
  KF-21 보라매, FA-50, KUH 수리온
