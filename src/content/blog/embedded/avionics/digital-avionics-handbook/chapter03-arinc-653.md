...
각 partition 안에 *1+ processes*. Process 간 — partition 내부 IPC (buffer·event·semaphore).
...
**Health Monitor (HM):** Partition error 발생 시 *action 결정*.
...
- **IDLE:** Partition stop, processes not running.
- **COLD_START:** Initialization phase. Process create. Resource init.
- **WARM_START:** Recovery 후 init. State preserved across restart (가능).
- **NORMAL:** Normal execution. Periodic·aperiodic processes run.
...
B787 Common Core System example — **CCR Cabinet (Right):**

**GPM-1: VxWorks 653**

| Partition | Function | DAL |
|-----------|----------|-----|
| 1 | Flight Display (Honeywell) | B |
| 2 | FMS (Honeywell) | A |
| 3 | Engine indication | B |
| 4 | Cabin alerting | C |
...
각 partition — independent supplier. 공유 platform = Wind River VxWorks 653 (인증 packet).

## 한국 ARINC-653 도입

- **KAI KF-21 보라매:** Mission computer — ARINC-653 채택. Wind River VxWorks 653 또는 자체. 자세한 사항 비공개.
- **KARI 위성 (KOMPSAT):** ECSS 기반 + 일부 ARINC-653-like 채택.
- **인노스페이스·페리지:** 민간 LV — federated 위주. ARINC-653 RTOS 비용 부담. 자체 RTOS 또는 commercial fallback.
- **한국 자체 ARINC-653 OS:** ETRI·KAIST 연구. Production 도입 부족.
...
"Partition = process group"으로 보면 Linux와 본질 다름을 놓친다. Time·space·hardware isolation이 강제된다.

→ ARINC-653 = *hardware-enforced*.

> ⚠️ Schedule slack 없음

Major frame 100% 채우면 jitter·overrun catch 못 함.

→ 10~20% slack 권장.

> ⚠️ HM action 부족

"Error → just ignore"는 fault propagate를 부른다.

→ Each error → *partition·module action*.

> ⚠️ Multi-core ARINC-653 검증 부족

"Single-core ARINC-653 → multi-core 자동 적용"은 안 된다. CAST-32A·AMC 20-193 evidence 필요.
