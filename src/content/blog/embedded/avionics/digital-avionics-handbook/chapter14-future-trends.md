...
1. **Cost pressure:** 상용 부품 도입, open architecture.
2. **Multi-program reuse:** FACE·SOSA·OMS, vendor 다양화.
3. **New mission:** eVTOL·UAV·UAM·Hypersonic, new paradigm.
4. **Software:** SW 비중 ↑↑, AI·ML 도입.
5. **Reusability:** Falcon 9 — reuse 가능 LV. Avionics — repair·refurbish.
6. **Cybersecurity:** Threat 증가. Standard (DO-326A·ED-202A).
...
**SOSA:** Sensor Open Systems Architecture (US DoD).

**대상:** Sensor·EW (Electronic Warfare)·comm payload.
...
**OMS:** Open Mission Systems (US Air Force initiative).
...
1. **Image-based navigation:** Terrain matching, visual landing aid, smart camera.
2. **Anomaly detection:** Engine·airframe health, sensor fusion.
3. **Pilot assist:** Auto-land (advanced), decision support, workload reduction.
4. **Mission planning:** Optimal route, trajectory replanning.
5. **Image processing:** Target ID, threat detection, SAR processing.
6. **Predictive maintenance:** Fleet-wide ML, failure prediction.
...
**Mars Helicopter Ingenuity (2021):** Mars rotorcraft.
...
**Optical computing:** Limited adoption — research.
...
"FACE·SOSA — open source"로 보면 cert·license·integration cost를 놓친다.

→ Open standard ≠ free.

> ⚠️ AI/ML 즉시 도입

"AI for safety-critical"은 DO-178C 미흡으로 cert가 부재하다.

→ Non-safety advisory부터 시작.

> ⚠️ Reusability 무시

"단일 사용이라 인증이 단순하다"고 보면 future business 손실과 cost 경쟁력 ↓로 이어진다.

→ Reusability 설계 고려.

> ⚠️ Cybersecurity 후순위

"Avionics는 격리되어 있다"는 가정은 connected aircraft·datalink 시대에 취약하다.
...
1. **Developing Safety-Critical Software (Rierson)** — 완료, 15 chapters. DO-178C 깊이.
2. **Launch Vehicle Flight Software** — 완료, 10 chapters. LV-specific 원본.
3. **(Future)** — Civil Avionics Systems (Moir·Seabridge), Spacecraft Systems Engineering (Fortescue). 각 책 필요 시 추가.
