...
- **P1 — Original:** Single-core partitioning.
- **P1 Supplement 1·2·3·4·5:** 점진 enhancement. Health Monitor, file system 등.
- **P2 — Extended:** File system, sampling protocol.
- **P3·P4 — Conformity:** Test suite.

각 RTOS의 supplement level:
- VxWorks 653: P1S4
...
| 종류 | 대상 | 비고 |
|------|------|------|
| Block driver | Storage·flash | Volatile·non-volatile |
| Character driver | UART·SPI·I2C | Single-byte interface |
| Network driver | Ethernet·AFDX·SpaceWire | — |
| Bus driver | 1553·CAN·ARINC-429 | Avionics-specific |
...
5. **Long-term support:** 20-30년 — vendor commitment.

6. **Ecosystem:** Tool·BSP·community.

7. **Project legacy:** Previous program continuity.
...
- **KARI KSLV-II:** 자체 개발 RTOS 또는 commercial. 자세한 사항 비공개.
- **KAI KF-21:** Mission system RTOS — commercial (Wind River·GHS). 국산 BSP·driver.
- **한화 미사일·LV:** Commercial RTOS. 자체 firmware (smaller subsystem).
- **ETRI·KAIST 연구:** AIR (Aerospace IMA Real-time) — 학술. 국산 ARINC-653 OS 개발 검토.
- **민간 LV (인노스페이스·페리지):** Commercial RTOS or RTEMS. Cost-sensitive.
...
"VxWorks → INTEGRITY mid-project"는 BSP·driver·test의 완전 rework로 1년+ delay를 부른다.

→ Early RTOS lock-in.

> ⚠️ ARINC-653 declaration만 신뢰

"ARINC-653 compliance"는 vendor 선언일 뿐 — 자체 verification 없이 사용하면 안 된다.

→ Conformity test 검증.

> ⚠️ Custom BSP 인증 부담 무시

"Reference BSP를 modify했다"고 끝나지 않는다. 새 BSP는 full DO-178C evidence를 요구한다.

→ BSP 인증 effort 계획.

> ⚠️ Tool license 만료

Project mid에서 tool license 갱신 비용을 예산에서 누락하기 쉽다.
