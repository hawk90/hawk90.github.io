...
DAL A — fail-operational typical
DAL B — fail-passive 또는 dual fault-tolerant
```
...
```text
DMR (Dual):
  2 identical channel + comparator
...
```text
TMR (Triple):
  3 identical channel + majority voter
...
Lock-step (Cycle-accurate):
  ...
  단점 — clock·power coupling
  Random transient — both same → undetected
...
사례:
  Boeing 777 PFC (Primary Flight Computer):
    3 lanes, different vendor·language
  
  Airbus A380 FCS:
    Diverse FCC, hardware·software 분리
...
Cyclic checks:
  Periodic data integrity (CRC·checksum)
...
```text
Space Shuttle GPC (General Purpose Computer):
  IBM AP-101
...
```text
SpaceX Falcon 9:
  Dual flight computer (x86)
  Linux + custom RTOS
  Software-level redundancy
  Sensors triplicated
  
Atlas V·Vulcan:
  Honeywell flight computer
  Triple processor
...
```text
TMR 도입 → 보장 끝
→ Voter·power·clock common-mode 무시
```
...
```text
Spare 동작 중, but state unsync
→ Takeover 시 wrong state
```
...
```text
"Different compiler version = diverse"
→ Same algorithm·spec
→ Common-mode 여전
```
