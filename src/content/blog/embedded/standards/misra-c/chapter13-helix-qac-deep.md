...
│   ├── EPB_ECU.prj                  ← 메인 프로젝트
│   ├── compilers/
│   │   └── gcc12-arm.qcs             ← Compiler specification
│   ├── rules/
│   │   ├── misra2012.rcc             ← Rule Configuration
│   │   ├── project_policy.rcc        ← 프로젝트 정책
│   │   └── advisory_promotion.rcc    ← Advisory→Required
│   ├── suppressions/
│   │   └── permit_files.scl          ← Permit 기반 suppression
│   ├── baseline/
│   │   └── 2024-06-baseline.qaf      ← Approved baseline
│   └── reports/
│       └── (생성된 리포트들)
└── src/
    ├── drivers/
...
Analysis:
  - QAC-only: 대부분 형식 규칙 (식별자, comment) — Polyspace는 metric 부족
...
```
프로젝트당 적정 suppression 수
  Total / KLoC = ?

  좋음:    < 5    (suppression이 5건 미만/1000줄 — 매우 깨끗)
  보통:    5~15
  주의:    15~30
...
- [Ch 11 — ISO 26262 audit walkthrough](/blog/embedded/standards/misra-c/chapter11-iso26262-audit-walkthrough)
