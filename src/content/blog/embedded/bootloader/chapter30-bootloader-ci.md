...
# QEMU를 background로 띄우고 stdout을 file로 캡처
timeout ${TIMEOUT_SEC}s qemu-system-aarch64 \
...
echo "[FAIL] U-Boot prompt not reached within ${TIMEOUT_SEC}s"
echo "--- last 30 lines ---"
tail -30 "$LOG"
kill $QEMU_PID 2>/dev/null || true
exit 1
...
sha256sum u-boot/u-boot.dtb   > u-boot.dtb.sha256

# Vault에서 ephemeral key로 서명
vault write -field=signature \
    transit/sign/${KEY_ID}/sha2-256 \
    input=$(base64 -w0 u-boot.bin.sha256) \
    > u-boot.bin.sig

# verify가 성공하는지 확인
vault write -field=valid \
    transit/verify/${KEY_ID}/sha2-256 \
    input=$(base64 -w0 u-boot.bin.sha256) \
