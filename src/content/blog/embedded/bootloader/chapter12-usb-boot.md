...
[Ch 10](/blog/embedded/bootloader/chapter10-storage-drivers)에서 본 storage stack 위에 UMS가 *얇은 USB 래퍼*로 얹히는 구조입니다.
...
`uuu`의 스크립트(`.uuu` 파일)는 이 *단계 전이*를 자동화합니다. 양산 jig가 SPI flash·eMMC가 빈 보드를 받아서 한 번에 부트로더부터 OS까지 굽는 경우, 이 스크립트 한 장이 핵심입니다. [Ch 23](/blog/embedded/bootloader/chapter23-bootrom-internals)에서 BootROM 내부를 더 깊이 봅니다.
...
호스트에서는 `uuu factory.uuu`로 한 번에 굽고, *시리얼 콘솔 로그*를 함께 캡처해서 [Ch 30](/blog/embedded/bootloader/chapter30-ci-factory)의 CI factory pipeline이 PASS/FAIL을 판정합니다. 양산 jig는 보통 *USB 허브 + 시리얼 어댑터 + 전원 제어*가 한 보드에 들어가서, 작업자는 PCB만 끼우면 됩니다.
...
- [Ch 10: storage 드라이버](/blog/embedded/bootloader/chapter10-storage-drivers) — UMS·DFU의 backend
...
- [Ch 23: BootROM 내부](/blog/embedded/bootloader/chapter23-bootrom-internals) — SDP·USB fallback의 출발점
- [Ch 30: CI factory pipeline](/blog/embedded/bootloader/chapter30-ci-factory) — `uuu` 자동화의 위층
