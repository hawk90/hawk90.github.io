...
```text
PROFINET 한 사이클 (CC-C IRT, 1 ms)
  ┌────────────────────┬──────────────────────────┐
  │  IRT (red phase)    │  NRT/RT (green phase)    │
  │  hw-scheduled        │  best-effort + priority  │
  └────────────────────┴──────────────────────────┘
   ← 200 μs reserved    ← 800 μs                    →
   for IRT traffic        for everything else

  IRT 트래픽:
    - 시각 0   : Controller → Device 1, 32 byte
    - 시각 50  : Controller → Device 2, 64 byte
    - 시각 110 : Device 1 → Controller, 32 byte
    ...
```
...
```text
PROFINET RT 프레임 (Ethernet II + VLAN tag + PN RT)
  ┌──────┬──────┬────────────┬────────┬───────────────┬─────┐
  │  DA  │  SA  │ VLAN 0x8100│ TCI    │EtherType 0x8892│ ... │
  └──────┴──────┴────────────┴────────┴───────────────┴─────┘
                              ↑          ↑
              PCP=6 (high)    PROFINET RT
```
