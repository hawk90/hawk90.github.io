...
**HBM3 12-Hi stack heat density** — 면적 121 mm² (11 × 11), 전력 12 W → **9.9 W/cm²**

| 비교 대상 | 열 밀도 |
|-----------|---------|
| 일반 CPU die | 5 – 15 W/cm² |
| GPU die (H100) — 3D 적층 X | 50 – 80 W/cm² |
| LED chip | 20 – 50 W/cm² |
| HBM3 stack (top surface 기준) | ~10 W/cm² |
...
| GPU | TDP | rack 전력 | Cooling |
|-----|-----|-----------|---------|
| A100 | 400 W | 3.2 kW (8 GPU) | 공기 (40 mm front-to-back fan), rack 12 kW |
| H100 | 700 W | 5.6 kW (8 GPU) | 공기 한계점, SXM5 module은 liquid 옵션 |
| B200 | 1000 W | 8 kW (8 GPU) | **Liquid 필수**, direct-to-die cold plate |
| GB200 NVL72 | 1200 W | 86 kW per rack (72 GPU) | 100 % liquid cooled, facility-level 인프라 필요 |
...
**Direct-to-die cooling (lapping)** — thermal stack 비교

| 구조 | 경로 | 저항 |
|------|------|------|
| 기존 (lid 포함) | cold plate → TIM2 → lid → TIM1 → die | 0.04 K/W |
| Direct-to-die | cold plate → TIM (single, liquid metal) → die | 0.015 K/W |

효과: junction temperature 약 **10 – 15 °C 낮음** → same TDP에서 더 안정적, 또는 같은 온도에서 더 높은 TDP 허용.
...
**Thermal throttling stages** (HBM3 `T_j_max = 95 °C`)

| Stage | 온도 | 동작 | BW 영향 |
|-------|------|------|---------|
| 1 | 85 °C | ASR 모드 hot 전환, refresh × 2 | −3 % |
| 2 | 90 °C | Clock frequency −10 % | −10 % |
| 3 | 95 °C | Clock frequency −25 % | −25 % |
| 4 | 100 °C | Emergency shutdown, host에 thermal interrupt | — (정지) |
...
| 사업자 | 인프라 | PUE |
|--------|--------|-----|
| KT 클라우드 천안 | H100 SXM5 (공기 냉각 700 W) | 1.4 |
| 네이버 각 춘천 | 자체 LCM(Liquid Cold-plate Module) 개발 | 1.2 |
| 삼성SDS / SK텔레콤 | B200 도입 예정, full liquid cooling 준비, 시설 retrofit 진행 | — |

Korean fab에서 HBM을 생산하면서도, 국내 AI 인프라는 cooling 측면에서 추격 중입니다.
