...
```text
HBM3E 12-Hi stack 단면 (위에서 아래로)

      ┌──────────────────────────────────────┐
   12 │   DRAM die #12 (top)                 │   ← 24 Gb DRAM
      ├──────────────────────────────────────┤
   11 │   DRAM die #11                       │
      ├──────────────────────────────────────┤
   10 │   DRAM die #10                       │
    ...
    1 │   DRAM die #1 (bottom)               │
      ├──────────────────────────────────────┤
    0 │   Base die (logic, PHY)              │   ← 로직 공정 (12 nm급)
      ├──────────────────────────────────────┤
      │   microbump (HBM bump field)         │   ← 55 μm pitch
      └──────────────────────────────────────┘
                       │
              interposer로 연결
```
...
```text
HBM 세대별 stack 두께·height 제한

JEDEC 표준 max height: 720 μm (HBM3)
                       775 μm (HBM3E 12-Hi)

세대별 stack height:
├── HBM2  4-Hi  : 약 400 μm
├── HBM2E 8-Hi  : 약 720 μm
├── HBM3  12-Hi : 약 720 μm (die 50 μm thinning)
└── HBM3E 12-Hi : 약 775 μm
```
...
```text
HBM3 DRAM die top-down (24 Gb die)

┌──────────────────────────────────────────────┐
│   Ch0 PC0 bank0..15       Ch0 PC1 bank0..15  │
│   ───────────────────     ──────────────────  │
│   Ch1 PC0 ...             Ch1 PC1 ...        │
│   ...                                        │
│   Ch7 PC0 ...             Ch7 PC1 ...        │
...
실제로는 KGSD test와 repair로 보정하지만,
"한 die가 불량이면 stack 전체가 불량"이 기본 위험.
```
...
│   normal row N-1                             │
│   ────────────────────────                   │
│   redundant row 0    ← 불량 row와 swap       │
│   redundant row 1                            │
│   ...                                        │
│   redundant row 32                           │
└──────────────────────────────────────────────┘

base die의 e-fuse에 remap 정보 저장
boot 시 row decoder가 자동으로 redundant로 redirect
```
