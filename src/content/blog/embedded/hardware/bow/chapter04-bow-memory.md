...
전통적 메모리 계층
┌──────────────────────────────────────────┐
│  SoC (CPU + Memory Controller)           │
│   ├── core                               │
│   ├── L2/L3 cache                        │
│   └── DDR PHY                            │
└──────────┬───────────────────────────────┘
           │ DDR4/5 (PCB trace, 6~7 Gbps/lane)
           ▼
        ┌──────────┐
        │   DIMM   │
        └──────────┘
...
한계:
- DDR PHY 면적이 SoC die의 20~30% 차지
- 다양한 메모리 (DDR/HBM/LPDDR/CXL) 지원하려면 PHY 여러 개
- 신호 무결성 한계로 lane 수·속도 정체
```
...
BoW + CXL.mem 구성
┌─────────────────────────┐         ┌──────────────────────┐
│  Host SoC               │         │  Memory Expander     │
│  (CPU + CXL.mem master) │ ◄─BoW─► │  (CXL.mem slave +    │
│                         │         │   DDR5 controller)   │
└─────────────────────────┘         └──────────┬───────────┘
                                               │ DDR5
                                               ▼
...
```text
Custom 예: AI accelerator memory link
┌─────────────────────┐         ┌────────────────────┐
│  AI accelerator     │         │  HBM-class memory  │
│  (compute units)    │ ◄─BoW─► │  controller die    │
└─────────────────────┘         └──────────┬─────────┘
                                            │
                                            ▼
                                         HBM stack
...
BoW config: 8 Slice × 16 lane × 12 Gbps = 1.5 Tbps (192 GB/s)
DDR5 config: 4 channel × DDR5-6400 = ~200 GB/s
```
