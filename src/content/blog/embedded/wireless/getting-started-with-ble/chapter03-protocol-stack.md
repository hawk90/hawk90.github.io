...
![Single-chip(왼쪽)은 모든 계층이 한 칩, Dual-chip(오른쪽)은 Host와 Controller를 분리하고 HCI가 UART/USB를 흐름](/images/blog/ble/diagrams/ch03-single-vs-dual-chip.svg)
...
대표 구성: *Raspberry Pi 내장 BCM43438*, *USB Bluetooth dongle*, *Linux 서버 + BLE 모듈*. Host는 *Linux의 BlueZ 또는 Zephyr*, Controller는 *별도 칩*. HCI 패킷이 *실제로 UART/USB를 흐릅니다*. UART는 *115200~3M baud*, USB는 *Bluetooth HID transport*가 표준입니다. 대표 Controller 칩으로 *CYW20706, BCM4343*이 있고, *nRF52840에 Zephyr의 HCI controller 펌웨어*를 올려 dual-chip controller로도 씁니다.
