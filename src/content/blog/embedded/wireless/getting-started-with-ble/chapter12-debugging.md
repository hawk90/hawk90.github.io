...
1. Scanner
   - 주변 BLE 디바이스 + adv data + RSSI
   - 필터 (이름, RSSI, manufacturer)
   - Raw adv data hex 표시

2. Device 페이지 (디바이스 클릭 후)
   - Service 트리 (16-bit/128-bit UUID)
   - Characteristic 별 read/write/notify 버튼
   - CCCD 활성화 토글
   - MTU 협상
   - Pairing 트리거
   - Connection parameter 표시

3. Logger
   - 모든 ATT operation 시간순 기록
   - .txt export 가능

4. Configuration
   - BLE adv 시뮬레이션
   - GATT server 시뮬레이션 (HID 헤드셋 흉내 등)
...
해결  : 클라이언트 코드에서 setCharacteristicNotification + writeDescriptor
        또는 시뮬레이션은 nRF Connect 앱에서 notification 버튼
```
