...
```text
링크 budget = TX power + 양쪽 안테나 이득 - 경로 손실
            - 수신 감도 (Rx sensitivity)

Coded S=8 sensitivity: -103 dBm 수준
1M PHY    sensitivity:  -93 dBm 수준
                       ───────────
차이                   +10 dB ≈ 거리 3~4배
```
...
```text
Periodic Advertising
  T ── T ── T ── T ── T ── T ── T ──  (정확한 interval)
  └ 광고 패킷, 수신자는 다음 T 시각을 알고 sleep
```
...
AoA (Angle of Arrival)
  TX: 단일 안테나
  RX: 안테나 어레이 (보통 4~16개)
  → RX 쪽이 위상차로 신호 도래각 계산

AoD (Angle of Departure)
  TX: 안테나 어레이
  RX: 단일 안테나
  → TX 쪽이 안테나 스위칭, RX가 IQ 샘플로 각도 추출
```
...
└── 서버가 "내 GATT 트리 바꿨음" 알림
    Client는 이때만 재발견, 나머지는 캐시 사용
```
