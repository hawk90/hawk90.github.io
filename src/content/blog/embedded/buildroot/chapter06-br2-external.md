...
| `external.mk` | 패키지의 `.mk`를 include |
| `Config.in` | 패키지의 `Config.in`을 include |
...
```bash
make acme-firmware-source
make acme-firmware
make acme-firmware-show-info
```

순서대로 *tarball 다운로드만*, *build + install*, *메타데이터 확인*입니다.
