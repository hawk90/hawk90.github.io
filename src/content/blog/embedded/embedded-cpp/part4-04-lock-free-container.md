...
[Part 4-03](/blog/embedded/embedded-cpp/part4-03-lock-free-basics)의 코드. 다시.
...
- SPSC queue가 *임베디드 lock-free의 표준*. ISR ↔ task 통신.
- *Producer는 head, Consumer는 tail* — 서로 무관 → CAS 불필요.
- *Cache line alignment* (alignas(64))로 false sharing 회피.
- MPMC는 *복잡 — 검증된 라이브러리* (Boost, moodycamel).
- *ABA problem*에 *tagged pointer/hazard pointer*. 또는 *MPMC 회피*.
- Cortex-M0/M0+는 *atomic 불가* — critical section.
