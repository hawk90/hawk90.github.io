...
slug: "embedded/embedded-security/chapter04-trustzone"
...
| CPU 상태 | 버스 NS bit | 메모리 controller 동작 |
|----------|-------------|----------------------|
| Secure | `NS=0` (secure access) | region의 NS 설정과 비교 — secure region이면 통과, non-secure region이면 정책에 따라 차단 |
| Non-Secure | `NS=1` | secure region 접근 시도 → *차단, fault*. non-secure region이면 통과 |
...
![EL × Secure/Non-secure matrix](/images/blog/embedded-security/diagrams/ch04-trustzone-el-matrix.svg)
...
![STM32L552 secure/non-secure memory map](/images/blog/embedded-security/diagrams/ch04-stm32l552-memory-map.svg)
