...
1. Device authentication (Ch 12: SPDM 흐름)
       │
       ▼
2. Key exchange (IDE_KM via SPDM)
   ├─ host: random k_h
   ├─ device: random k_d
   └─ shared K = KDF(k_h ⊕ k_d, context)
       │
       ▼
3. IDE stream 활성화 (k_data·k_ctrl 각각)
       │
       ▼
4. Key refresh (주기적, 보통 sequence counter overflow 전)
       │
       ▼
5. Tear-down (link drop·security event)
```
...
[Ch 9 (HBM·GDDR 심화)](/blog/embedded/hardware/hbm/chapter09-cxl-mem)에서 봤듯 *Samsung CMM-D·SK Hynix Niagara*가 *CXL 메모리 디바이스 시장*의 선두권입니다. *2025년 revision*부터 *CXL 2.0 IDE*가 *qualification 필수 요건*에 들어갔습니다.

*Samsung Knox*와 *SK Hynix*의 *MARO* 컨트롤러는 *AES-GCM 256 가속기*를 *디바이스 controller die 내부*에 내장해 *firmware update만으로 IDE 활성화*가 가능합니다. *Astera Labs Leo*는 *fabless 모델*이라 *DRAM은 한국 제품·controller는 자체 ASIC*으로 *IDE 가속기를 chip 안에 통합*합니다.
