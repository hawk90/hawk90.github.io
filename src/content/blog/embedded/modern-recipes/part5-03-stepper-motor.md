...
MCU                        DRV8825
─────                      ─────────
STEP   ──────────────►     STEP  ─ rising edge마다 1 step
DIR    ──────────────►     DIR   ─ 0 = CW, 1 = CCW
EN     ──────────────►     /EN   ─ 0 = enable (active-low)
GPIO[3]──────────────►     MS1/2/3 ─ micro-step select

                           AOUT1/2, BOUT1/2 → motor
                           VMOT (8-45V), VREF (current limit)
```
