...
```text
CPU:           명령을 *순차적*으로 실행 (한 cycle 한 명령)
FPGA:          *모든 회로가 동시*에 동작 (한 cycle 모든 회로)

CPU:           clock 빠름 (GHz), latency 가변
FPGA:          clock 느림 (수백 MHz), latency 결정적

CPU:           SW로 알고리즘 수정 가능
FPGA:          알고리즘 = 회로 → 재합성 필요

CPU:           매 명령 fetch/decode overhead
FPGA:          fetch/decode 없음 — 회로가 그 자체
```
