...
```text
Baud = 9600일 때, sampling clock = 9600 × 16 = 153.6 kHz

 start  ←── 1 bit (104 µs) ──→
   ┐             ↑ sample at 8, 9, 10 → majority vote
    │            ↑     ↑     ↑
    └────────────│─────│─────│─────
                  중심 부근 3 sample로 0/1 판정
```
...
- 더 깊이 — [Practical RTOS Internals: ISR latency](/blog/embedded/rtos/practical-internals/)
