...
│                                                             │
│   Role (역할):                                               │
│   "이 객체가 시스템에서 어떤 역할을 하는가?"                   │
│   예: AuctionEventListener, MessageTranslator               │
│                                                             │
│   Responsibility (책임):                                     │
│   "이 객체가 무엇을 해야 하는가?"                             │
│   예: 경매 이벤트 수신, 메시지 번역                          │
...
│   Collaboration (협력):                                      │
│   "이 객체가 누구와 협력하는가?"                              │
│   예: Translator → Listener에게 이벤트 전달                  │
└─────────────────────────────────────────────────────────────┘
```
...
│   ┌──────────────────┐                                     │
│   │ MessageTranslator│ ◄─── 테스트 대상                    │
│   └────────┬─────────┘                                     │
│            │                                                │
│            │ 협력                                           │
│            ▼                                                │
│   ┌──────────────────┐                                     │
│   │ AuctionEvent     │ ◄─── Mock 객체                      │
│   │ Listener (Mock)  │      (협력자 역할 수행)              │
│   └──────────────────┘                                     │
└─────────────────────────────────────────────────────────────┘
```
...
│              테스트를 통한 인터페이스 발견                    │
│                                                             │
│   1. 테스트 작성 (아직 인터페이스 없음)                      │
│      └─► "이 객체가 협력자에게 무엇을 요청해야 할까?"        │
│                                                             │
│   2. Mock 정의 (인터페이스 스케치)                           │
│      └─► "이런 메서드가 필요하겠군"                         │
│                                                             │
