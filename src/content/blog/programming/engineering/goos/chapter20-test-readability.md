...
│  • Should_ThrowException_When_BalanceIsNegative                 │
│                                                                 │
│  패턴 2: MethodName_Scenario_ExpectedResult                     │
│  ─────────────────────────────────────────────                  │
│  • Withdraw_SufficientFunds_DecreasesBalance                    │
│  • Login_InvalidPassword_ReturnsError                           │
│                                                                 │
│  패턴 3: Given_When_Then (BDD 스타일)                           │
│  ─────────────────────────────────────────────                  │
│  • GivenEmptyCart_WhenAddItem_ThenCartHasOneItem                │
│  • GivenLoggedInUser_WhenLogout_ThenRedirectToHome              │
│                                                                 │
│  핵심 원칙:                                                     │
│  • 무엇을 테스트하는지 명확히                                   │
│  • 조건과 기대 결과 포함                                        │
...
│  When (실행)                                                    │
│  ─────────────────                                              │
│  "다른 입찰자가 더 높은 가격을 제시하면"                        │
│                                                                 │
│  Then (기대 결과)                                               │
│  ─────────────────                                              │
│  "Sniper는 그보다 높은 가격으로 입찰해야 한다"                  │
│                                                                 │
│  차이점:                                                        │
│  • AAA: 기술적 관점 (어떻게)                                    │
│  • GWT: 비즈니스 관점 (무엇을)                                  │
...
```
┌─────────────────────────────────────────────────────────────────┐
│                 OBJECT MOTHER PROBLEMS                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. 조합 폭발                                                   │
│     • N개의 옵션 → 2^N 개의 메서드 필요                         │
│     • 유지보수 어려움                                           │
│                                                                 │
│  2. 숨겨진 의도                                                 │
│     • create_order_for_happy_path()가 정확히 무엇인지?          │
│     • 테스트 의도가 팩토리 메서드에 숨김                        │
│                                                                 │
│  3. 결합도 증가                                                 │
│     • 팩토리 변경 시 많은 테스트 영향                           │
