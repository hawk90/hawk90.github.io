...
TDD는 단순히 테스트를 먼저 작성하는 것이 아니다. 테스트가 설계의 첫 사용자가 되어 인터페이스를 발견하고, 역할을 정의하며, 좋은 객체지향 설계를 이끌어낸다.
...
│   ─────────────────────────────────────────────────         │
│                                                             │
│   셋업이 너무 복잡           →  객체가 너무 많은 의존성       │
│   Mock이 너무 많음           →  객체가 너무 많은 협력자       │
│   테스트가 깨지기 쉬움       →  구현 세부사항에 결합          │
│   테스트 이름이 어색         →  객체의 책임이 불명확          │
│   assertion이 많음          →  한 테스트에 여러 동작 검증    │
│   테스트 코드 중복           →  추상화 부족                  │
│                                                             │
│   "테스트하기 어렵다" = "사용하기 어렵다"                    │
│                                                             │
...
│   3. 역할(Role) 정의                                        │
│      ─────────────                                          │
│      interface Auction { void bid(int amount); }            │
│                                                             │
│   4. Mock으로 검증                                           │
│      ────────────                                           │
