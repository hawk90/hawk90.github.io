...
![프로젝트 시작의 두 가지 위험](/images/blog/goos/diagrams/ch04-project-risks.svg)

## 4.2 인수 테스트부터 시작

### 첫 번째 테스트

![바깥에서 시작하라](/images/blog/goos/diagrams/ch04-first-test-requirements.svg)
...
매우 많은 작업이지만 모든 것이 필요합니다.

## 4.3 빌드/배포 인프라 우선

### 왜 인프라가 먼저인가?

![인프라 우선의 이유](/images/blog/goos/diagrams/ch04-infrastructure-priority.svg)
...
| 접근법 | 장점 | 단점 |
|--------|------|------|
| 인프라 우선 (Infrastructure First) | 기술 위험 조기 제거, 팀 전체가 같은 환경에서 작업, 배포 가능성 조기 확인 | 비즈니스 가치 지연, "진짜 기능"까지 시간 소요 |
| 도메인 우선 (Domain First) | 빠른 비즈니스 피드백, 도메인 이해 우선 | 통합 문제 후반에 발견, "내 컴퓨터에서는 동작해요" 위험 |
...
| 단계 | 목표 | 세부 작업 |
|------|------|-----------|
| 1 | 컴파일 가능하게 만들기 | 필요한 클래스 스텁 생성, 메서드 시그니처 정의, 빈 구현으로 컴파일 통과 |
| 2 | 테스트 실행 가능하게 만들기 | 가짜 서버 기본 동작, 애플리케이션 시작/종료, 아직 실패하지만 실행은 됨 |
| 3 | 테스트 통과시키기 | 실제 통신 구현, 메시지 파싱/생성, UI 상태 업데이트 |
...
- Agile/Lean Engineering — TDD as XP — Walking Skeleton의 XP/Lean 맥락
