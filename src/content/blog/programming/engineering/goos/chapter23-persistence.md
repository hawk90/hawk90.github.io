...
│                   │ E2E DB  │  ← 실제 DB, 느림          │
│                  ┌───────────┐                          │
│                 │Integration│  ← 실제 DB, 중간 속도     │
│                ┌─────────────┐                          │
│               │ Repository  │  ← In-memory/Mock         │
│              ┌───────────────┐                          │
│             │  Domain Logic │  ← 순수 로직, DB 없음     │
│            └─────────────────┘                          │
│                                                         │
│  아래로 갈수록: 더 빠름, 더 많이 실행                     │
│  위로 갈수록: 더 현실적, 적게 실행                        │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
│  └─ 수동 테스트만, 회귀 버그 빈번                        │
│                                                         │
│  Level 1: 테스트 후작성 (Test-After)                    │
│  └─ 코드 완성 후 테스트 추가, 커버리지 낮음              │
│                                                         │
│  Level 2: 테스트 선작성 (Test-First)                    │
│  └─ 테스트 먼저, 하지만 설계 영향 제한적                 │
│                                                         │
│  Level 3: TDD (Red-Green-Refactor)                     │
│  └─ 테스트가 설계를 주도, 지속적 리팩토링                │
│                                                         │
│  Level 4: Outside-In TDD                               │
│  └─ 인수 테스트부터 시작, mock으로 협력 설계             │
│                                                         │
│  Level 5: Continuous Deployment                        │
│  └─ TDD + CI/CD, 자동 배포, 피드백 루프 최소화          │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
