...
|--------|-------------------|
| UI | 상태 `LOST` 표시 |
| Domain | `AuctionSniper`가 패배 판정 |
| Infrastructure | XMPP 메시지 수신 |
| External | 경매 서버 `CLOSE` 이벤트 |
...
참여자: `Test`, `Sniper`, `Auction`. 메시지 시퀀스.

| # | 발신 | 메시지 | 도착 |
|---|------|--------|------|
| 1 | Test | `start auction` | Auction |
| 2 | Test | `start sniper` | Sniper |
| 3 | Sniper | `JOIN` | Auction |
| 4 | Test | `close auction` | Auction |
| 5 | Auction | `CLOSE` | Sniper |
| 6 | Sniper | (내부) 상태 `LOST`로 갱신 | (UI) |
| 7 | Test | `verify LOST` | Sniper |
