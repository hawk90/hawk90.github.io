...
> 코드 블록에 *이름을 붙여 별도 method*로. *Refactor* 단계 핵심 도구. 가독성·중복 제거·test 용이성.
...
### 4. *부수효과 숨김*
추출 method 안 *외부 state 변경* → 호출자가 모름. *순수 또는 명확*.
...
class method 아닌 *module-level function*으로:
...
method 호출 추가 — JIT inline. 무관. *과도한 분해*가 *cache miss* 가능 (극히 드물게).
