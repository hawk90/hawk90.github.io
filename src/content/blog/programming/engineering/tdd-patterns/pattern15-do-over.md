...
> 너무 깊이 막혔을 때 지금 작업을 버리고 처음부터. 첫 시도의 학습이 두 번째 시도의 기반.
...
**Do Over**는 두 번째. 지금 코드를 버리고 깨끗한 상태에서 다시.
...
복잡한 architecture가 문제. 함수 한 개로 시작이 옳음.
...
조금만 어려워도 매번 restart → 진척 0. 진짜 막힌 경우만.

### 4. 같은 접근 반복

Do Over 후 같은 방식으로 시작 → 같은 막힘. 학습 반영.
...
추상 — 코드 성능 무관. 개발 시간에 큰 영향. 한 번의 Do Over가 2-3배 시간 절약.
