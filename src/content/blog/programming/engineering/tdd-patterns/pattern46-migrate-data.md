...
호출처가 수십 곳이면 모두 동시 변경 어려움. **Migrate Data**는 3단계: expand → migrate → contract.
...
ALTER TABLE 직접 → rollback 어려움. expand 가능한 변경만 single deploy.
...
new system을 production 데이터로 실행하지만 결과 사용 안 함 — 검증 단계.
...
- Dual-write는 2배 write. throughput 영향.
