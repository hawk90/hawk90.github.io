...
### 2. *Method 누락*
production class에 method 추가하면서 *Null Object에 안 추가* → NPE/AttributeError. *abstract base class*로 강제.
...
### 5. *Singleton 강제*
NullObject가 *singleton instance*인 척 → multiple Null type 필요할 때 부담.

### 6. *너무 많은 Null Object*
모든 dependency에 NullObject → boilerplate. 진짜 *no-op 의미*인 case만.
...
no-op method 호출은 *JIT이 dead code 제거*. 거의 *비용 0*. singleton 활용으로 *allocation 없음*.
