...
> 실제 객체가 테스트하기 어려울 때(비쌈/느림/비결정적/부수효과) mock 객체로 대체. 실제처럼 보이지만 테스트 제어 하의 가짜.
...
- 테스트가 외부 시스템에 의존 (DB, network, file).
- 테스트가 느림, flaky.
- 비용 발생 (API 호출).
- *시간/random* 의존.
...
WireMock, MockServer — 전체 service를 mock. integration test에 적합.
...
[Hide Delegate](/blog/programming/design/refactoring-catalog/pattern18-hide-delegate)와 결합 — 객체에 명령만 → mock 검증이 자연스러운 boundary.
