...
slug: "programming/engineering/tdd-patterns/pattern03-test-list"
...
> 코드 작성 전에 "어떤 테스트를 쓸지"를 목록으로. 머릿속 부담을 외부로 옮겨 집중·진행률·재개를 돕는다.

## 동기

머릿속에 모든 테스트 케이스를 담고 있으면 코드 작성 중 "이 경우도 테스트해야 하는데..."가 떠오른다. 그 순간 작업 중단. 집중이 깨진다.
...
Beck은 이것을 "working memory의 외부화"라 부른다. 뇌는 테스트 목록을 기억하는 데 쓰지 말고 코드 작성에 쓰자.
...
시작 전 모든 케이스 다 적기에 매달림 → 코드 시작 지연. brain dump면 충분.
...
IntelliJ/VS Code의 TODO panel이 자동 수집.
...
test method 이름이 곧 test list 항목. pending 표시는 `@pytest.mark.skip`.
...
추상적 패턴 — 코드 성능과 무관. 작업 효율에 미치는 영향은 큼.
