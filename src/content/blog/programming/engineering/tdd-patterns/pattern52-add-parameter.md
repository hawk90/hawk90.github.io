...
> Method에 *새 parameter 추가*. *기본값*으로 *호환성 유지* + 점진적 이전.
...
호출처가 *수십 곳*이면 *signature 변경* 부담. *default value*로 점진.
...
parameter 추가 자체는 *무관*. *너무 많은 parameter*는 stack pressure (CPU 등록기 spill). 일반 무시.
