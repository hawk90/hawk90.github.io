...
> 테스트를 assertion부터 쓰고 거꾸로 올라가며 action·setup을 채운다. 목적이 먼저 명확해진다.
...
목적(reservation confirm 후 상태 + email 전송)이 먼저, 그것을 검증하기 위한 fixture만 도입.

## 자주 보는 안티패턴

### 1. 복사된 setup 그대로 사용

다른 테스트 fixture를 복사해서 불필요 부분 남김. Assert First 정신 위배.
...
표준 패턴. Assert First는 작성 순서는 반대지만 최종 구조는 같음.
...
BDD의 Given-When-Then을 반대로 작성하면 같은 구조. Outside-In TDD (London school)의 출발점.
...
작성 순서일 뿐 — 코드 성능 무관. fixture 최소화로 test 실행 빨라질 수 있음.
