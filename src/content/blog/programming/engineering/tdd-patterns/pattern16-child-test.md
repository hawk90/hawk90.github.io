...
스텝 크기를 동적으로 조절하는 가장 흔한 도구.

### 신호

- Parent test가 30분+ Red.
- 한 번에 여러 개념 도입 필요.
- 어디서부터 production 작성할지 모름.
- production 변경 수십 줄이 한 테스트.
...
Children이 covering하면 Parent의 복잡함은 부담.
...
트리 구조로 분해. 단 깊이 3 이상이면 Do Over 고려.
...
Child test로 test 수 증가 → 실행 시간 ↑. 단 fast unit이라 보통 무관. Parent 1개 vs Children 5개 — 보통 비슷.
