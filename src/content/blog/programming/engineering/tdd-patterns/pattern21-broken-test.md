...
퇴근 시간 — 코드를 짜다 멈춰야 한다. 다음 날 "어디까지 했더라?" — 코드 보고 상황 재구성에 시간 소모.

**Broken Test**는 작업을 멈출 때 의도적으로 실패하는 테스트를 남긴다. 다음 날 test 실행 → *fail 메시지 = bookmark*.
...
VS Code/IntelliJ의 bookmark와 함께 — 코드 위치 + 의도.
...
추상 — 코드 성능 무관. 개발자 throughput에 큰 효과. 매일 5-10분 절약 → 연간 수십 시간.
