...
> 에러 경로 테스트 시 *의도적으로 예외를 던지는 fake*를 사용. 자동차 충돌 테스트 인형처럼 코드의 *충돌 상황을 결정적*으로 재현.

## 동기 (Motivation)

에러 처리 코드 테스트 — *에러가 실제로 발생*해야 한다. 하지만 실제 에러는:
...
- 에러 처리 코드가 *production에서만 발견*되는 버그.
- 코드 커버리지에서 *catch 블록 0%*.
- "내가 작성한 에러 처리가 *진짜 동작*하나?" 불안.
...
dummy의 *exception throw*는 *극히 빠름*. test 부담 0. 단 *수많은 chaos test*는 *time 누적* — 별도 schedule.
