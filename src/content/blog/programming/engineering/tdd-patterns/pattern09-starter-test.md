...
slug: "programming/engineering/tdd-patterns/pattern09-starter-test"
...
> 새 기능 시작 시 가장 작고 퇴화된 케이스를 첫 테스트로. 인프라 검증 + 모멘텀 확보가 목적.

## 동기

새 기능을 시작하려는데 어디서부터 손대야 할지 모름. Test List는 있지만 첫 테스트가 막막.
...
다음 테스트가 일반화를 끌어낸다 ([Pattern 24: Triangulate](/blog/programming/engineering/tdd-patterns/pattern24-triangulate)).
...
```
Starter는 시작. One Step Test로 계속 진전.
...
```
검증할 production 동작 없음. starter도 어떤 동작 검증이어야.
...
example code를 그대로 복사 — 진짜 starter가 아닌 demo. 실제 도메인.

## Modern variants

### Walking skeleton (Cockburn)

end-to-end 최소 동작 시스템. UI → API → DB까지 얇은 슬라이스로 한 번 통과. 이후 기능 추가.
...
Yeoman, Rails generator 등이 starter test를 자동 생성 (예: `rails generate scaffold User name email`).
