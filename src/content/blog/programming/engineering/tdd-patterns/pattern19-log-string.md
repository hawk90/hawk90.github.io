...
slug: "programming/engineering/tdd-patterns/pattern19-log-string"
...
> 협력 객체가 호출될 때마다 log에 기록하고, 테스트 끝에 expected log와 비교. mock보다 단순한 호출 순서 검증.
...
Mock의 `assert_called_before()`도 있지만 **Log String**이 더 단순하다.
...
모든 내부 호출 기록 → 노이즈, 결합. 핵심 이벤트만.

### 3. Order infinitive

순서 무관한데 list로 검증 → false negative. set 또는 contains.
...
internal helper까지 log → 리팩터링 시 test 깨짐. 외부 동작만.
...
큰 log를 baseline 파일과 자동 비교. ApprovalTests library.
