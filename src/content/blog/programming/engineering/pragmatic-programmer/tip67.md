...
**Top-Down (위에서 아래로):** 전체 아키텍처를 설계하고, 모든 모듈 인터페이스를 정의한 뒤, 각 모듈을 위에서 아래로 구현하고, 마지막에 통합한다. 문제는 마지막에 통합할 때 모든 문제가 터진다는 것이다.

**Bottom-Up (아래에서 위로):** 기반 유틸리티 라이브러리를 먼저 구현하고, 그 위에 서비스 레이어를 올리고, 그 위에 UI를 구현한 뒤, 마지막에 연결한다. 문제는 필요 없는 기능을 미리 만들고, 실제 사용 시 맞지 않는다는 것이다.

## End-to-End 접근법

작은 기능을 UI부터 데이터베이스까지 완전히 동작하게 만든다. 예를 들어 "사용자 로그인" 기능이라면:
...
이것이 완전히 동작한 후에 다음 기능으로 넘어간다.

## 예: 온라인 상점

| 전통적 접근 | End-to-End 접근 |
|------------|----------------|
| 1달차: DB 스키마 설계 | 1주차: 상품 목록 보기 (UI→API→DB) |
| 2달차: 백엔드 API 전체 구현 | 2주차: 상품 상세 보기 (UI→API→DB) |
| 3달차: 프론트엔드 구현 | 3주차: 장바구니 추가 (UI→API→DB) |
| 4달차: 통합 및 테스트 | 4주차: 결제 (UI→API→결제→DB) |
| **4달 후에야 동작 확인** | **매주 동작 확인** |
