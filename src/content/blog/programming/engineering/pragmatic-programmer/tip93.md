...
같은 버그를 두 번 찾는 것은 낭비다. 버그가 발생하면 수동으로 발견하고, 수정하고, 시간이 지나면 같은 버그가 재발하고, 또 수동으로 발견하는 악순환이 반복된다.
...
버그 리포트 "사용자가 이메일에 + 기호를 넣으면 로그인 실패"를 테스트로 바꾼다.
...
예를 들어 이메일에 `+` 기호 처리가 안 되는 버그를 발견했다면 다음을 검사한다.
