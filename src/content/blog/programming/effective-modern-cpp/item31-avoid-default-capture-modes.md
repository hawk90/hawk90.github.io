...
람다의 기본 캡처 모드 `[&]`(모두 참조 캡처)와 `[=]`(모두 값 캡처)는 편하지만 **댕글링 참조**와 **의도치 않은 캡처**를 만듭니다. 명시적으로 캡처할 변수를 적는 게 안전.
...
기본 캡처 `[=]`/`[&]`는 **사용된 변수**만 자동 캡처. (`[=]`는 모든 자동 변수가 아님.)
...
`[=]`에 멤버처럼 보이는 게 사실 `this`. `Widget` 객체가 사라지면 `this` 댕글링.
...
값을 복사한 것처럼 보이지만 **실제로는 매번 같은 static을 읽음**. divisor가 다른 곳에서 변경되면 람다 동작도 바뀜.

→ 명시적 캡처면 컴파일 에러로 알 수 있음:
...
표현식의 결과를 새 변수 `name`에 저장. 자세한 건 [항목 32](/blog/programming/effective-modern-cpp/item32-use-init-capture-to-move-objects-into-closures).
...
기본 `[=]`가 `this`를 캡처하는 건 C++20부터 deprecated. `[=, this]` 또는 `[*this]` 명시 권장.

## 핵심 정리

1. `[&]`는 **댕글링 참조** 위험 — 람다가 캡처 스코프보다 오래 살면 UB
2. `[=]`는 **`this`를 캡처**해 멤버 접근이 의외의 동작
3. `[=]`는 **static 변수를 캡처하지 않음** — 오해 유발
4. **명시적 캡처가 안전** — 의도가 코드에 드러남
5. C++14 init capture `[x = ...]`로 멤버 값 복사
6. C++17 `[*this]`로 객체 자체 복사
7. 짧은 수명 람다(`for_each`)면 `[&]` OK

## 관련 항목

- [항목 32: init capture](/blog/programming/effective-modern-cpp/item32-use-init-capture-to-move-objects-into-closures)
- [항목 33: decltype + forward](/blog/programming/effective-modern-cpp/item33-use-decltype-on-auto-parameters-when-forwarding)
- [항목 34: 람다 vs bind](/blog/programming/effective-modern-cpp/item34-prefer-lambdas-to-std-bind)
