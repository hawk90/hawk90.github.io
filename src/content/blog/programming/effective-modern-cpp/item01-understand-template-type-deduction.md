...
`int& f()`, `int f()`, `int&& f()` — **반환 타입 한 글자 차이**로 호출 결과의 값 카테고리가 완전히 달라집니다. "함수가 무엇을 돌려주는가"를 기준으로 비교하면 명확해집니다.
...
getRef() = 10;        // OK! storage = 10 과 동일
                      // 반환값이 lvalue → '=' 의 왼쪽에 올 수 있음
int* p = &getRef();   // OK! 주소도 구할 수 있음
```

`storage`라는 **메모리에 자리 잡은 진짜 객체**를 가리키므로 lvalue입니다.
...
getValue() = 5;       // 에러! 임시 객체는 '=' 의 왼쪽에 못 옴
int* p = &getValue(); // 에러! 임시 객체는 주소를 구할 수 없음
int y = getValue();   // OK: 값을 y에 복사
```

반환값은 어디 저장된 변수가 아니라 표현식 도중에만 존재하는 임시 객체입니다 → prvalue.
...
`storage`는 함수 밖에 그대로 살아있지만, 반환 타입이 `int&&`라서 호출 결과는 "곧 사라질 객체"로 취급됩니다 → xvalue. 이 카테고리 덕분에 `std::move`가 lvalue를 rvalue처럼 다룰 수 있습니다.
...
f1() = 99;     // OK   — lvalue, x를 덮어씀
f2() = 99;     // 에러 — prvalue (임시 객체)
f3() = 99;     // OK이지만 위험 — xvalue도 실제 메모리에 있음 → x 덮어씀

&f1();         // OK   — 실제 객체를 가리킴
&f2();         // 에러 — 임시 객체는 주소 없음
&f3();         // OK   — xvalue도 실제 객체를 가리킴
```

**기억할 한 줄**

- 참조(`&`, `&&`) 반환 = **어딘가의 객체**를 가리킴 → lvalue 또는 xvalue
- 값(`int`) 반환 = **그 자리에 새로 만들어진 임시** → prvalue
...
값 분류를 이야기할 때 자주 등장하는 **임시 객체**(temporary object)는 prvalue를 이해하는 핵심입니다.

**임시 객체란?**

표현식을 평가하는 동안 컴파일러가 자동으로 만드는, **이름 없는** 객체입니다. 대부분 prvalue로 분류되고, 그 표현식이 끝나는 시점까지만 살아 있다가 사라집니다.
...
take("hello");
//   └── const char* → std::string 임시 객체가 만들어져 전달됨
```

3. **함수의 값 반환**
```cpp
std::string make() { return "x"; }
make();   // 반환값은 임시 객체 (RVO로 최적화될 수 있음)
```
...
"컴파일러가 자동으로 만든다"는 말은 추상적으로 들리지만, 실제로는 **현재 함수의 스택 프레임 안 자동 저장소(automatic storage)** 한 자리를 잠깐 빌려 쓰는 것에 가깝습니다. `new`로 힙에 잡는 게 아니므로 우리가 따로 `delete`할 필요가 없습니다.

```cpp
void caller() {
    bar(std::string("hello"));
    //  └── caller()의 스택 프레임 안에 임시 string의 자리가 잡힘
    //      (string 객체 본체: 보통 16~32 byte, 포인터/크기/용량 등)
    //      "hello" 문자 데이터는 string 내부 SSO 버퍼나 힙에 별도 저장
}   // ; 에서 소멸자 호출, 함수 종료 시 스택 자리 자동 회수
```

**객체 종류별로 실제 위치는 조금씩 다름**

| 임시 객체 종류 | 실제 저장 위치 |
| --- | --- |
| `1 + 2` 같은 작은 정수/실수 | 보통 **CPU 레지스터** — 메모리에 안 갈 수도 있음 |
...
임시 객체의 "소멸"은 두 단계가 합쳐진 일입니다.

1. **소멸자 호출** — 객체가 잡고 있던 자원(힙 메모리, 파일 핸들 등)을 정리
2. **스택 자리 회수** — 함수가 끝나면 스택 프레임 전체가 사라지면서 자동으로 회수
...
C++ 컴파일러는 임시 객체 생성을 **건너뛸 권한**이 있습니다. C++17부터는 일부 경우에 강제됩니다(guaranteed copy elision).
...
// 실제로는:
//   컴파일러가 s 자리에 곧바로 string을 구성
//   임시 객체는 메모리상 한 번도 별도로 존재하지 않음
```

즉, 소스코드에 적힌 `std::string("hi")`가 실제 메모리에 **별개의 임시 자리**로 잡힌다는 보장은 없습니다. 컴파일러는 가능한 한 호출자 변수의 자리에 객체를 곧바로 구성합니다.

**임시 객체의 수명**

임시 객체는 보통 그것이 등장한 **전체 표현식이 끝나는 지점(세미콜론)**에서 소멸합니다.

```cpp
foo(std::string("a"));   // foo() 호출 동안만 임시 객체가 살아 있음
                         // ; 에 도달하는 순간 소멸
```

**예외: const 참조나 rvalue 참조에 묶이면 수명 연장**

임시 객체를 `const T&` 또는 `T&&`에 바인딩하면 수명이 그 참조의 수명까지 연장됩니다.

```cpp
const std::string& ref = std::string("hello");
// 임시 객체의 수명이 ref와 같아짐
std::cout << ref;   // OK: 아직 살아 있음

std::string&& rref = std::string("world");
// 마찬가지로 rref가 살아있는 동안 임시 객체도 유지됨
```

다만 이 수명 연장은 **이름 있는 참조에 직접 바인딩될 때만** 적용됩니다. 함수에 전달되거나 다른 객체로 다시 묶이면 적용되지 않으니 주의하세요.

```cpp
const std::string& danger = std::string("oops").substr(0, 2);
// substr(...)이 만든 또 다른 임시 객체에 묶임
// 원본 std::string("oops") 임시는 이미 소멸 → 댕글링!
```

**왜 템플릿 타입 추론에서 중요한가?**

임시 객체는 rvalue이기 때문에, 보편 참조 `T&&`에 전달되면 `T`가 참조가 아닌 일반 타입으로 추론됩니다. 같은 함수 호출이라도 lvalue를 넘기느냐, 임시 객체(rvalue)를 넘기느냐에 따라 추론 결과가 달라집니다.
...
이 차이가 다음 항목들에서 다룰 `std::move`, `std::forward`, perfect forwarding의 토대가 됩니다.
...
`T&` 파라미터는 **rvalue를 받지 못합니다.** 반면 `const T&` 파라미터는 lvalue, rvalue를 모두 받습니다. 이 비대칭 때문에 라이브러리 함수는 대부분 `const T&`로 받도록 설계됩니다.
...
int x = 27;
f(x);                        // OK   — lvalue
f(27);                       // 에러 — rvalue를 non-const 참조에 못 묶음
...
g(x);                        // OK: lvalue          → T = int
g(27);                       // OK: rvalue도 받음    → T = int
g(x + 1);                    // OK: prvalue도 받음   → T = int
```

`const T&`가 만능처럼 동작하는 이유는 C++이 const lvalue 참조에 한해서 **임시 객체 바인딩과 수명 연장**을 허용하기 때문입니다(앞 절의 임시 객체 규칙 참고).
...
f2(&x);   // T = int        → param: const int*       (const는 매개변수에 이미 있음)
f2(&cx);  // T = int        → param: const int*

f3(&x);   // T = int        → param: int* const
          // (param이 const라는 사실은 호출자에겐 보이지 않음 — 함수 내부 약속)
```
...
핵심은 컴파일러가 "내가 만든 ParamType 자리에 expr이 들어가려면 T는 무엇이어야 하는가"를 **패턴 매칭**으로 풀어낸다는 점입니다. 포인터가 몇 겹이든, 멤버 포인터든 같은 발상입니다.
...
`&&`만 봤다고 다 보편 참조가 아닙니다. 모양이 살짝만 달라도 그냥 rvalue 참조가 됩니다 — 이걸 헷갈리면 의도와 다른 함수가 호출됩니다.
...
void f3(std::vector<T>&& v);   // ❌ rvalue 참조
                               //    T가 직접이 아니라 다른 타입에 감싸져 있음

template<typename T>
class Box {
    void f4(T&& param);        // ❌ rvalue 참조!
                               //    여기서 T는 Box를 인스턴스화할 때 이미 결정됨
...
1. **타입 추론**이 일어나는 자리여야 한다 (= 호출 시점에 `T`가 결정되어야 함)
2. **정확히 `T&&`** 형태여야 한다 (const, volatile, 다른 타입으로 감싸짐 모두 X)

`auto&&`도 같은 두 조건을 만족하므로 보편 참조입니다 (항목 2에서 다룸).
...
**왜 값 전달에선 배열이 포인터로 decay되나?**

C 시절부터 이어진 호환성 때문입니다. C/C++에서 함수 매개변수의 배열 문법은 **컴파일러가 내부적으로 포인터로 바꿔치기**합니다:

```cpp
void f(int param[]);    // 컴파일러가 보기엔
void f(int* param);     // 이 둘이 완전히 동일
void f(int param[10]);  // 크기는 무시되어 — 역시 int* 와 동일
```

따라서 값 전달 템플릿 매개변수도 같은 규칙을 따라 배열을 포인터로 decay시킵니다. 배열을 "값으로 복사"한다는 개념 자체가 함수 호출 규약에 존재하지 않기 때문입니다.

**왜 참조로 받으면 decay되지 않나?**

참조는 객체를 복사하지 않고 **그대로 가리킵니다.** 배열에 대한 참조라는 개념(`T (&)[N]`)이 C++에 존재하므로, 컴파일러는 배열 크기까지 포함한 진짜 타입을 그대로 추론할 수 있습니다.
...
함수도 함수 포인터로 decay됩니다 (배열과 같은 이유로):
...
값 전달이면 함수 포인터로, 참조면 함수 참조로 — 배열과 정확히 같은 패턴입니다.
...
일부만 명시하는 것도 가능합니다. 이때 **명시한 부분 뒤로** 추론이 시작됩니다.
...
같은 템플릿 매개변수가 서로 다른 타입을 받으면 컴파일러는 어느 쪽을 골라야 할지 결정하지 못합니다.
...
### 3. `{1, 2, 3}` 중괄호 초기화 리스트는 추론 못 한다

이게 `auto`와 템플릿 추론의 가장 큰 차이입니다 (자세한 건 항목 2):
...
함수 호출의 **다른 부분**에서 일어나는 암묵적 변환과 달리, 추론 과정에선 타입 변환이 거의 적용되지 않습니다.

```cpp
template<typename T>
void f(T x, T y);

f(1, 2L);         // 에러! int와 long을 묶어줄 변환 안 함
                  // (명시 또는 캐스팅 필요)
...
`auto` 타입 추론은 **거의 정확히** 위 세 가지 경우와 같은 규칙을 사용합니다 — 한 가지 예외만 빼고요.
...
이 마지막 한 줄의 차이가 항목 2의 핵심 주제입니다.

## 추론 결과를 확인하고 싶다면

타입 추론은 머릿속으로 따라가다 보면 헷갈리기 쉽습니다. 컴파일러가 실제로 무슨 타입으로 추론했는지 확인하는 자세한 방법은 **항목 4**에서 다룹니다. 간단한 트릭만 미리 보면:
...
런타임에 확인하려면 `typeid(T).name()` (cv/참조 정보가 깎이는 한계 있음) 또는 `boost::typeindex::type_id_with_cvr<T>()`를 사용합니다.

## 핵심 정리

1. **참조 파라미터**: expr의 참조성은 무시되지만, const는 타입 추론에 포함됨
2. **`T&`는 rvalue를 못 받음 / `const T&`는 lvalue·rvalue 모두 받음**
3. **보편 참조 파라미터**: 정확히 `T&&` 형태일 때만 보편 참조이며, lvalue는 `T`가 참조 타입으로 추론됨
4. **값 전달 파라미터**: const와 volatile이 무시됨 (포인터가 가리키는 대상의 const는 유지)
5. **배열과 함수**: 값 전달에선 포인터로 decay, 참조 전달에선 원래 타입 유지
