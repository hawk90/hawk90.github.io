...
자세한 건 [항목 37](/blog/programming/effective-modern-cpp/item37-make-std-threads-unjoinable-on-all-paths). C++20 `std::jthread`가 자동 join.
...
기본은 `async | deferred` — 실용적 함정 있음 ([항목 36](/blog/programming/effective-modern-cpp/item36-specify-launch-async-if-asynchronicity-is-essential)).
...
4. **`async`의 launch policy** 함정에 주의 ([항목 36](/blog/programming/effective-modern-cpp/item36-specify-launch-async-if-asynchronicity-is-essential))
...
- [항목 37: jthread / unjoinable](/blog/programming/effective-modern-cpp/item37-make-std-threads-unjoinable-on-all-paths)
