...
`x = x`는 직접 적는 일이 드물지만, **별칭(alias)을 통해 흔히 발생**합니다 — 같은 객체를 두 포인터/참조로 가리키거나, 같은 컨테이너 안의 두 원소가 우연히 같은 객체일 수 있습니다. 자원 관리 클래스의 `operator=`가 자기 대입을 처리하지 않으면 **이중 해제** 또는 **자원 손실**.
...
- [항목 10: 대입 연산자는 *this 참조 반환](/blog/programming/effective-cpp/item10-have-assignment-operators-return-reference-to-this) — operator= 시그니처
- [항목 12: 객체의 모든 부분을 복사](/blog/programming/effective-cpp/item12-copy-all-parts-of-an-object) — operator=의 책임
- [항목 25: non-throwing swap 지원](/blog/programming/effective-cpp/item25-consider-support-for-a-non-throwing-swap) — copy-and-swap의 핵심 도구
- [항목 29: 예외 안전 코드](/blog/programming/effective-cpp/item29-strive-for-exception-safe-code) — 강력한 예외 보증
