...
데이터 멤버를 `private`으로 두는 것은 OOP의 **캡슐화** 원칙의 출발점입니다. 접근을 멤버 함수로 한정하면 — 검증·로깅·동기화·내부 표현 변경이 클라이언트를 깨뜨리지 않고 가능. 이 항목은 public 데이터의 4가지 단점, **protected가 public과 같다**는 핵심, 그리고 `struct` vs `class` 컨벤션을 다룹니다.
...
- [항목 18: 인터페이스는 쓰기 쉽게](/blog/programming/effective-cpp/item18-make-interfaces-easy-to-use-correctly-and-hard-to-use-incorrectly) — public 인터페이스 설계
- [항목 23: 비-멤버 비-friend 선호](/blog/programming/effective-cpp/item23-prefer-non-member-non-friend-functions-to-member-functions) — 캡슐화 한 걸음 더
- [항목 28: 객체 내부 핸들 반환 금지](/blog/programming/effective-cpp/item28-avoid-returning-handles-to-object-internals) — 데이터 노출의 다른 형태
