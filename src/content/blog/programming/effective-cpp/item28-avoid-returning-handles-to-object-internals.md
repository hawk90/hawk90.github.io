...
객체 내부 데이터에 대한 **참조·포인터·반복자**(handles)를 반환하면 두 가지 위험이 따라옵니다 — 사용자가 그 핸들로 캡슐화를 우회 변경할 수 있고, 객체가 소멸한 후에도 핸들을 들고 있을 수 있습니다(댕글링). const로 막아도 댕글링 위험은 그대로 — 가능하면 **값으로 반환**, 어쩔 수 없으면 const + 라이프타임 문서화.
...
- [항목 3: const 사용](/blog/programming/effective-cpp/item03-use-const-whenever-possible) — bitwise vs logical const
- [항목 21: 객체 반환은 값으로](/blog/programming/effective-cpp/item21-dont-try-to-return-a-reference-when-you-must-return-an-object) — 참조 반환의 위험
- [항목 22: 데이터 멤버는 private](/blog/programming/effective-cpp/item22-declare-data-members-private) — 핸들 노출 차단
