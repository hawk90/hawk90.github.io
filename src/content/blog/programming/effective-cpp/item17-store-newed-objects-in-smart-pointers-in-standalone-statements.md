...
`new`로 만든 raw 포인터를 같은 함수 호출의 다른 인자와 한 문장에 섞어 쓰면 — **C++14 이전**엔 평가 순서가 미지정이어서 자원 누수가 일어날 수 있었습니다. C++17 이후 일부 완화됐지만, **`make_unique`/`make_shared` 사용**이 모든 시대에 안전하고 우아한 답입니다.
...
- [항목 13: RAII](/blog/programming/effective-cpp/item13-use-objects-to-manage-resources) — 스마트 포인터의 기본 동기
- [항목 18: 인터페이스는 쓰기 쉽게](/blog/programming/effective-cpp/item18-make-interfaces-easy-to-use-correctly-and-hard-to-use-incorrectly) — make_* 같은 팩토리의 가치
- [Effective Modern C++ 항목 21: make_unique/make_shared 선호](/blog/programming/effective-modern-cpp/item21-prefer-make-unique-and-make-shared-to-direct-new) — make_* 심층 분석
