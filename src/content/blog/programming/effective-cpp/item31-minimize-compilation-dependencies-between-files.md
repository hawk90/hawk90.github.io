...
C++ 빌드 시간이 길어지는 주 원인은 **헤더 파일의 의존성 그래프**. 한 헤더를 수정하면 그 헤더를 직접 또는 간접적으로 include하는 모든 .cpp가 재컴파일. 큰 프로젝트에선 이 영향이 수십 분~수 시간. 이 항목은 **Pimpl 관용구**와 **인터페이스 클래스** 두 가지 핵심 패턴으로 의존성을 끊는 법을 다룹니다.
...
- [항목 13: RAII](/blog/programming/effective-cpp/item13-use-objects-to-manage-resources) — Pimpl의 unique_ptr 자동 해제
- [항목 25: non-throwing swap](/blog/programming/effective-cpp/item25-consider-support-for-a-non-throwing-swap) — Pimpl의 효율적 swap
- [Effective Modern C++ 항목 22: Pimpl idiom](/blog/programming/effective-modern-cpp/item22-when-using-pimpl-define-special-members-in-impl-file) — 함정 정리
