...
표준 `operator new`와 `operator delete`는 **범용적**으로 잘 만들어져 있습니다 — 평균적으로 빠르고, 정확하며, thread-safe. 그러나 특정 도메인에선 사용자 정의 할당기가 더 우수할 수 있습니다 — 메모리 패턴 추적, 풀 할당, 캐시 정렬, 클러스터링 등. 이 항목은 사용자 정의 할당기가 **정당한 시나리오 6가지**와 **외부 대체 라이브러리** 옵션을 다룹니다.
