...
직관과 반대로 — **멤버 함수가 항상 캡슐화에 좋은 것은 아닙니다**. 캡슐화의 척도는 "클래스 내부 데이터에 직접 접근하는 코드의 양". 멤버 함수와 friend는 private에 접근 가능, 비-멤버 비-friend는 public 인터페이스만 사용. 후자가 많을수록 **클래스 내부 변경의 자유**가 커집니다.

이 항목은 그 역설을 풀고, **네임스페이스로 비-멤버 함수를 묶어 모듈화**하는 패턴을 소개합니다.
...
- [항목 22: 데이터 멤버는 private](/blog/programming/effective-cpp/item22-declare-data-members-private) — 캡슐화의 시작
- [항목 24: 양쪽 변환 필요 시 비-멤버](/blog/programming/effective-cpp/item24-declare-non-member-functions-when-type-conversions-should-apply) — 비-멤버가 강제되는 경우
- [항목 25: non-throwing swap](/blog/programming/effective-cpp/item25-consider-support-for-a-non-throwing-swap) — 비-멤버 + ADL의 대표 사례
