...
이항 연산자(`+`, `-`, `*` 등)에서 **양쪽 피연산자 모두 암묵 변환이 적용되어야 한다면**, 멤버 함수로는 불가능합니다 — 멤버의 좌변(`*this`)은 변환 대상이 아니기 때문. 비-멤버 함수로 만들어야 양쪽이 모두 동등하게 변환 후보가 됩니다. 이 항목은 `Rational`처럼 정수와 자연스럽게 섞이는 타입에서 흔히 등장하는 함정입니다.
...
- [항목 23: 비-멤버 비-friend 선호](/blog/programming/effective-cpp/item23-prefer-non-member-non-friend-functions-to-member-functions) — 캡슐화 측면
- [항목 18: 인터페이스는 쓰기 쉽게](/blog/programming/effective-cpp/item18-make-interfaces-easy-to-use-correctly-and-hard-to-use-incorrectly) — explicit 변환과의 균형
- [항목 25: non-throwing swap](/blog/programming/effective-cpp/item25-consider-support-for-a-non-throwing-swap) — 비-멤버 + ADL의 또 다른 사례
