...
`new`와 `new[]`는 서로 다른 함수이며, 만들어진 메모리의 **레이아웃과 소멸 방식**도 다릅니다. 짝이 안 맞는 `delete`/`delete[]`는 즉시 UB — 한쪽은 객체 하나의 소멸자만 부르고, 다른 쪽은 배열 헤더에서 개수를 읽어 N번의 소멸자 호출. 잘못 짝지으면 **메모리 해석 자체가 어긋남**.

C++11+ 에선 **raw `new`/`delete`를 피하는 것**이 가장 안전 — `std::vector`, `std::array`, `std::unique_ptr` 등이 정확한 짝을 자동으로 관리.
...
- [항목 13: RAII](/blog/programming/effective-cpp/item13-use-objects-to-manage-resources) — raw new/delete 회피의 기본
- [항목 17: new로 만든 객체는 스마트 포인터에](/blog/programming/effective-cpp/item17-store-newed-objects-in-smart-pointers-in-standalone-statements) — 짝 맞추기를 라이브러리에 위임
- [항목 51: new와 delete 작성 시 규약](/blog/programming/effective-cpp/item51-adhere-to-convention-when-writing-new-and-delete) — 사용자 정의 할당
