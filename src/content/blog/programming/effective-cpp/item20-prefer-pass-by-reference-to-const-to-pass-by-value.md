...
함수 매개변수의 기본 전달 방식은 **값 전달**(by-value) — 인자가 매개변수에 복사됩니다. 큰 클래스 객체에선 이 복사가 비싸고, 다형성 객체에선 **슬라이싱**(derived 부분 손실)이 일어납니다. 대부분의 경우 `const T&` 참조 전달이 안전하고 효율적 — 단, **작은 내장 타입과 STL 반복자/함수 객체**는 예외. C++11 이동 의미론으로 일부 영역에선 by-value도 재평가됐습니다.
...
- [항목 21: 객체 반환 시 참조 X](/blog/programming/effective-cpp/item21-dont-try-to-return-a-reference-when-you-must-return-an-object) — 참조의 라이프타임
- [항목 25: non-throwing swap](/blog/programming/effective-cpp/item25-consider-support-for-a-non-throwing-swap) — 큰 객체 효율 교환
- [Effective Modern C++ 항목 41: by-value vs move](/blog/programming/effective-modern-cpp/item41-consider-pass-by-value-for-copyable-parameters-that-are-cheap-to-move-and-always-copied) — 모던 트레이드오프
