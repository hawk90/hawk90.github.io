...
`x = y = z = 15` 같은 **체인 대입**을 자연스럽게 지원하려면 `=`이 lvalue를 반환해야 합니다. C++ 표준 관용구는 `*this`를 참조로 반환 — 강제는 아니지만 모든 표준 라이브러리와 사용자 코드가 이 관용구를 가정합니다.
...
- [항목 11: operator=에서 자기 대입 처리](/blog/programming/effective-cpp/item11-handle-assignment-to-self-in-operator-equals) — 대입의 또 다른 정석
- [항목 12: 객체의 모든 부분을 복사](/blog/programming/effective-cpp/item12-copy-all-parts-of-an-object) — 대입 본문의 책임
- [항목 23: 비-멤버·비-friend 함수 선호](/blog/programming/effective-cpp/item23-prefer-non-member-non-friend-functions-to-member-functions) — operator=가 예외인 이유
