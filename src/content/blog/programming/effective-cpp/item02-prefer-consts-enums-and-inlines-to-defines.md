...
`#define`은 컴파일러가 소스를 보기 **전에 전처리기가 처리**합니다. 컴파일러 입장에선 매크로의 이름은 존재한 적이 없습니다 — 디버그 심볼에 안 잡히고, 타입 체크가 없으며, 스코프도 없습니다. C 시절의 관성으로 남아 있는 이 도구를, C++ 영역에서는 `const`·`constexpr`·`enum`·`inline 함수 템플릿`으로 거의 모두 대체할 수 있습니다.
...
`#define ASPECT_RATIO 1.653`은 전처리기에서 **단순 텍스트 치환**으로 끝납니다. 컴파일러가 보는 것은 `1.653`이라는 리터럴뿐 — "`ASPECT_RATIO`"라는 이름은 디버거에도, 에러 메시지에도, 심볼 테이블에도 없습니다.
...
수십 개의 매크로 상수가 있는 코드베이스에서 "이 1.653은 어떤 상수인가"를 찾는 데 들이는 시간은 결코 작지 않습니다.
...
`const double pi = 3.14;`로 두면 명확히 `double`. 오버로드 해상도가 결정적이 됩니다.
...
매크로는 정의된 시점부터 `#undef` 또는 파일 끝까지 살아남습니다. 클래스/네임스페이스 안에 둘 수 없습니다.
...
설계자가 모든 호출자의 사용 패턴을 통제할 수는 없습니다.
...
`constexpr`은 "**컴파일 타임에 알려진 값**"임을 명시합니다. 배열 크기·템플릿 인자 등 상수식이 필요한 자리에 사용 가능.
...
C++17 이전엔 헤더의 `const`는 자동으로 **내부 연결(internal linkage)** 이 되므로 여러 번 include 되어도 충돌 없음. **단**, C++17 이전에 `inline`이 없는 `constexpr` 변수를 헤더에 두면 ODR 위반 가능. C++17부터 `inline constexpr`이 정답.
...
**제약**: in-class 초기화는 **정수형 + `const`** 또는 `constexpr` 한정. `double`·`std::string` 같은 비-정수 상수는 .cpp에 정의.
...
C++17부터 모든 타입의 in-class 정의가 가능 — ODR 걱정 없음.
...
`enum` 멤버는 **rvalue**(주소가 없음) — `&NumTurns`처럼 주소를 얻을 수 없습니다. 이게 단점이자 장점:
...
지금은 `static constexpr int`가 가장 깔끔 — 하지만 **TMP 코드와 일부 헤더-온리 라이브러리**에서는 여전히 enum hack을 만나게 됩니다.
...
이점:
- **타입 안전** — `int`와 `string`을 섞어 호출하면 컴파일 에러
- **스코프** — 네임스페이스·클래스에 둘 수 있음
- **인라인** — 컴파일러가 적절히 인라인, 디버그 빌드에선 진짜 함수
- **인자 평가 횟수 고정** — 한 번씩
...
순수한 "상수"나 "함수 매크로"가 아닌, **컴파일 단계 자체를 조작**하는 용도. 이 외에는 거의 없습니다.

## 모던 변형

### `consteval` — C++20

"반드시 컴파일 타임에만" 평가되도록 강제.
...
기존 `constexpr` 함수는 컴파일/런타임 모두 가능했지만, `consteval`은 컴파일 타임 한정.

### `if constexpr` — C++17

매크로의 `#ifdef`스러운 분기를 함수 안에서.
...
이전엔 SFINAE 트릭으로 풀던 분기를 깔끔하게 표현.
...
4. 매크로는 디버그·타입·스코프·평가 횟수 모두 잃는다 — 사용 영역 최소화
5. **유일하게 매크로가 필요한 곳**: 헤더 가드, 조건부 컴파일, 소스 위치 매크로

## 관련 항목

- [항목 1: C++를 언어들의 연합체로 보라](/blog/programming/effective-cpp/item01-view-cpp-as-a-federation-of-languages) — C 영역에서 OOP 영역으로 옮겨갈 때 매크로를 버리는 한 단계
- [항목 3: const를 가능한 곳마다](/blog/programming/effective-cpp/item03-use-const-whenever-possible) — `const`가 컴파일러에 약속을 박는 방식
- [항목 30: inline의 이해](/blog/programming/effective-cpp/item30-understand-the-ins-and-outs-of-inlining) — `inline` 함수의 비용과 한계
