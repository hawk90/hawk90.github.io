...
복제와 동등성 비교에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **얕은 복사/깊은 복사 혼동** | twin 후 내부 객체 공유 → 의도치 않은 수정 | 필요에 따라 deep_twin 사용. 복사 깊이 명확히 |
| **참조 동등성(=)과 값 동등성(~) 혼동** | 같은 값인데 다르다고 판단 → 로직 오류 | = 는 참조, ~ 는 값. 의도에 맞게 선택 |
| **is_equal 재정의 누락** | 기본 비트 비교로 의미 없는 결과 | 도메인 의미에 맞게 is_equal 재정의 |
| **hash_code 불일치** | is_equal 재정의 후 hash_code 미재정의 → 해시테이블 오작동 | is_equal 재정의 시 hash_code도 함께. 일관성 보장 |
| **Void 검사 누락** | detachable 변수 직접 사용 → 런타임 에러 | attached 구문으로 Void 검사. Void-safe 프로그래밍 |
| **내부 참조 노출** | 컬렉션 직접 반환 → 외부에서 수정 가능 | 방어적 복사 또는 불변 뷰. 캡슐화 유지 |
| **expanded 타입 오용** | 큰 객체에 expanded 사용 → 복사 비용 폭발 | 작은 값 객체에만 expanded. 참조가 기본 |
