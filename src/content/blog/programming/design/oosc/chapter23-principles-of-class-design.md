...
클래스 설계에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **낮은 응집도** | 관련 없는 피처가 한 클래스에 → 여러 이유로 변경 | 단일 책임. 책임별로 분리 |
| **추상화 수준 불일치** | `balance`(비즈니스)와 `disk_sector`(물리)가 공존 | 같은 추상화 수준의 피처만. 레이어 분리 |
| **일관성 없는 명명** | `add`, `put`, `insert`가 혼용 → 학습 비용 증가 | 명명 규칙 통일. 동사·접두사 일관되게 |
| **과도한 인터페이스** | 사용 안 하는 피처까지 공개 → 오용·복잡성 | 최소 인터페이스. export로 가시성 제한 |
| **불완전한 인터페이스** | `add`는 있는데 `remove` 없음 → 클라이언트 불편 | 완전성 검증. 클라이언트 시나리오로 확인 |
| **Feature Envy** | 다른 클래스 데이터를 과도하게 사용 → 결합도 증가 | 데이터를 가진 클래스로 로직 이동 |
| **Data Class** | getter/setter만 있고 행위 없음 → 절차적 설계 | 관련 행위를 클래스로. OO답게 |
