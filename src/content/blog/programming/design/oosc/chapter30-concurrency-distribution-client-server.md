...
slug: "programming/design/oosc/chapter30-concurrency-distribution-client-server"
...
동시성과 분산 프로그래밍에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **공유 가변 상태** | 여러 스레드가 같은 데이터 수정 → 경쟁 조건, 재현 어려운 버그 | 객체 단위 캡슐화. separate 키워드, 불변 객체 활용 |
| **수동 잠금 관리** | lock/unlock 명시적 호출 → 잠금 누락, 교착 상태 | SCOOP 활용. 암묵적 잠금, 런타임이 순서 관리 |
| **교착 상태 (Deadlock)** | 서로 상대방의 잠금 대기 → 시스템 멈춤 | 잠금 순서 일관성. SCOOP이 자동 관리, 여러 separate 인자 |
| **wait/notify 오용** | 신호 누락, spurious wakeup → 무한 대기, 조건 미충족 실행 | 계약 기반 대기. 전조건이 대기 조건 역할 |
| **stale read** | 분리된 객체 쿼리 후 상태 변경 → 잘못된 가정으로 진행 | 쿼리 결과 즉시 사용. 조건은 전조건으로, 배치 처리 |
| **과도한 동기화** | 모든 곳에 잠금 → 성능 저하, 병렬성 상실 | 공유 최소화. 로컬 객체 선호, 불변 객체 공유 |
| **분산 실패 무시** | 네트워크를 신뢰 → 부분 실패, 타임아웃 미처리 | 실패 가정 설계. 예외 처리, 보상 동작, 재시도 전략 |
