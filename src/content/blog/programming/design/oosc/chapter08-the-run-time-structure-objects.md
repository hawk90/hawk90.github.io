...
객체 다루기에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **참조와 값 혼동** | `p2 := p1` 후 p2 수정 → p1도 바뀜 "왜?" | 참조 의미론 이해. 공유 원치 않으면 `twin` 사용 |
| **Void 참조 방치** | `p.set(...)` 호출 시 런타임 크래시 | `attached` 타입 또는 `if attached ... as` 패턴으로 Void 안전성 확보 |
| **동등성 연산자 오용** | `p1 = p3`이 False라서 "값이 다르네" → 착각 | `=`는 참조, `~`는 값 비교. 의도에 맞는 연산자 선택 |
| **얕은 복사 후 공유 객체 수정** | `person2 := person1.twin` 후 address 변경 → 둘 다 영향 | 중첩 객체까지 독립 원하면 `deep_twin` |
| **Current 남용** | 모든 곳에 `Current.x` 명시 → 코드 장황 | Current는 보통 생략. 모호할 때만 명시 |
| **확장 타입과 참조 타입 혼동** | INTEGER에 참조 의미론 기대 | 기본 타입(INTEGER, BOOLEAN 등)은 값 복사. 확장 타입 개념 이해 |
| **객체 생성 없이 사용** | 선언만 하고 `create` 안 함 → Void | 선언 ≠ 생성. `create` 호출로 객체 실체화 필수 |
