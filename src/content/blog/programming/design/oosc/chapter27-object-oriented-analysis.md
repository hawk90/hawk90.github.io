...
객체지향 분석에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **분석과 설계 과도한 구분** | 분석 모델 따로, 설계 모델 따로 → 전환 비용 | Seamless 접근. 같은 언어로 점진적 정제 |
| **구현 세부사항 조기 도입** | "성능 때문에 해시테이블" → 분석 오염 | 분석은 WHAT. 데이터 구조는 설계에서 |
| **비즈니스 규칙 누락** | 계약 없이 메서드만 → 규칙 암묵적 | 전조건/후조건/불변식으로 규칙 명시 |
| **도메인 전문가 배제** | 개발자만 분석 → 오해, 누락 | Short Form으로 검토. 공통 어휘 확립 |
| **명사 추출만 의존** | 문서 언어에 좌우 → 개념적 분석 부족 | 행위 분석(유스케이스) 병행. ADT 관점 |
| **관계 모델링 소홀** | 클래스만, 관계 없음 → 불완전 모델 | 연관/상속/집합 관계 명시. 다중성 표기 |
| **검증 없이 진행** | 완전성/일관성 미검사 → 후반 재작업 | 체크리스트 검증. 도메인 전문가 리뷰 |
