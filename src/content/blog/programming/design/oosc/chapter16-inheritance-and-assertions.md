...
상속과 계약에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **사전조건 강화** | 자식이 `require` 조건 추가 → 부모 계약으로 호출 시 거부 | `require else`로 완화만 가능. 강화하면 LSP 위반 |
| **사후조건 약화** | 자식이 보장을 줄임 → 클라이언트 기대 위반 | `ensure then`으로 강화만 가능. 약화는 LSP 위반 |
| **Rectangle-Square 함정** | SQUARE가 RECTANGLE 상속 → set_width 시 height도 변경 | 수학적 is-a와 OO is-a는 다름. 설계 재검토 |
| **불변식 무시** | 자식이 부모 불변식 위반 → 부모 타입 계약 붕괴 | 불변식은 누적. 부모 불변식 반드시 유지 |
| **부모 계약 모름** | 재정의 시 부모 계약 확인 안 함 → 암묵적 위반 | Short Form으로 부모 계약 확인 후 재정의 |
| **Liskov 원칙 오해** | "상속하면 자동으로 대체 가능" 착각 | 행위 호환성 확인. 계약이 핵심 |
| **추상 계약 누락** | deferred 피처에 require/ensure 없음 → 구현자 혼란 | 추상 피처도 계약 명시. 구현 가이드라인 제공 |
