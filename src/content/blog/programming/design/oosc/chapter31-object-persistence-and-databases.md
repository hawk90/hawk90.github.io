...
객체 영속성과 데이터베이스에서 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **영속성 코드가 도메인 침투** | 도메인 객체에 `save()`, `database_id` 존재 → 테스트 어려움, 저장소 종속 | 리포지토리 패턴. 도메인 객체 순수하게, 영속성 계층 분리 |
| **N+1 쿼리 문제** | 컬렉션 접근 시 쿼리 폭발 → 성능 급락 | 지연 로딩 + 배치 페칭. 필요한 데이터 한 번에 로드 |
| **순환 참조 직렬화 실패** | A → B → A 구조에서 무한 루프 또는 예외 → 직렬화 실패 | 객체 ID 기반 직렬화. 참조를 ID로 대체 |
| **낙관적 잠금 충돌 무시** | 버전 불일치 시 덮어쓰기 → 데이터 손실, 동시 수정 충돌 | 충돌 검출 + 재시도. 버전 체크, 사용자 알림 |
| **트랜잭션 범위 부적절** | 너무 길거나 짧은 트랜잭션 → 잠금 경합 또는 일관성 깨짐 | 비즈니스 단위로 트랜잭션. 작업 완료 후 즉시 커밋 |
| **상속 매핑 전략 무고려** | 기본 전략 사용 → NULL 컬럼 폭발 또는 조인 폭발 | 요구사항에 맞는 전략. 단일 테이블 vs 클래스별 테이블 선택 |
| **영속성 컨텍스트 누수** | 세션 범위 밖에서 지연 로딩 → LazyInitializationException | 명확한 세션 범위. 필요한 데이터 미리 페치 |
