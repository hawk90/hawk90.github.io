...
OO 언어 사용 시 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **언어 철학 무시** | Java에서 C++ 스타일, Smalltalk에서 정적 타이핑 기대 → 언어와 싸움 | 언어 관용구 학습. 각 언어의 설계 철학 이해 |
| **다형성 오남용** | 모든 메서드를 virtual/override → 불필요한 오버헤드, 의도 불명확 | 필요할 때만 다형성. 확장점 명시적 설계 |
| **Null 안전성 무시** | nullable 언어에서 검사 없이 접근 → NullPointerException 다발 | Kotlin `?`, Swift Optional 활용. 또는 명시적 검사 |
| **상속 vs 인터페이스 혼동** | Java에서 다중 상속 시도, 인터페이스로 구현 공유 기대 → 컴파일 에러 또는 코드 중복 | 언어 규칙 이해. 단일 상속 + 다중 인터페이스 (Java), 다중 상속 (C++, Eiffel) |
| **GC 의존성 과신** | 종료화 로직을 finalize에 → 호출 보장 없음, 자원 누수 | 명시적 close/dispose. try-with-resources, using 활용 |
| **제네릭 타입 소거 무시** | Java에서 런타임 타입 정보 기대 → 캐스팅 오류, 타입 정보 손실 | 타입 토큰 패턴. 또는 reified generics 언어 (Kotlin) |
| **함수형 요소 무시** | 현대 OO 언어에서 람다/스트림 미활용 → 장황한 코드, 가독성 저하 | 함수형 API 학습. map, filter, reduce 활용 |
