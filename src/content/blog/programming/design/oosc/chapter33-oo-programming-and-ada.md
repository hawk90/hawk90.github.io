...
Ada에서 OO 프로그래밍을 할 때 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **패키지를 클래스로 오해** | 패키지 하나에 모든 것 → 응집도 낮음, 타입과 연산 혼재 | 패키지 = 모듈. 태그 타입이 클래스 역할, 패키지는 그룹화 |
| **'Class 누락으로 정적 바인딩** | `Figure` 파라미터에 동적 바인딩 기대 → 항상 부모 메서드 호출 | `Figure'Class` 사용. 다형성이 필요하면 클래스와이드 타입 |
| **추상 연산 구현 누락** | `is abstract` 연산 재정의 안 함 → 컴파일 에러, 인스턴스화 불가 | 모든 추상 연산 구현. 구체 타입에서 반드시 재정의 |
| **Finalize 순서 무시** | Controlled 타입 종료화 순서 가정 → 자원 해제 오류 | 명시적 순서 관리. 부모 Finalize 호출 순서 주의 |
| **제네릭 인스턴스화 누락** | 제네릭 패키지 직접 사용 시도 → 컴파일 에러 | `package ... is new ...` 필수. 타입 파라미터 명시 |
| **다중 상속 시도** | 여러 태그 타입 상속 시도 → 컴파일 에러 | 인터페이스 활용. 구현 상속은 단일, 인터페이스만 다중 |
| **private 접근 혼동** | 자식 패키지에서 private 접근 기대 → 컴파일 에러 | private 파트는 본체에만. 자식 패키지는 명세 부분만 접근 |
