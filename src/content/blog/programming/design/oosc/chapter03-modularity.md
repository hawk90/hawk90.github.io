...
| 실수 | 증상 | 해결 |
|------|------|------|
| **전역 변수로 통신** | 모듈 A가 전역 상태 설정, 모듈 B가 읽음 → 암묵적 의존 | 명시적 인터페이스 원칙. 매개변수나 의존성 주입 |
| **God Module** | 한 모듈이 2000줄, 모든 책임 담당 | 단일 책임으로 분해. 한 모듈 = 한 이유로 변경 |
| **순환 의존** | A → B → C → A | 의존 방향 정리. 추상화 도입으로 역전 |
| **넓은 인터페이스** | 함수 매개변수 15개 | 관련 데이터를 객체로 묶기. `processOrder(order)` |
| **정보 은닉 위반** | `public` 필드 직접 접근 → 구현 변경 시 클라이언트 깨짐 | 비공개 기본, 접근자 메서드 제공 |
| **분해만, 조합 무시** | 특정 문제에 맞춰 분해 → 다른 문제에 재사용 불가 | 조합 기준 고려. 맥락 독립적 모듈 설계 |
| **Open-Closed 위반** | 새 기능 추가 시 기존 모듈 코드 수정 | 상속/다형성으로 확장. 기존 코드 수정 없이 |
