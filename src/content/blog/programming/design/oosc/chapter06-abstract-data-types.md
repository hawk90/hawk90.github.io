...
ADT 적용 시 흔히 빠지는 함정이다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **ADT 없이 구현 시작** | "일단 배열로 스택 만들자" → 명세 없이 구현 | 연산과 공리 먼저 정의. 그다음 구현 |
| **공리 정의 누락** | "push, pop 있으면 되지" → 동작이 모호 | `top(push(s,x))=x` 같은 공리로 의미 명확화 |
| **사전조건 누락** | `pop()` 빈 스택에 호출 → 예외? 무시? undefined? | 사전조건 명시. `require not is_empty` |
| **명령-질의 혼합** | `pop()`이 값 반환 + 상태 변경 | 분리: `top`(질의) + `remove`(명령) |
| **구현 노출** | `stack.array[stack.count]` 직접 접근 | ADT 연산만 공개. 내부 구현 숨김 |
| **불충분한 공리** | 모든 질의 결과가 유도 불가 | 충분성 검증. 임의 상태에서 질의 결과 유도 가능해야 |
