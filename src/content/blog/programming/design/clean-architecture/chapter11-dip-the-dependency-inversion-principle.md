...
이제 **의존성 화살표가 뒤집혔다**. OrderProcessor가 OrderRepository만 알면 되고, MySqlOrderRepository는 OrderRepository에 의존한다. MySQL이 변해도 OrderProcessor는 그대로다.

## 누가 인터페이스를 소유하는가

이 부분이 핵심이다. **인터페이스는 정책 측(고수준)이 소유한다**.

![인터페이스 소유권 — 정책 컴포넌트가 인터페이스를 소유](/images/blog/clean-architecture/diagrams/ch11-interface-ownership.svg)
...
- [GoF Abstract Factory](/blog/programming/design/gof-design-patterns/item01-abstract-factory/) — DIP의 객체 생성 확장
