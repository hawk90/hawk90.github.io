...
| 징후 | 설명 | 해결 |
|------|------|------|
| 신 클래스 (God Class) | 모든 것을 아는 클래스. 수백, 수천 줄 | 책임별로 클래스 분리 |
| 데이터 클래스 (Data Class) | 게터/세터만 있음. 행동 없음 | 행동을 데이터 클래스 안으로 이동 |
| 기능 질투 (Feature Envy) | 다른 클래스의 데이터에 집착 | 메서드를 데이터 소유 클래스로 이동 |
| 긴 파라미터 목록 | 너무 많은 파라미터 | 파라미터 객체 도입 |

## 자주 하는 실수

클래스와 객체를 설계할 때 흔히 빠지는 함정입니다.

| 실수 | 증상 | 해결 |
|------|------|------|
| **상태와 정체성 혼동** | `equals()`를 ID 없이 상태만으로 구현 | 엔티티는 ID, 값 객체는 상태로 비교 |
| **불변식 미정의** | "올바른 상태"가 무엇인지 모름 | 생성자에서 검증, 불변식 주석 |
| **모든 필드에 setter** | `setBalance(-1000)` 허용 | 행동 메서드(`withdraw`)로 상태 변경 |
| **관계 방향 혼란** | 양방향 참조를 무분별하게 | "누가 누구를 *알아야* 하는가?" 질문 |
| **합성 vs 집합 혼동** | 생명주기 관리 없이 ◆ 사용 | "부분이 전체 없이 존재할 수 있는가?" |
| **정체성 없는 엔티티** | UUID 없이 "같은 이름이면 같은 객체" | 명시적 ID 필드 도입 |
