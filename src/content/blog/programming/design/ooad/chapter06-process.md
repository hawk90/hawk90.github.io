...
| 목적 | 설명 |
|------|------|
| 예측 가능성 | 무엇을 언제 할지 알 수 있음, 진행 상황 측정 가능, 위험 관리 |
| 품질 보장 | 일관된 산출물, 검증 포인트, 결함 조기 발견 |
| 의사소통 | 팀 내 공통 언어, 역할과 책임 명확, 이해관계자 기대 관리 |
| 지식 축적 | 경험의 체계화, 교훈의 전파, 조직 역량 성장 |

### 폭포수 vs 반복

| 모델 | 흐름 | 특성 |
|------|------|------|
| 폭포수 | 요구분석 → 설계 → 구현 → 테스트 → 배포 | 요구사항 변경에 취약, 피드백이 너무 늦음, 위험이 후반에 집중, 통합 문제 발견이 늦음 |
| 반복적 | [분석 → 설계 → 구현 → 테스트] × N | 조기 피드백, 위험 분산, 변경 수용 용이, 점진적 학습 |
...
| 차이점 | 아키텍처 강조 | 개인과 상호작용 강조 |
| 통합 가능 | 아키텍처 스파이크 + 스프린트, 모델링 세션 + 페어 프로그래밍, 반복 계획 + 백로그 관리 | — |
