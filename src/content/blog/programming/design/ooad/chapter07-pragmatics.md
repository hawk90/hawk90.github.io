...
> 좋은 설계만으로는 부족합니다. **팀 조직**, **재사용 전략**, **품질 보증**, **문서화**가 프로젝트 성공을 결정합니다.

## 팀 관리

기술적으로 완벽한 설계도 *팀이 제대로 동작하지 않으면* 성공할 수 없습니다. 프로젝트 실패의 대부분은 기술 문제가 아니라 *사람 문제*입니다. 의사소통 실패, 역할 혼란, 갈등.

### 팀 구성

작은 팀에서는 한 사람이 여러 역할을 겸합니다. 대규모 팀에서는 역할이 전문화됩니다.

| 역할 | 책임 | 작은 팀에서는 |
|------|------|-------------|
| 프로젝트 관리자 | 일정, 예산, 자원 관리. 이해관계자 의사소통 | 리드 개발자가 겸임 |
| 아키텍트 | 전체 아키텍처 설계. 기술 결정 | 시니어 개발자가 겸임 |
| 분석가 | 요구사항 수집. 도메인 모델링 | 팀 전체가 참여 |
| 설계자 | 클래스 설계. 인터페이스 정의 | 개발자가 겸임 |
| 개발자 | 코드 구현. 단위 테스트 | 핵심 역할 |
| 테스터 | 테스트 계획. 결함 보고 | 개발자가 겸임하거나 없음 |

**핵심 원칙**: 역할이 *누가 하느냐*보다 *누군가가 하느냐*가 중요합니다. 작은 팀에서 "우리는 테스터가 없어요"는 "테스트를 안 해도 돼요"가 아닙니다.
...
| 실수 | 증상 | 해결 |
|------|------|------|
| **역할 없이 시작** | "다 같이 하면 되지" → 아무도 책임지지 않음 | 최소한 "누가 최종 결정권자인가"만이라도 명확히 |
| **재사용 과신** | "나중에 다른 프로젝트에서도 쓰려고" → 과도한 일반화 | 지금 *두 번* 필요하면 그때 일반화 (DRY의 진정한 의미) |
| **테스트 후순위** | "기능 먼저, 테스트는 시간 나면" → 테스트 안 함 | 테스트는 *개발의 일부*, 별도 태스크가 아님 |
| **문서 과잉/과소** | 모든 것을 문서화 vs 아무것도 문서화 안 함 | "6개월 후의 나 자신"이 읽을 문서만 |
| **코드 리뷰 형식화** | "LGTM" 한 줄 리뷰, 리뷰 없이 병합 | 리뷰 체크리스트 도입, 최소 1개의 실질적 코멘트 요구 |
| **버전 관리 혼란** | 임의의 버전 번호, 릴리스 브랜치 없음 | Semantic Versioning 준수, 브랜치 전략 문서화 |
| **지식 사일로** | 특정 코드를 한 사람만 앎 | 페어 프로그래밍, 코드 리뷰, 지식 공유 세션 |

**핵심 원칙**: "이게 최선인가?"보다 "이걸 안 하면 어떻게 되는가?"를 먼저 물어보세요. 팀 관리, 문서화, 품질 보증 모두 *하지 않았을 때의 비용*이 명확합니다.
...
Chapter 8부터는 **케이스 스터디**를 다룹니다. 첫 번째는 위성 기반 항법 시스템 — 실시간, 분산, 안전 중시 시스템 아키텍처.
