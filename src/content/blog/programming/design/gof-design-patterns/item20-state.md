---
title: "GoF 20: State"
slug: "programming/design/gof-design-patterns/item20-state"
...
신호등은 *세 가지 상태*가 있습니다 — *빨강, 노랑, 초록*. 같은 *"시간이 흐른다"*는 입력이지만, 동작은 상태마다 다릅니다.

- *빨강*: 일정 시간 후 → *초록*으로
- *초록*: 일정 시간 후 → *노랑*으로
- *노랑*: 짧은 시간 후 → *빨강*으로

핵심은 *다음에 무엇이 될지를 상태 자신이 안다*는 것입니다. *빨강은 자기 다음이 초록*임을 압니다. 외부에서 *"다음은 노랑으로 바꿔주세요"*라고 시킬 필요가 없습니다.

State가 이 구조입니다.

- *신호등 자체* = Context
- *빨강·노랑·초록* = State 객체 (각자 클래스)
- *전이 규칙* = State 객체 안에 캡슐화

TCP 연결 상태, 자판기, 미디어 플레이어, 캐릭터 상태가 모두 같은 모델입니다.
...
<img src="/images/blog/gof/diagrams/item20-state-seq.svg" alt="State 시퀀스 — 같은 요청이 현재 State에 따라 다른 동작" style="max-width:100%; background:white; padding:8px; border-radius:6px;" />
...
| 비교 대상 | 무엇이 다른가 |
| --- | --- |
| [Strategy](/blog/programming/design/gof-design-patterns/item21-strategy) | 구조 동일. *State는 자기가 다음 상태로 전이* (내부). *Strategy는 외부에서 알고리즘 선택* (외부). |
| [Command](/blog/programming/design/gof-design-patterns/item14-command) | Command는 *동작 객체화*. State는 *상태 객체화*. |
| [Flyweight](/blog/programming/design/gof-design-patterns/item11-flyweight) | State 객체가 *stateless면 공유 가능* — Flyweight와 결합. |

판별 한 줄: *"같은 메서드 호출인데 객체의 내부 상태에 따라 다르게 동작하고, 상태가 스스로 다음 상태로 전이한다"*면 State.
