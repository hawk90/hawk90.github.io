...
가구점 IKEA의 시리즈를 떠올려봅시다. *MALM 시리즈*를 고르면 그 시리즈의 식탁, 의자, 침대, 서랍이 *디자인이 어울립니다*. *HEMNES 시리즈*를 골랐을 때도 마찬가지입니다.

손님 입장에서 *MALM 식탁 + HEMNES 의자*를 일부러 섞지 않습니다. 시리즈를 *하나 고르면* 그 안의 모든 가구가 *자동으로 통일*됩니다.

Abstract Factory는 이 *시리즈*에 해당합니다.

- 시리즈 = 팩토리 (`MacFactory`, `WinFactory`)
- 가구 = 제품 (`Button`, `Window`)
- *섞이지 않음* = 같은 팩토리에서 나온 부품끼리만 어울림

손님(클라이언트)이 *시리즈 하나*만 고르면 그 안의 모든 가구가 자동으로 일관되게 따라옵니다.
...
<img src="/images/blog/gof/diagrams/item01-abstract-factory-seq.svg" alt="Abstract Factory 시퀀스 — Client → Factory → Products" style="max-width:100%; background:white; padding:8px; border-radius:6px;" />
...
| 비교 대상 | 무엇이 다른가 |
| --- | --- |
| [Factory Method](/blog/programming/design/gof-design-patterns/item03-factory-method) | Factory Method는 *한 종류의 객체*를 서브클래스가 결정. Abstract Factory는 *여러 종류의 객체 군*을 한 팩토리가 일관성 있게. |
| [Builder](/blog/programming/design/gof-design-patterns/item02-builder) | Builder는 *복잡한 단일 객체*를 단계적으로 조립. Abstract Factory는 *여러 별개 객체*를 한 군으로 묶어 반환. |
| [Prototype](/blog/programming/design/gof-design-patterns/item04-prototype) | Prototype은 *기존 인스턴스를 복제*. Abstract Factory는 *매번 새로 생성*. 동적으로 군이 늘면 Prototype, 정적으로 결정되면 Abstract Factory. |

판별 한 줄: *"여러 종류의 객체가 같은 군에 속해야 한다"*면 Abstract Factory.
