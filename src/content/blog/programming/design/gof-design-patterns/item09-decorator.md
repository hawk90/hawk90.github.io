...
slug: "programming/design/gof-design-patterns/item09-decorator"
...
스타벅스에서 음료를 주문하는 과정을 떠올려봅시다.

- *기본*: 에스프레소
- *+ 우유* → 카페라떼
- *+ 우유 + 시럽* → 바닐라라떼
- *+ 우유 + 시럽 + 휘핑* → 바닐라라떼 with whip
- *+ 우유 + 시럽 + 휘핑 + 휘핑* → 더블 휘핑

각 *옵션*은 *기본 음료를 감싸 추가 기능을 더합니다*. 휘핑을 한 번 더 추가해도 *같은 인터페이스의 음료*입니다 — 마실 수 있고, 가격이 있고, 칼로리가 있습니다.

Decorator가 정확히 이 구조입니다.

- *기본 음료* = Component (구체 클래스)
- *옵션*(우유, 시럽, 휘핑) = Decorator
- *체인* = Decorator가 또 다른 Decorator를 감싸 적층
- *같은 인터페이스* = 어느 단계에서도 *음료*로 다룰 수 있음

옵션 *조합 폭발*(클래스 N개)을 *동적 적층*(런타임에 끼우기)으로 해결합니다.
...
| 비교 대상 | 무엇이 다른가 |
| --- | --- |
| [Adapter](/blog/programming/design/gof-design-patterns/item06-adapter) | Adapter는 *인터페이스 변환*. Decorator는 *인터페이스 유지 + 책임 추가*. |
| [Proxy](/blog/programming/design/gof-design-patterns/item12-proxy) | Proxy는 *접근 제어*가 목적 (caching, lazy load, 권한). Decorator는 *기능 추가*가 목적. |
| [Composite](/blog/programming/design/gof-design-patterns/item08-composite) | Composite는 *여러 자식*. Decorator는 *항상 자식 하나* (degenerate Composite). |
| [Chain of Responsibility](/blog/programming/design/gof-design-patterns/item13-chain-of-responsibility) | Decorator는 *항상 작업 수행*. CoR은 *처리하거나 패스*. |
| [Strategy](/blog/programming/design/gof-design-patterns/item21-strategy) | Strategy는 *알고리즘 통째로 교체*. Decorator는 *점진적 적층*. |

판별 한 줄: *"같은 인터페이스를 유지하면서 기능을 동적으로 더한다"*면 Decorator.
