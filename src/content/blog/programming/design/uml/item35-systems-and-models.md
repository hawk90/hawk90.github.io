...
slug: "programming/design/uml/item35-systems-and-models"
date: 2026-05-03T07:00:00
description: "35편의 UML 2.5.1 시리즈 정리. System은 Models를 포함하고, Models는 Diagrams를 가집니다."
...
> **"시스템 ⊃ 모델 ⊃ 다이어그램"** — 시스템은 여러 모델의 묶음, 모델은 여러 다이어그램으로 그려집니다. 35편 동안 다룬 모든 것이 이 한 줄에 모입니다.
...
한 시스템은 **여러 모델**로 봅니다. Kruchten의 4+1 뷰가 대표:
...
각 모델은 자기 다이어그램 집합을 가집니다.

## Diagram — 모델의 시각적 투영

다이어그램은 모델의 **한 페이지**입니다 — 모든 정보를 다 보여주지 않습니다.

- 같은 모델에서 여러 다이어그램이 나옵니다.
- 같은 요소가 다른 다이어그램에 다른 디테일로.
- 모델 자체는 도구의 데이터베이스에 들어있습니다.
...
1. **공통 어휘** — 팀이 같은 그림으로 같은 이야기를 합니다.
2. **다중 시각** — 시스템을 여러 면에서 봅니다.

도구·언어·플랫폼이 바뀌어도 이 두 가치는 변하지 않습니다.

> **"좋은 모델은 좋은 코드보다 오래갑니다."**
