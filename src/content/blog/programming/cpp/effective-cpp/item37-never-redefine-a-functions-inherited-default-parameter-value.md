...
slug: "programming/cpp/effective-cpp/item37-never-redefine-a-functions-inherited-default-parameter-value"
...
C++에서 가상 함수의 **본문은 동적 바인딩**, **기본 매개변수 값은 정적 바인딩**이라는 비대칭이 있다. 이 둘이 어긋나면 매우 헷갈리는 동작이 일어난다.

```cpp
Base* p = new Derived;
p->f();    // Derived의 본문 + Base의 기본 매개변수 값으로 호출!
```

객체는 Derived인데 함수 호출에 쓰이는 기본값은 Base의 것이다. 컴파일러는 경고하지 않고, 디버깅 시 함수 본문은 확실히 Derived인데 인자 값이 이상해 보이는 미스터리 버그가 된다.

해결책은 **derived에서 기본값을 재정의하지 않는 것**이다. 또는 NVI 패턴으로 기본값 의존을 base에 가둔다. 이 항목은 그 메커니즘과 회피 패턴을 정리한다.

## 개요

가상 함수의 본문은 **동적 바인딩**(런타임에 객체 실제 타입의 함수 호출)이지만, 기본 매개변수 값은 **정적 바인딩**(컴파일 타임에 호출자 포인터 타입으로 결정)이다. 둘이 어긋나면 **derived의 본문이 base의 기본값으로 호출**되는 매우 헷갈리는 동작이 일어난다.
...
- [항목 35: 가상 함수의 대안](/blog/programming/cpp/effective-cpp/item35-consider-alternatives-to-virtual-functions) — NVI 패턴
- [항목 36: non-virtual 재정의 금지](/blog/programming/cpp/effective-cpp/item36-never-redefine-an-inherited-non-virtual-function) — 정적 바인딩 함정의 사촌
- [항목 38: composition](/blog/programming/cpp/effective-cpp/item38-model-has-a-or-implemented-in-terms-of-through-composition) — 상속 대안
