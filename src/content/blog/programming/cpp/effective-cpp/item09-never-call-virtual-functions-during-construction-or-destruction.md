...
생성자나 소멸자 안에서 가상 함수를 호출하면, **현재 실행 중인 클래스 레벨의 함수**가 호출된다. derived의 오버라이드는 호출되지 않는다. 이는 직관에 어긋나지만 C++ 객체 모델의 필연적 결과 — derived 부분이 아직 존재하지 않거나 이미 사라진 상태에서 derived 함수를 부르면 UB가 되기 때문입니다.
