...
`new`가 메모리를 못 구하면 곧바로 `std::bad_alloc`을 던질 것 같지만, **그 전에 한 번 더 기회**가 있다. 등록된 **new-handler** 함수가 호출되고, 그것이 적절한 조치(메모리 풀 비우기, 다른 처리, 핸들러 교체)를 한 후에야 다시 할당이 시도된다.

이 메커니즘은 메모리 부족 상황에서 **단순 죽음 대신 우아한 복구**를 가능하게 한다. 임베디드 시스템, 게임 엔진, 장기 실행 서버 등에서 결정적이다.

이 항목은 new-handler 설치 방법, RAII 패턴, 클래스별 new-handler, nothrow `new`의 위치를 정리한다.

## 개요

`operator new`가 메모리를 못 구하면 곧바로 `std::bad_alloc`을 던지지 않는다. 먼저 등록된 **new-handler**(함수)를 호출하고, 그 핸들러가 적절한 조치를 한 후 다시 할당을 시도한다. 사용자는 이 핸들러를 설치해서 메모리 풀에서 release, 다른 핸들러로 교체, `bad_alloc` 던지기, 종료 등 다양한 정책을 구현할 수 있다.
...
- [항목 50: new/delete 교체 시기](/blog/programming/cpp/effective-cpp/item50-understand-when-it-makes-sense-to-replace-new-and-delete) — 사용자 정의 할당
