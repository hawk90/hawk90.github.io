...
`inline` 키워드를 둘러싼 오해가 많다. "`inline`을 적으면 빨라진다"는 통념은 거의 사실이 아니다.

진실은 이렇다.

- `inline`은 컴파일러에 **요청**일 뿐이다. 보장이 아니다.
- 컴파일러는 본문을 호출 지점에 펼치는 게 비용 대비 가치 있는지 **자체 판단**한다.
- 인라이닝의 장단은 트레이드오프다. 호출 비용 제거 + 최적화 기회 vs **코드 부피, 디버깅 어려움, ABI 영향**.

모던 컴파일러는 `inline` 없이도 적극적으로 인라인한다. LTO(Link-Time Optimization)가 등장하면서 컴파일 단위 경계도 사라졌다. **`inline` 키워드를 거의 안 적어도** 잘 최적화되는 경우가 많다.

이 항목은 inline의 정확한 의미, 자동 inline (클래스 안 정의), ODR 측면, 그리고 LTO 시대의 위치를 정리한다.

## 개요

`inline` 키워드는 컴파일러에게 **인라이닝을 요청**하는 것이다. **보장이 아니다**. 컴파일러는 함수 본문을 호출 지점에 펼치는 게 비용 대비 가치 있는지 자체 판단한다. 인라이닝의 장점(호출 비용 제거, 추가 최적화 기회)은 분명하지만, 코드 부피·디버깅 어려움·헤더-의존성·라이브러리 ABI 영향 등 단점도 크다. 모던 컴파일러 + LTO 환경에선 **`inline` 키워드를 거의 안 적어도 잘 동작**하는 경우가 많다.
