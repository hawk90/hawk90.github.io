...
slug: "programming/cpp/effective-cpp/item54-familiarize-yourself-with-the-standard-library-including-tr1"
date: 2026-05-04T06:00:00
description: "C++98 표준 라이브러리, TR1, 그리고 C++11~23에서 추가된 항목들 — 직접 만들기 전에 표준 검토."
tags: [cpp, effective-c++, standard-library, TR1]
series: "Effective C++"
seriesOrder: 54
draft: true
...
C++ 표준 라이브러리는 가장 저평가되는 자산이다. 많은 개발자가 자기가 필요한 도구를 **표준에 이미 있는지 확인하지 않고** 직접 만든다.

표준 라이브러리의 가치는 세 가지다.

- **동료 평가된 코드** — 표준 위원회가 검토하고 컴파일러 벤더가 구현한다.
- **이식성** — 모든 컴파일러에서 같이 동작한다.
- **최적화** — 평균적으로 잘 튜닝되어 있다.

직접 만들기 전에 표준에 같은 기능이 있는지 검토하는 습관 하나가 가장 큰 생산성 향상이다. 이 항목은 C++98 표준, TR1 (C++03 시기), 그리고 C++11~23의 주요 추가를 정리한다.

## 개요

C++ 표준 라이브러리는 동료 평가된 코드이고, 모든 컴파일러에 이식 가능하며, 평균적으로 잘 최적화되어 있다. **직접 만들기 전에 표준에 같은 기능이 있는지 검토하는 습관**이 가장 큰 생산성 향상이다. 이 항목은 C++98 표준, TR1 (C++03 시기), 그리고 C++11~23의 주요 추가를 정리한다.
