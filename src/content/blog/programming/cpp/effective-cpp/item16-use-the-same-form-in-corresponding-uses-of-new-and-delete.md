...
`delete[]`는 포인터 앞의 메타에서 N을 읽어 각 요소의 소멸자를 N번 호출. `delete`는 그런 메타가 없다고 가정 — 잘못된 짝지으면 잘못된 메모리 영역을 해석한다.
...
호출자는 `new AddressLines`만 보고 — "단일 객체겠지" 추측하기 쉽다. typedef가 배열을 감춰서 문법이 매칭을 깨트림.
...
C++의 `new`/`delete`는 C의 `malloc`/`free`와 호환되지 않는다. 짝을 정확히 맞춰야 함.
