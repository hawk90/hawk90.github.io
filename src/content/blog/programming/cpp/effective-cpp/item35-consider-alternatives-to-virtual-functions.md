...
slug: "programming/cpp/effective-cpp/item35-consider-alternatives-to-virtual-functions"
date: 2026-05-04T11:00:00
description: "NVI, Strategy 패턴, std::function, 템플릿 — 다형성 구현의 다양한 방법과 그 트레이드오프."
tags: [cpp, effective-c++, Virtual, design-pattern]
series: "Effective C++"
seriesOrder: 35
draft: true
...
가상 함수는 다형성의 가장 직관적 도구지만 **유일한 도구가 아니다**. 한 도구만 알면 모든 문제를 그 도구로 풀려는 경향이 생긴다. 사실 다형성을 구현하는 방법은 다섯 가지 이상이고, 각각의 트레이드오프가 다르다.

- **평범한 virtual** — 직관적이지만 인라인 불가, vtable 비용.
- **NVI (Non-Virtual Interface)** — public은 non-virtual, 내부에서 private virtual 호출. 호출 전후 처리 삽입 가능.
- **Strategy 패턴 + 함수 포인터** — 런타임 교체 가능, 객체 외부 함수 사용.
- **`std::function`** — Strategy의 일반화. 함수·람다·멤버 모두 받음.
- **템플릿 (정적 다형성)** — 컴파일 타임 결정, 인라인 가능, 그러나 인터페이스가 코드에 박힘.

이 항목은 다섯 방식을 비교하고 도메인에 맞는 선택 기준을 정리한다.

## 개요

가상 함수는 다형성의 가장 직관적 도구지만 유일한 도구는 아니다. **NVI**(Non-Virtual Interface), **Strategy** 패턴, **`std::function`**, **템플릿** 등 각자 다른 트레이드오프를 가진 대안이 있다. 이 항목은 다섯 가지 접근법을 비교하고, 도메인에 맞는 도구를 선택하는 기준을 제시한다.
