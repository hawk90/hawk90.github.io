...
slug: "programming/cpp/effective-cpp/item12-copy-all-parts-of-an-object"
date: 2026-05-04T12:00:00
description: "복사 함수에서 멤버와 base 부분을 빠뜨리지 않는 법. 멤버 추가 시 컴파일러는 침묵한다."
tags: [cpp, effective-c++, copy-constructor]
series: "Effective C++"
seriesOrder: 12
draft: true
...
복사 함수를 직접 작성하는 순간 **모든 책임이 본인 것**이 된다. 컴파일러가 자동 생성하던 memberwise copy가 사라지고, 개발자가 멤버 하나하나를 직접 챙겨야 한다.

가장 흔한 사고는 **나중에 멤버를 추가했을 때**다. 복사 함수를 업데이트하지 않으면 컴파일러는 **경고 없이** 부분 복사를 허용한다. 새 멤버는 초기화되지 않은 채 남는다. 디버깅이 매우 어려운 종류의 버그다.

이 항목은 두 가지 함정을 정리한다.

- 멤버 추가 시 복사 함수 업데이트 누락.
- 상속 시 base 클래스 부분 복사 누락 (`Base::operator=(rhs)` 호출 빠짐).

해결의 첫 단계는 **가능하면 컴파일러 자동 생성에 맡기는 것** (rule of zero)이다.

## 개요

복사 생성자와 복사 대입 연산자를 **직접 작성하면 책임도 본인 것**이다. 모든 멤버와 base 클래스 부분을 빠뜨리지 않고 복사해야 한다. 멤버를 새로 추가한 후 복사 함수를 업데이트하지 않으면 컴파일러는 **경고도 없이** 부분 복사를 허용한다. 컴파일러 자동 생성에 맡길 수 있으면 그게 가장 안전한 길이다.
...
- [항목 5: C++가 자동 생성하는 함수들](/blog/programming/cpp/effective-cpp/item05-know-what-functions-cpp-silently-writes) — rule of zero 기반
- [항목 11: 자기 대입 처리](/blog/programming/cpp/effective-cpp/item11-handle-assignment-to-self-in-operator-equals) — copy-and-swap의 다른 측면
- [항목 13: 자원 관리에는 객체](/blog/programming/cpp/effective-cpp/item13-use-objects-to-manage-resources) — 복사 책임을 RAII에 위임
