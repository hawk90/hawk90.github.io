...
slug: "programming/cpp/effective-cpp/item21-dont-try-to-return-a-reference-when-you-must-return-an-object"
date: 2026-05-04T21:00:00
description: "지역 변수 참조 = 댕글링, 힙 + 참조 = 누수, static + 참조 = 공유 함정. RVO/move를 신뢰하고 값으로 반환."
tags: [cpp, effective-c++, Reference, return-value]
series: "Effective C++"
seriesOrder: 21
draft: true
...
`result`는 stack 변수 — 함수 스코프가 끝나면 사라진다. 반환된 참조가 가리키는 메모리는 곧 다른 용도로 재사용. 디버그 빌드에선 값이 그대로 남아 동작할 수도 있지만 — 표준상 UB.
...
참조 반환은 호출자에게 "delete 호출 책임"을 전달할 수단이 없다. raw pointer 반환이라면 호출자가 delete 해야 하는데, 참조는 그 의도 자체가 부재.
