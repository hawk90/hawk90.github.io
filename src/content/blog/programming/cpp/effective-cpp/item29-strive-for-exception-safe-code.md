...
slug: "programming/cpp/effective-cpp/item29-strive-for-exception-safe-code"
date: 2026-05-04T05:00:00
description: "기본·강력·noexcept 보증 세 단계, copy-and-swap 패턴, 가장 약한 단계가 함수 전체 보증을 결정."
tags: [cpp, effective-c++, exception-safety]
series: "Effective C++"
seriesOrder: 29
draft: true
...
예외가 발생해도 프로그램이 깨지지 않으려면 두 가지가 보장돼야 한다.

- **자원 누수 없음** — RAII로 해결한다.
- **데이터 구조의 일관성** — 예외 후에도 객체가 사용 가능한 상태여야 한다.

C++ 표준 라이브러리와 실무는 이 보장을 **세 단계**로 분류한다.

- **basic** — 자원 누수 없음, 객체는 유효하지만 정확한 상태는 불명.
- **strong** — 작업 실패 시 호출 이전 상태로 완전 복귀 (트랜잭션 의미).
- **nothrow** — 예외가 절대 안 나옴.

강한 보증일수록 사용자 코드가 단순해진다. copy-and-swap 패턴이 강력 보증을 우아하게 구현하는 핵심 도구다. 이 항목은 세 단계와 그 구현 패턴을 정리한다.

## 개요

예외-안전(exception-safe) 코드는 두 가지를 보장한다.

1. **자원 누수 없음** (RAII로 해결).
2. **데이터 구조의 일관성** (예외 후에도 객체가 깨지지 않음).

세 단계 보증 — **basic**, **strong**, **nothrow** — 이 있으며, 강할수록 사용자 코드가 단순해진다. copy-and-swap 같은 패턴이 강력 보증을 단순하게 구현하는 핵심 도구다.
