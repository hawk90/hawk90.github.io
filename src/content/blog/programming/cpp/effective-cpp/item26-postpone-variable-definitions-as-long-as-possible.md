...
slug: "programming/cpp/effective-cpp/item26-postpone-variable-definitions-as-long-as-possible"
date: 2026-05-04T02:00:00
description: "사용 직전에 정의 — 불필요한 생성·소멸 회피, 의미 있는 값으로 초기화, 루프 안/밖의 트레이드오프."
tags: [cpp, effective-c++, Performance, Initialization]
series: "Effective C++"
seriesOrder: 26
draft: true
...
C 시절엔 함수 시작에 모든 변수를 선언하는 것이 표준 컨벤션이었다. C에서는 변수가 단순한 메모리 자리라 비용이 거의 없었기 때문이다.

C++에선 다르다. 변수 하나가 **생성자 + 소멸자**라는 무거운 행위를 수반한다. 함수 도입부에 일찍 정의하면 두 가지 비용이 생긴다.

- **사용 안 될 수도 있는 변수**가 예외 경로/조기 return에서 만들었다 바로 소멸한다.
- **의미 없는 default 값**으로 만들고 나중에 대입하면 default ctor + 대입의 두 단계가 일어난다.

해결책은 단순하다. **사용 직전에 의미 있는 초기치로 정의**한다. 효율과 가독성 모두에 유리하다. 이 항목은 그 원칙과 루프 안/밖의 트레이드오프를 정리한다.

## 개요

C 시절의 컨벤션은 함수 시작에 모든 변수를 선언하는 것이었지만, **C++의 변수는 생성자·소멸자라는 무거운 행위가 따라온다**. 사용하지 않을 변수까지 함수 도입부에 만들면 예외 경로에선 만들었다가 바로 소멸하는 비용이 생긴다. 사용 직전에 의미 있는 초기치로 정의하는 것이 효율과 가독성 모두에 유리하다.
