...
C++ 템플릿을 처음 다룰 때 가장 헷갈리는 자리 하나가 **`typename`이 도대체 왜 두 곳에 다 쓰이는가**다.

```cpp
template<typename T>                       // ① 매개변수 선언
void f() {
    typename T::iterator it = ...;          // ② 의존 타입 명시
}
```

두 의미가 완전히 다르다.

- **①** — "T는 타입 매개변수다." `class T`와 동일한 용법.
- **②** — "`T::iterator`는 (값이 아니라) **타입**이다." 컴파일러는 의존 이름의 기본을 값으로 가정하므로, 타입임을 명시해야 한다.

C++20부터 일부 자리(상속 base, ::기반 선언 등)에서 ②의 `typename`이 생략 가능해졌다. 다만 여전히 많은 자리에서 필수다. 이 항목은 두 의미의 정확한 구분과 C++20의 완화를 정리한다.

## 개요

`typename` 키워드는 C++ 템플릿에서 **두 가지 다른 의미**로 사용된다.

1. **템플릿 매개변수 선언** — `class`와 동일한 용법.
2. **의존 타입(dependent type) 명시** — "이건 타입이다"를 컴파일러에 알림.

같은 키워드인데 의미가 다른 두 자리에 등장한다. 처음 마주치면 혼란스럽지만, 두 의미가 모두 필요한 이유가 있다. C++20부터 일부 자리에서 생략이 가능하다.
