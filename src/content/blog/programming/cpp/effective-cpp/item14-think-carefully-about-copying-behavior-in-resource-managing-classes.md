...
slug: "programming/cpp/effective-cpp/item14-think-carefully-about-copying-behavior-in-resource-managing-classes"
date: 2026-05-04T14:00:00
description: "복사 금지·참조 카운트·깊은 복사·소유권 이전 — 네 가지 복사 정책과 C++11 이동 의미론의 결합."
tags: [cpp, effective-c++, RAII, Copy]
series: "Effective C++"
seriesOrder: 14
draft: true
...
자원 관리 클래스(RAII)에서 가장 먼저 결정해야 할 것은 **복사 의미론**이다. "이 객체를 복사하면 자원에는 무슨 일이 일어나야 하는가?"

이 질문에 답을 안 정하면 컴파일러가 자동 생성한 정책(멤버별 비트 복사)이 적용된다. raw 포인터 멤버를 가진 RAII 객체에선 거의 항상 잘못된 동작이다. 이중 해제, 댕글링, 자원 손실로 이어진다.

이 항목은 네 가지 흔한 복사 정책과 그 선택 기준을 정리한다.

- **복사 금지** — 뮤텍스, 파일 핸들 같은 독점 자원.
- **참조 카운트** — 공유 가능한 자원 (`shared_ptr` 스타일).
- **깊은 복사** — 자원의 사본을 만든다 (`string`, `vector` 스타일).
- **소유권 이전** — 한 객체만 자원을 가진다 (`unique_ptr` 스타일).

## 개요

자원 관리(RAII) 클래스를 만들 때 가장 먼저 물어야 할 질문은 "**이 클래스를 복사하면 자원에 어떤 일이 일어나야 하는가?**" 다. 답은 자원의 성격(공유 가능한가, 독점인가)에 따라 다르다. 네 가지 흔한 정책 중 도메인에 맞는 것을 의식적으로 선택하지 않으면 컴파일러가 자동 생성하는 정책(보통 멤버별 비트 복사)이 적용된다. 거의 항상 잘못된 동작을 한다.
