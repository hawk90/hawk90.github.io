...
C++98에 작성된 클래스, 또는 명시적으로 copy를 정의해 자동 move 생성이 막힌 클래스 ([항목 17](/blog/programming/cpp/effective-modern-cpp/item17-understand-special-member-function-generation)).
...
`std::vector::push_back`이 재할당할 때, move 생성자가 `noexcept`이어야만 move 사용 ([항목 14](/blog/programming/cpp/effective-modern-cpp/item14-declare-functions-noexcept-if-they-wont-emit-exceptions)). 그렇지 않으면 copy로 떨어짐.
