...
slug: "programming/cpp/effective-modern-cpp/item42-consider-emplacement-instead-of-insertion"
date: 2026-05-04T18:00:00
description: "emplace_back 등 emplace API가 push_back보다 효율적인 시점과 주의점."
tags: [cpp, Container, Performance, modern-cpp]
...
`v.push_back("hello")`는 `const char*`를 `std::string` 임시 객체로 변환한 뒤 컨테이너로 move한다. `v.emplace_back("hello")`는 컨테이너 안에서 string을 **직접 생성**한다. 임시 객체가 사라진다.

성능 이득이 있을 수 있지만, **항상** 그런 건 아니다. 게다가 emplace에는 네 가지 함정이 있다.

- explicit 생성자도 호출되어 의도치 않은 객체가 만들어진다.
- `new Widget` 같은 raw resource를 emplace에 직접 넘기면 예외 시 누수가 일어난다.
- `{1, 2, 3}` 같은 braced init list는 추론에 실패한다.
- 인자 검증의 한 단계가 사라져 타입 안전성이 약해진다.

이 항목은 emplace가 진짜로 이기는 세 조건과 push_back을 유지해야 하는 자리를 정리한다. "무지성 emplace 치환"이 위험한 이유까지 함께 본다.

## 개요

`std::vector::push_back(x)`는 인자를 컨테이너로 **복사 또는 이동**한다. `emplace_back(args...)`는 컨테이너 안에서 **직접 객체를 생성**한다. 임시 객체가 사라진다.

성능 이득이 클 수도 있지만, 항상 그런 건 아니고 함정도 있다. 무조건 emplace로 치환은 위험하다.
...
`emplace_back("hello")` → `T("hello")` 직접 호출이다. 이 경우 `T = std::string`이라 `string(const char*)` 호출이다. 컨테이너 내부 메모리에서 직접 만든다.
...
`set`/`map`은 중복이면 추가에 실패한다. emplace는 일단 객체를 만들고 검사한다. 중복이 빈번하면 손해다.

### 조건 3 — 컨테이너가 저장 형태와 무관하게 객체 생성

`map`처럼 pair로 저장하는 컨테이너는 emplace가 분리 인자를 받을 수 있다.
...
`emplace`는 **direct initialization**을 사용한다. `explicit` 생성자도 호출한다. 의도와 다른 객체 생성의 위험이 있다.
...
explicit 생성자가 있는 타입은 emplace로 의도치 않은 객체 생성의 위험이 있다.
...
resource를 즉시 wrap (`make_shared`/`make_unique`)하면 안전하다.
...
emplace는 perfect-forward한다. `{1,2,3}` 같은 braced init list는 타입이 아니라 deduce에 실패한다.
...
push_back을 쓰면 호출자가 명시적으로 `Date(2020,13,99)`를 만들어야 한다. 한 단계라도 실수 발견이 가능하다. emplace는 그 단계를 합쳐 약점이 묻힌다.
...
emplace의 이득은 **변환 비용**에 비례한다. 이미 element 타입 인자 / 트리비얼 타입엔 차이가 거의 없다.
...
"emplace가 항상 빠르다"는 신화는 ❌다.

## 핵심 정리

1. **emplace = 컨테이너 안에서 직접 생성**이다. 임시 객체가 없다.
2. **변환이 필요한 인자**, **중복 거의 없는 컨테이너**에서 이득이 있다.
3. **explicit 생성자도 호출**한다. 의도치 않은 객체 생성에 주의한다.
4. **raw resource (`new`) 직접 전달 시 예외-안전성 손실**이 있다. `make_shared`/`make_unique`를 사용한다.
5. 무지성 emplace 치환은 X다. 조건을 확인한 후 적용한다.
6. 이미 element 타입 인자엔 차이가 없다.
