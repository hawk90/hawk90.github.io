...
별도 힙 할당. `make_shared`는 객체와 함께 한 번에 할당 — 효율적 ([항목 21](/blog/programming/cpp/effective-modern-cpp/item21-prefer-make-unique-and-make-shared-to-direct-new)).
...
해결: 한쪽을 **`weak_ptr`** ([항목 20](/blog/programming/cpp/effective-modern-cpp/item20-use-weak-ptr-for-shared-ptr-like-pointers-that-can-dangle)).
...
`make_shared`는 객체와 control block을 **한 메모리 블록**으로 — 두 번 할당 → 한 번. 자세한 건 [항목 21](/blog/programming/cpp/effective-modern-cpp/item21-prefer-make-unique-and-make-shared-to-direct-new).
...
5. **사이클은 `weak_ptr`로 끊기** ([항목 20](/blog/programming/cpp/effective-modern-cpp/item20-use-weak-ptr-for-shared-ptr-like-pointers-that-can-dangle))
