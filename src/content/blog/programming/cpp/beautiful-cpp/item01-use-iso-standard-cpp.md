...
"내 컴퓨터에선 되는데"는 C++ 개발자가 가장 자주 마주치는 문장이다. 같은 코드가 Windows에선 돌고 Linux에선 안 돌거나, MSVC에선 통과하지만 Clang에선 에러가 난다. 팀원·CI 서버·고객 환경마다 결과가 달라진다.

원인은 단순하다. **ISO 표준 외의 것에 의존했기 때문**이다. 컴파일러 확장 기능, 플랫폼 API, 비표준 헤더 — 한 환경에선 자연스럽지만 다른 환경에선 존재하지 않는 것들.

해결책도 단순하다. **표준 C++ 안에서 작성**하고, 플랫폼 차이는 **별도 파일에 캡슐화**한다. 컴파일러 확장 대신 표준 라이브러리를 쓴다. 여러 컴파일러로 정기적으로 빌드한다.

이 항목은 이식성을 깨는 흔한 패턴과 표준으로 우회하는 방법, 그리고 임베디드처럼 표준의 일부만 쓸 수 있는 환경의 트레이드오프까지 정리한다.
