...
slug: "programming/cpp/beautiful-cpp/item02-prefer-default-arguments"
date: 2026-05-05T02:00:00
description: "함수 오버로딩과 기본 인수를 언제, 어떻게 선택해야 하는지"
tags: [cpp, api-design, default-arguments]
series: "Beautiful C++"
seriesOrder: 2
draft: true
...
함수의 옵션이 늘어날 때 두 가지 선택지가 있다. **오버로딩** 여러 개를 만들거나, **기본 인수** 하나를 추가하거나. 둘 다 같은 일을 하는 것처럼 보이지만 트레이드오프가 다르다.

오버로딩은 함수 시그니처가 매번 새로 만들어진다. 함수 개수가 늘어나고 본문이 중복된다. 인터페이스가 복잡해 보이고 호출자가 어느 버전을 쓸지 헷갈린다.

기본 인수는 함수 하나에 옵션이 명시된다. 호출자가 "필요한 만큼만" 적으면 된다. 다만 ABI 호환성·가상 함수와의 결합에서 함정이 있다.

이 항목은 둘의 선택 기준과 기본 인수의 함정을 정리한다.
