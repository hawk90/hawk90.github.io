...
└── Temporal Unit (시간 단위)
    └── Frame (프레임)
        ├── Sequence Header (전역 설정)
        ├── Frame Header (프레임별 설정)
        └── Tile Group (타일 데이터)
            └── Tile (병렬 처리 단위)
                └── Superblock (64×64 또는 128×128)
                    └── Block (4×4 ~ 128×128)
```
...
[OBU 파싱] ─── Ch 2 (이 장)
 ↓
[Sequence Header] ─── Ch 3
 ↓
[Frame Header] ─── Ch 5
 ↓
[Tile / Superblock / Partition] ─── Ch 3, 4
 ↓
[엔트로피 디코딩] ─── Ch 7
 ↓
[Intra/Inter 예측] ─── Ch 8, Ch 12~15
 ↓
[역양자화 + 역변환] ─── Ch 9
 ↓
[복원: 예측 + 잔차] ─── Ch 10
 ↓
[Deblocking → CDEF → Loop Restoration] ─── Ch 16~18
 ↓
[Film Grain Synthesis] ─── Ch 19
 ↓
Output Frame
```
...
│ Bit │  7      │  6-3      │    2     │   1    │   0   │
├─────┼─────────┼───────────┼──────────┼────────┼───────┤
│     │ forbidn │ obu_type  │ extension│has_size│reservd│
│     │ (0)     │ (4 bits)  │ flag     │ field  │ (0)   │
└─────┴─────────┴───────────┴──────────┴────────┴───────┘

extension_flag=1이면 추가 1바이트:
┌───────────────────────────────────────────────────────┐
│ Byte 1: Extension Header                               │
├─────────────┬─────────────┬───────────────────────────┤
│   7-5       │    4-3      │         2-0               │
├─────────────┼─────────────┼───────────────────────────┤
│temporal_id  │ spatial_id  │       reserved            │
