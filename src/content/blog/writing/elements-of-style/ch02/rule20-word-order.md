---
title: "Ch 2 Rule 20: 관련 있는 단어를 함께 두라"
slug: "writing/elements-of-style/ch02/rule20-word-order"
date: 2026-05-14T22:00:00
description: "문장에서 어순은 의미를 결정한다. 관련 있는 단어들은 가까이, 관련 없는 단어들은 멀리."
series: "The Elements of Style"
seriesOrder: 22
tags: [writing, composition, word-order, strunk, white]
type: book-review
bookTitle: "The Elements of Style"
bookAuthor: "William Strunk Jr., E.B. White"
draft: true
topics: ["writing"]
---

> Keep related words together.

문장에서 어순은 의미를 결정합니다. 영어는 어순 의존이 강한 언어입니다. 관련 있는 단어들은 가까이, 관련 없는 단어들은 멀리 둡니다.

## 기본 원칙

```
모호:
"He noticed a large stain in the rug that was right in the center."

→ 얼룩이 가운데? 양탄자가 가운데?
→ "that was right in the center"가 무엇을 수식하는가?

명확:
"He noticed a large stain right in the center of the rug."

→ 얼룩이 가운데 있음
→ 어순 변경으로 모호함 제거
```

## 우스꽝스러운 어순

어순이 잘못된 자리는 우스워지거나 무서워집니다.

```
우스움:
"New York's first commercial human-sperm bank opened Friday
with semen samples from eighteen men frozen in a stainless
steel tank."

→ 18명의 남자가 냉동됐는가? (실은 정자가 냉동)

수정:
"New York's first commercial human-sperm bank opened Friday.
Samples from eighteen men were then frozen in a stainless
steel tank."
```

```
우스움:
"I saw the man with the telescope."

→ 누가 telescope를 가졌는가? (모호)

명확:
"Using the telescope, I saw the man."
또는: "I saw the man who had a telescope."
```

## 주어와 동사 — 가까이

주어와 동사 사이를 긴 구·절로 끊지 마세요. 흐름이 끊깁니다.

```
끊어짐:
"Toni Morrison, in Beloved, writes about characters who have
escaped from slavery but are haunted by its heritage."

분석:
- 주어 "Toni Morrison"
- 끼어든 구 "in Beloved"
- 동사 "writes"
- 약간 끊긴 느낌

선호 (흐름):
"In Beloved, Toni Morrison writes about characters who have
escaped from slavery but are haunted by its heritage."
```

전치사구를 문장 시작으로 옮기면 주어와 동사가 바로 이어집니다.

### 더 긴 끊김

```
회피 (매우 긴 끊김):
"The proposal, which was made last week by the committee that
had been working on it for several months and which faced
considerable opposition from various stakeholders, was finally
approved."

→ 주어 "proposal"과 동사 "was approved" 사이가 너무 멀다

수정:
"The proposal was finally approved. The committee had worked on
it for several months, facing considerable opposition from
various stakeholders."

→ 두 문장으로 나눠 주어-동사 거리를 줄임
```

## 관계대명사와 선행사

관계대명사는 선행사 바로 뒤에 옵니다.

```
모호:
"A proposal to amend the Sherman Act, which has been variously
judged ..."

→ 제안이 평가됨? 법이 평가됨?
→ "which"가 무엇을 가리키는지 모호

명확:
"A proposal, which has been variously judged, to amend the
Sherman Act ..."

→ "which"가 proposal을 명확히 가리킴
```

### Dangling Relative Clause

```
회피:
"He had a small farm in the country that grew vegetables."

→ 시골이 채소를 길렀나? (모호)

선호:
"In the country, he had a small farm that grew vegetables."
또는: "His small farm in the country grew vegetables."
```

## 수식어 — 수식 대상에 붙이기

수식어는 수식 대상에 바로 인접해야 합니다.

```
회피 (모호):
"All the members were not present."

→ 전원 없음? 일부 없음?
→ not의 위치 때문에 모호

선호 (명확):
"Not all the members were present."  ← 일부만 없음
또는: "None of the members were present."  ← 아무도 없음
```

```
회피:
"She only found two mistakes."

→ only가 무엇을 수식하는가?
→ 그녀만? 발견했다는 행위만? 두 개의 실수만?

선호:
"She found only two mistakes."  ← 두 개뿐
"Only she found two mistakes."  ← 그녀만이 발견
"She only found two mistakes."  (그녀가 한 일은 발견뿐, 수정 안 함)
```

### *Only*의 위치

`Only`는 위치에 따라 의미가 완전히 다릅니다.

```
"Only I love you."          ← 나만이 (다른 누구도 아닌)
"I only love you."          ← 사랑할 뿐 (다른 행위 안 함)
"I love only you."          ← 너만 (다른 누구도 아닌)
"I love you only."          ← 너만 (강조)
```

수식 대상 바로 앞에 두는 것이 표준입니다.

## 부사구 / 전치사구의 위치

```
회피:
"He told us yesterday that he had passed the exam."

→ yesterday가 told? 또는 passed?
→ 모호

선호 (명확):
"He told us that he had passed the exam yesterday."
   ← yesterday = passed (어제 시험에 합격)

"Yesterday, he told us that he had passed the exam."
   ← yesterday = told (어제 말했다)
```

## Squinting Modifier — 모호한 수식어

양쪽 어느 것이든 수식할 수 있는 수식어를 squinting(곁눈질) 수식어라 부릅니다.

```
회피:
"The teacher said on Monday she would give a test."

→ 월요일에 말했다? 월요일에 시험?

선호:
"On Monday, the teacher said she would give a test."  ← 말한 시점
"The teacher said she would give a test on Monday."   ← 시험 시점
```

## Misplaced Modifier의 우스꽝

```
회피 (전치사구):
"For sale: an antique desk for a lady with thick legs and
large drawers."

→ 두꺼운 다리의 부인? 책상의 두꺼운 다리?

선호:
"For sale: an antique desk with thick legs and large drawers,
suitable for a lady."
```

```
회피 (분사구):
"Walking down the road, the trees were beautiful."

→ 나무들이 걸음? (Rule 11 dangling)

선호:
"Walking down the road, I noticed the beautiful trees."
또는: "As I walked down the road, the trees looked beautiful."
```

## 영어의 어순 의존

영어는 어순이 의미를 결정합니다. 격 어미·조사가 없기 때문입니다.

```
영어:
"Dog bites man." vs "Man bites dog."
→ 같은 단어, 어순이 다르면 의미가 완전히 다름

한국어:
"개가 사람을 물었다." vs "사람을 개가 물었다."
→ 조사로 구분되므로 어순이 상대적으로 자유
```

영어는 어순에 모든 정보가 의존합니다.

## 한국어의 어순

한국어는 조사 덕분에 어순이 유연합니다. 그러나 수식에는 영어와 같은 문제가 있습니다.

```
회피 (한국 모호):
"빠르게 달리는 사람을 보았다."

→ 빠르게 달리는 사람? 빠르게 보았다?
→ 보통 전자로 해석되지만 모호

선호 (명확):
"달리는 사람을 빠르게 보았다."  ← 빠르게 보았다
"빠르게 달리는 사람을 보았다."  ← 빠르게 달림 (일반 해석)
```

### *Only*의 한국어 "만"

```
"오직 나만이 너를 사랑해."   ← 나만이
"나는 오직 너만 사랑해."   ← 너만
"나는 너를 사랑할 뿐이야." ← 사랑하는 행위뿐
```

"만", "뿐", "오직"의 위치가 의미를 결정합니다.

## 자기 글 점검

- [ ] 모든 수식어가 수식 대상에 인접한가?
- [ ] 주어와 동사 사이에 긴 끼어듦이 없는가?
- [ ] 관계대명사가 선행사 바로 뒤에 있는가?
- [ ] *Only*, *just*, *even*, *almost* 같은 부사가 올바른 위치인가?
- [ ] Dangling·squinting modifier가 없는가?
- [ ] 문장이 읽고 한 번에 이해되는가?
- [ ] 한국 글에 모호한 수식이 없는가?

## 다음 Rule 예고

다음은 **Rule 21: 동사 시제는 일관되게**입니다.

## 관련 항목

- [Rule 11: 현수 분사](/blog/writing/elements-of-style/ch01/rule11-dangling)
- [Rule 19: 평행 구조](/blog/writing/elements-of-style/ch02/rule19-parallel)
- [Rule 21: Keep tense consistent](/blog/writing/elements-of-style/ch02/rule21-tense)
