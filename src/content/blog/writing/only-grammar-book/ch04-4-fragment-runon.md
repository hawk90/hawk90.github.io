---
title: "Chapter 4.4: Fragments and Run-ons"
date: 2026-05-14T04:40:00
description: "단편 문장과 런온 문장 — 식별, 회피, 수정 전략."
series: "The Only Grammar Book You'll Ever Need"
seriesOrder: 26
tags: [writing, grammar, mistakes, fragment, run-on]
type: book-review
bookTitle: "The Only Grammar Book You'll Ever Need"
bookAuthor: "Susan Thurman"
draft: true

---

> 문장은 *완전*해야 하고 *적절히 끝나야* 합니다. *단편*은 *덜 끝난* 문장, *런온*은 *너무 길게 이어진* 문장입니다.

## 정의 및 개요

영어 문장의 두 가지 구조적 오류:

- **Fragment (단편 문장)** — 완전한 문장이 아닌 단위가 문장처럼 쓰임
- **Run-on (연결 오류)** — 두 독립절이 *적절한 부호·접속사 없이* 연결됨

두 오류 모두 *문장의 경계*에 대한 오해에서 비롯됩니다. 한국어 화자는 *단편*에서, 영어 화자도 *런온*에서 자주 실수합니다.

## Fragment (단편 문장)

*완전한 문장*은 *주어 + 동사 + 완결된 의미*. 셋 중 하나라도 빠지면 단편.

### 유형 1: 종속절을 단독 문장으로

| 분류 | 예문 |
|---|---|
| 회피 | Because it was raining. (종속절만 — 독립절 필요) |
| 바름 | Because it was raining, we stayed home. |
| 바름 | We stayed home because it was raining. |

종속접속사 (because, although, when, if, since, while …)로 시작하면 *반드시 독립절과 결합*.

### 유형 2: 구를 문장으로

| 분류 | 예문 |
|---|---|
| 회피 | Walking down the street. (분사구만 — 동사 없음) |
| 바름 | Walking down the street, I saw a rainbow. |
| 바름 | She was walking down the street. |
| 회피 | The book on the table. (명사구만 — 동사 없음) |
| 바름 | The book on the table is mine. |
| 회피 | To finish the project. (부정사구만) |
| 바름 | To finish the project, we need more time. |

### 유형 3: 주어 또는 동사 누락

| 분류 | 예문 |
|---|---|
| 회피 | Went to the store.   (주어 없음) |
| 바름 | I went to the store. |
| 회피 | She to the store.    (동사 없음) |
| 바름 | She went to the store. |

### 유형 4: 보조 정보의 분리

| 분류 | 예문 |
|---|---|
| 회피 | She passed the exam. With the highest score in the class. |
| 바름 | She passed the exam with the highest score in the class. |
| 바름 | She passed the exam — with the highest score in the class. |

### 유형 5: 관계절을 문장으로

| 분류 | 예문 |
|---|---|
| 회피 | The man who lives next door. Is a doctor. |
| 바름 | The man who lives next door is a doctor. |

### 의도적 단편 (예외)

문학·창작·광고에서 *효과를 위해* 의도적으로 단편을 사용합니다.

```text
허용 (문학적): It was a cold morning. Bitter. Unforgiving.
       (단편 — 강조 효과)
허용 (대화): "Are you coming?" "Yes. Tomorrow."
허용 (광고): Just do it.
```

학술·격식 글에서는 *완전한 문장*이 표준.

### 단편 식별 방법

자기 글의 각 *문장*에 대해:

1. *주어*가 있는가?
2. *정형 동사*가 있는가?
3. *완전한 의미*를 전달하는가?

셋 모두 *예*가 아니면 단편.

## Run-on Sentence (런온 문장)

두 *독립절*이 *적절한 부호·접속사 없이* 연결된 문장. 두 가지 유형.

### 유형 1: Fused Sentence (융합 문장)

부호도 접속사도 없이 두 독립절을 *공백만으로* 연결.

```text
회피: She came he left.
회피: It was raining we stayed home.
회피: The deadline is tomorrow we need to hurry.
```

### 유형 2: Comma Splice (쉼표 연결)

두 독립절을 *쉼표만으로* 연결.

```text
회피: She came, he left.
회피: It was raining, we stayed home.
회피: The deadline is tomorrow, we need to hurry.
```

쉼표는 *FANBOYS와 함께*만 두 독립절을 연결할 수 있습니다.

## Run-on 수정 방법

### 방법 1: 두 문장으로 분리

```text
바름: She came. He left.
바름: It was raining. We stayed home.
```

가장 간단. 두 절이 *대등하고 가까운 의미*가 없을 때.

### 방법 2: 등위접속사 + 쉼표

```text
바름: She came, and he left.
바름: It was raining, so we stayed home.
바름: The deadline is tomorrow, but we need to hurry.
```

### 방법 3: 세미콜론

```text
바름: She came; he left.
바름: It was raining; we stayed home.
```

두 절이 *의미적으로 가까울 때*.

### 방법 4: 종속접속사 (복문)

```text
바름: When she came, he left.
바름: Because it was raining, we stayed home.
바름: The deadline is tomorrow, so we need to hurry.
바름: Although the deadline is tomorrow, we should not rush.
```

종속접속사로 *논리 관계*를 명시.

### 방법 5: 접속 부사 + 세미콜론

```text
바름: She came; however, he left.
바름: It was raining; therefore, we stayed home.
```

### 다섯 방법 비교

| 정보 | 방법 | 결과 |
|---|---|---|
| 비가 왔다. 집에 있었다. | 두 문장 | It was raining. We stayed home. |
| | 등위접속사 | It was raining, so we stayed home. |
| | 세미콜론 | It was raining; we stayed home. |
| | 종속접속사 | Because it was raining, we stayed home. |
| | 접속 부사 | It was raining; therefore, we stayed home. |

각 방법이 *다른 효과*. 글의 흐름과 강조에 따라 선택.

## 자주 헷갈리는 자리

### 접속 부사를 접속사로 착각

가장 흔한 comma splice 원인.

| 분류 | 예문 |
|---|---|
| 회피 | She tried hard, however she failed. |
| 바름 | She tried hard; however, she failed. |
| 바름 | She tried hard. However, she failed. |
| 회피 | We were tired, therefore we left early. |
| 바름 | We were tired; therefore, we left early. |
| 바름 | We were tired, so we left early. |

*However, therefore, moreover, furthermore, nevertheless, consequently, thus, indeed, instead*은 *접속 부사*. 두 절을 *쉼표만으로* 잇지 못합니다.

### 짧은 평행절은 예외 허용 (문학)

```text
허용 (문학): I came, I saw, I conquered.
격식: I came; I saw; I conquered.
바름: I came, saw, and conquered.   (한 주어 + 직렬)
```

### 직접 인용의 처리

직접 인용은 그 자체가 *독립절*이므로 *쉼표 + 인용*은 OK.

```text
바름: She said, "I will come."
바름: "I will come," she said.
```

### 동격은 쉼표로 연결 가능

```text
바름: My friend John, the doctor, called.
```

동격은 *명사구*이므로 독립절이 아님.

## 한국어 화자의 함정

### 1. 종속절 단독

한국어는 종속절이 *문장처럼 보일 수* 있어 영어로 옮길 때 단편이 됨.

| 분류 | 예문 |
|---|---|
| 회피 | 비가 왔기 때문에. → Because it was raining. |
| 바름 | 비가 왔기 때문에 집에 있었다. → Because it was raining, we stayed home. |

### 2. *그리고, 그러나* 직역의 comma splice

한국어 *그리고/그러나*는 *접속사*이지만, *however/therefore*는 *접속 부사*.

```text
회피 (직역): 그녀는 노력했다. 그러나 실패했다. → She tried hard, however she failed.
바름: She tried hard; however, she failed.
바름: She tried hard, but she failed.
```

### 3. 너무 긴 문장 — Fused

한국어는 *조사*로 연결이 자유로워 긴 문장도 자연스럽지만, 영어로 직역하면 fused.

| 분류 | 예문 |
|---|---|
| 회피 | She finished the project the deadline was tight she was tired. |
| 바름 | She finished the project. The deadline was tight, and she was tired. |
| 바름 | Although the deadline was tight and she was tired, she finished the project. |

## 예문

| 영어 | 한국어 |
|---|---|
| Because the deadline was approaching, the team worked late into the night. | 마감일이 다가오고 있었기 때문에, 팀은 밤 늦게까지 일했습니다. |
| The report was thorough; however, the data needed verification. | 보고서는 철저했지만, 자료는 확인이 필요했습니다. |
| She finished the presentation, and the audience applauded. | 그녀는 발표를 마쳤고, 청중은 박수를 쳤습니다. |
| Although it was raining heavily, we decided to go for a walk. | 비가 많이 왔지만, 우리는 산책하러 가기로 했습니다. |
| The conference will be held next week. Many speakers are expected to attend. | 회의는 다음 주에 열릴 것입니다. 많은 연사들이 참석할 것으로 예상됩니다. |

## 자주 보는 함정

### 1. 종속절 단편

| 분류 | 예문 |
|---|---|
| 회피 | Because she was tired. |
| 바름 | Because she was tired, she went home. |

### 2. 분사구 단편

| 분류 | 예문 |
|---|---|
| 회피 | Walking down the street. |
| 바름 | Walking down the street, I saw her. |

### 3. Comma splice with 접속 부사

| 분류 | 예문 |
|---|---|
| 회피 | It was late, however we kept working. |
| 바름 | It was late; however, we kept working. |

### 4. Fused sentence

| 분류 | 예문 |
|---|---|
| 회피 | She came he left. |
| 바름 | She came, and he left. |
| 바름 | She came. He left. |

### 5. 짧고 가까운 두 절을 쉼표만으로

```text
회피 (격식): The sun rose, the birds sang.
바름: The sun rose, and the birds sang.
바름: The sun rose; the birds sang.
허용 (문학): The sun rose, the birds sang.
```

### 6. 너무 긴 중복문 (사실상 fused)

| 분류 | 예문 |
|---|---|
| 회피 | She studied hard for the exam and she stayed up late and she drank coffee and she finished the assignment and she went to bed. |
| 바름 | She studied hard for the exam, staying up late to drink coffee and finish the assignment. Then she went to bed. |

## 정리

- *Fragment* — 종속절·구·관계절을 단독 문장으로 쓰는 오류
- 완전한 문장 = *주어 + 정형 동사 + 완결된 의미*
- *Run-on* = *Fused sentence* (부호·접속사 없음) + *Comma splice* (쉼표만)
- 수정 방법 다섯 가지 — 분리, 등위접속사, 세미콜론, 종속접속사, 접속 부사 + 세미콜론
- *접속 부사* (however, therefore)는 접속사가 아님 — comma splice 주의
- 의도적 단편은 문학·창작에서만, 격식 글에서는 회피

## Part 4 마무리

Chapter 4 시리즈를 마칩니다. 시제 오류, 대명사 참조, 수식어 위치, 단편·런온 — 자주 마주치는 *구조적 오류*들을 다루었습니다.

다음 Part는 **단어 사용(Word Usage)**입니다. 비슷한 단어들의 구분과 *불필요한 단어*의 제거를 다룹니다.

## 다음 장 예고

다음은 **Chapter 5: Word Usage** — 단어 선택의 정확성.

## 관련 항목

- [Chapter 4: Common Mistakes](/blog/writing/only-grammar-book/ch04-common-mistakes)
- [Chapter 4.3: Modifier Placement](/blog/writing/only-grammar-book/ch04-3-modifier-placement)
- [Chapter 2.3: Sentence Types](/blog/writing/only-grammar-book/ch02-3-sentence-types)
- [Chapter 3.2: Comma](/blog/writing/only-grammar-book/ch03-2-comma)
- [Chapter 5: Word Usage](/blog/writing/only-grammar-book/ch05-word-usage)
- Elements of Style — Rule 5: Comma Splice
- Elements of Style — Rule 6: Sentence Fragment
