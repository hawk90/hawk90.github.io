...
slug: "parallel/cpp-concurrency-in-action/chapter03-sharing-data-between-threads"
...
동시성에서 *문제 거리가 되는 모든 버그*의 뿌리를 한 곳에서 찾으라면, 답은 **공유 데이터**다. 이 단언이 과장이 아닌 이유는 단순하다. 스레드들이 *서로의 데이터를 건드리지 않는다면*, 동시 실행은 어려운 문제가 아니다. 각 스레드는 자기 메모리에서 자기 일을 하고, 결과만 합치면 된다. race도, deadlock도, 메모리 모델도 등장할 자리가 없다.

문제는 *공유*가 시작되는 순간 발생한다.

- **읽기-쓰기 공유.** 한 스레드가 쓰는 동안 다른 스레드가 읽는다. 무엇을 읽을지 결정할 수 없다.
- **쓰기-쓰기 공유.** 두 스레드가 같은 위치에 쓴다. 누가 *이긴* 값을 남기는지 결정할 수 없다.
- **읽기-읽기 공유.** 이건 안전하다. 데이터가 *불변*인 한.

세 번째가 *유일한* 안전 지대다. 따라서 동시성을 다루는 첫 번째 설계 원칙은 단순하다. **공유를 줄여라.** Erlang/Go의 message passing, 함수형의 immutable 자료구조, thread-local 저장소가 모두 같은 원칙의 다른 형태다. 공유를 *0*으로 만들 수 있다면 이 장의 나머지는 필요 없다.

현실에서 공유를 0으로 만들기는 어렵다. 캐시, 인덱스, 카운터, 큐, 로그, 데이터베이스, 네트워크 상태 — 거의 모든 시스템에 *어떤 형태의 공유*가 있다. 그래서 동시성을 다룬다는 것은 결국 **남은 공유를 안전하게 다루는 기술**을 익히는 것이다. 이 장은 그 기술의 가장 기본인 *상호 배제(mutual exclusion)*를 다룬다. 더 진보된 기법(원자 연산, lock-free, 메모리 순서)은 5장과 7장에서 이어진다.

비유로 정리하면, 이 장 전체는 다음 네 가지 일상적 상황의 컴퓨터 버전이다.

- **race condition** — *한 화장실에 두 명이 동시에 들어가는 상황*. 누구도 안전하지 않다.
- **mutex** — *화장실 문 잠금*. 한 번에 한 명만.
- **deadlock** — *좁은 외나무다리에서 두 사람이 마주친 상황*. 둘 다 자기 길을 양보하지 않으면 영원히 못 지난다.
- **shared_mutex** — *도서관*. 여러 명이 동시에 읽을 수는 있지만, 책에 메모를 쓸 때는 혼자여야 한다.

각 절을 읽을 때 이 비유를 머리에 두면 코드의 의도가 쉽게 잡힌다.
...
### 화장실 문 잠금 — mutex가 푸는 문제

`std::mutex`의 본질은 화장실 문 잠금과 같다. 단 하나의 자원(화장실)을 *한 번에 한 명*만 쓰게 한다. 다른 사람들은 문 앞에서 기다린다(블로킹). 안에 있는 사람이 나오면(`unlock`), 기다리던 사람들 중 *어느 한 명*이 들어간다(누가 들어갈지는 OS 스케줄러가 결정).

이 비유의 두 가지 중요한 함의가 있다.

- **잠그는 사람과 푸는 사람은 같아야 한다.** 화장실에 들어간 A가 잠그고, 밖에 있는 B가 푸는 일은 없다. `std::mutex`도 마찬가지다. lock을 호출한 스레드만 unlock할 수 있다(소유권 의미론).
- **잠그는 것을 잊으면 안 된다.** 잠그지 않은 화장실은 보호되지 않는다. 코드에서 *어떤 경로로도* `lock` 없이 데이터에 접근하면 race가 살아난다. 그래서 데이터와 mutex를 *클래스로 묶어 캡슐화*하는 것이 표준 패턴이다.
...
### 좁은 외나무다리 — deadlock의 본질

두 사람이 좁은 외나무다리 양 끝에서 동시에 출발했다. 가운데서 마주쳤다. 둘 다 *지금 발 디딘 자리*를 양보하지 않으면, 둘 다 반대편으로 갈 수 없다. 누군가 *되돌아가야* 길이 열린다. 시간이 아무리 지나도, 양쪽이 *기다리기만 하면* 평생 그 자리에 있다. 이것이 deadlock이다.

컴퓨터 버전은 더 단순하다. 스레드 A가 자원 1을 잡고 자원 2를 기다린다. 스레드 B가 자원 2를 잡고 자원 1을 기다린다. 누구도 *놓아주지* 않으므로 누구도 *얻지* 못한다. 외나무다리에서는 *최소 한 명이 되돌아가야* 풀리듯, deadlock에서는 *최소 한 스레드가 자기 락을 풀고 처음부터 다시 시도해야* 풀린다(이 패턴이 7장에서 다룰 *backoff and retry*다).

비유는 회피 전략까지 자연스럽게 알려준다. 외나무다리에서 deadlock을 피하는 방법들을 그대로 옮긴다.

- **다리 입구에 신호등을 둔다** — 한 방향만 진입 허용. 락 *순서*를 고정한 것과 같다.
- **다리를 두 개 만든다** — 자원을 *분리*해 경합 자체를 없앤다. 락 입자도를 잘게 쪼개는 것과 같다.
- **다리에 *동시에 두 사람*이 올라갈 수 있게 넓힌다** — 자원이 *공유 가능*하다면 mutex 대신 `shared_mutex`로 동시 접근을 허용한다.
- **양쪽이 다리 앞에서 *동시에* 들어갈지 결정한다** — 두 락을 *원자적으로* 함께 잡는다. `std::scoped_lock` / `std::lock`이 이 방식이다.

이 절은 네 번째 방식의 표준 구현인 `std::scoped_lock`을 중심으로, 락 순서 규칙(첫 번째 방식)과 hierarchical_mutex 같은 강제 도구까지 본다.
...
`std::mutex`가 *화장실*이라면, `std::shared_mutex`는 *도서관*이다. 도서관에는 같은 책을 *여러 사람이 동시에 읽어도* 문제가 없다. 단, 누군가 그 책에 *메모를 적거나 페이지를 갈아 끼우려면* 다른 모든 사람이 그 책을 내려놓아야 한다.

같은 원칙을 코드로 옮긴 것이 reader-writer lock(공유/배타 락)이다.

- **공유 락 (`lock_shared` / `shared_lock`)** — *읽기 권한*. 동시에 여러 스레드가 보유 가능. 도서관에 여러 독자가 같은 책을 들여다보는 상태.
- **배타 락 (`lock` / `unique_lock` / `scoped_lock`)** — *쓰기 권한*. 단 한 스레드만 보유 가능. 그동안 모든 공유 락도 차단된다. 사서가 책을 바꿔 끼우는 동안 독자는 잠시 물러난다.

이 구조의 *이득*은 단순하다. **읽기가 압도적으로 많은 워크로드에서 동시성을 크게 높일 수 있다.** 99%가 읽기인 캐시·라우팅 테이블·DNS 응답·설정 데이터에 적합하다.

*비용*도 있다. 첫째, 공유 락 자체의 구현이 일반 mutex보다 무겁다(reader 카운터 갱신, writer 대기 처리). 짧은 임계 영역에서는 일반 mutex가 더 빠를 수도 있다. 둘째, **writer starvation** 가능성이 있다. 읽기 스레드가 끝없이 들어오면 writer가 영원히 기다린다. 표준은 이 정책을 *구현 정의*로 두므로 플랫폼마다 다르다. 일부 구현은 writer 우선(write preference), 일부는 reader 우선, 일부는 공정(FIFO)이다.

비유로 다시 말하면, 도서관이 *너무 인기 많아서* 사서가 책을 바꿔 끼울 틈을 못 얻는 상황이다. 해결책은 두 가지. 사서가 *대기 중*임을 표시해 새 독자를 들이지 않거나(write preference), 일정 시간마다 사서에게 *우선권*을 부여하거나.
...
## 시스템 사례 — 큰 시스템들의 동기화 전략

`std::mutex` / `std::shared_mutex`가 표준이 제공하는 *원시 도구*라면, 실제 큰 시스템들은 그 위에 *문제에 맞는 변형*을 쌓는다. 세 가지 사례를 본다.

### Linux 커널 — spinlock vs mutex

Linux 커널은 동기화 *원시*를 여러 종류 제공한다. 핵심 두 가지는 spinlock과 mutex다.

| 도구 | 동작 | 어느 컨텍스트에서 |
|------|------|-------------------|
| `spin_lock` | 락이 풀릴 때까지 *바쁘게 대기*(busy wait) | 인터럽트 핸들러, 임계 영역이 *극히 짧을* 때 |
| `mutex` (kernel) | 락 대기 시 *프로세스를 sleep* | 프로세스 컨텍스트, 임계 영역이 길거나 sleep 가능한 작업 포함 |
| `rwlock` / `rw_semaphore` | 읽기-쓰기 분리 | 읽기 비중이 압도적일 때 |
| `seqlock` | 읽는 쪽이 *비차단*, 쓰는 쪽이 단일 | 자주 읽고 가끔 쓰는 데이터(시스템 시각 등) |
| `RCU` | 읽는 쪽이 *완전 무동기*, 쓰는 쪽이 카피-갱신 | 거의 모든 읽기가 핫 패스인 경우(라우팅 테이블, dentry 캐시) |

spinlock과 mutex의 선택 기준은 단순하다. **임계 영역이 컨텍스트 스위치 시간(보통 수 μs)보다 짧으면 spinlock**, 그보다 길면 mutex가 유리하다. spin은 코어를 점유하지만 컨텍스트 스위치 비용이 0이고, mutex는 코어를 양보하지만 깨우는 데 시간이 든다.

C++ 사용자 공간 코드도 같은 트레이드오프를 만난다. 짧은 임계 영역에서 `std::mutex`가 *너무 느려* 보이면, atomic 기반 스핀(7장)이나 `std::atomic_flag`로 사용자 공간 spinlock을 만든다. 다만 사용자 공간 spinlock은 *우선순위 역전*에 취약하므로 일반적으로 권장되지 않는다(스레드가 양보 없이 도는데 우선순위 낮은 락 보유자가 스케줄되지 않는 상황).

### MySQL — row-level lock과 lock manager

MySQL의 InnoDB는 *테이블 전체*를 잠그지 않는다. **행(row) 단위**로 잠근다. 같은 테이블의 *다른 행*은 다른 트랜잭션이 동시에 수정할 수 있다.

구현은 이렇다. 디스크의 각 행에 *락 비트* 같은 게 박혀 있는 게 아니다. InnoDB는 메모리에 **lock manager**라는 자료구조를 두고, "트랜잭션 T1이 테이블 *users*의 페이지 *0x1234*의 행 슬롯 *7*에 X 락(exclusive)을 보유한다"는 식의 *논리적 락 객체*를 등록한다. 다른 트랜잭션이 같은 행을 수정하려 하면 lock manager가 *충돌 검사*를 한 뒤 대기 큐에 넣는다.

이 구조는 `std::shared_mutex` 한 개로 풀 수 없다. *행 단위 입자도*에 *공유/배타 모드*와 *intention lock*(테이블 단위 의도 표시), *gap lock*(인덱스 사이의 빈 공간), *next-key lock*까지 결합돼 있다. *그러나 모든 단위 동작*은 결국 `std::mutex` 또는 spinlock 한두 개로 만들어진다. lock manager 자체의 내부 자료구조(해시 테이블, 대기 큐)가 그것으로 보호된다.

요점은 *입자도(granularity)를 잘게 쪼개는 것*이 *큰 동시성 이득*을 만든다는 점이다. 책 3.8절(락 입자도)이 다루는 원칙을 산업 규모로 구현한 것이 InnoDB다.

### Java `synchronized` — 객체 단위 모니터

Java는 *언어 차원*에서 동기화를 통합했다. 모든 `Object`는 *내장 모니터*(intrinsic lock)를 가진다.

```java
class Counter {
    private int value;

    public synchronized void increment() {  // this의 모니터를 잠근다
        value++;
    }

    public int read() {
        synchronized (this) {                // 같은 의미
            return value;
        }
    }
}
...
`synchronized` 키워드는 *내부적으로* 두 가지를 한다. (1) 모니터 진입(monitor enter), (2) 메소드 종료 또는 블록 종료 시점에 monitor exit. JVM이 *바이트코드 레벨*에서 RAII를 보장하는 셈이다. 예외가 던져져도 monitor exit가 자동 호출된다.

C++ 대비 차이를 정리하면 이렇다.

| 항목 | Java `synchronized` | C++ `std::mutex` |
|------|---------------------|------------------|
| 락 객체 | 모든 객체에 내장 | 명시적 `std::mutex` 선언 |
| 자동 해제 | 언어 보장 (try-finally 자동 생성) | RAII 가드 사용자 책임 |
| 공유 락 | 없음 (`ReentrantReadWriteLock` 별도) | `std::shared_mutex` |
| 재진입 | 기본 재진입 가능 | `std::mutex`는 불가, `std::recursive_mutex`별도 |
| 조건 변수 | `Object.wait/notify` (모니터 내장) | `std::condition_variable` 별도 |

장단점이 갈린다. Java는 *항상 안전한 기본값*을 강제하지만, *비용이 비싸다*. 모든 `Object`가 헤더에 모니터 슬롯을 들고 있다. 그래서 JVM은 lightweight lock, biased lock 같은 최적화를 추가했다. C++은 *명시적*이라 비용을 사용자가 통제한다.

세 시스템의 답을 한 표로 본다.

| 시스템 | 단위 | 도구 | 비용 모델 |
|--------|------|------|----------|
| Linux 커널 | 자료구조마다 다른 락 | spinlock, mutex, rwlock, RCU | 컨텍스트별 최적 선택 |
| MySQL InnoDB | row-level | lock manager + 다단계 잠금 | 입자도 vs 메타데이터 비용 |
| Java | 객체 단위 | `synchronized` (모니터) | 안전성 우선, 자동화 |

C++의 *원시 도구* `std::mutex` / `std::shared_mutex`는 이 셋 어느 쪽에도 직접 대응하지 않는다. 대신 *어느 쪽이든 만들 수 있는 재료*를 제공한다. 위 시스템들이 자체 구현을 둔 이유도 *각자의 문제에 가장 잘 맞는 형태*를 만들기 위해서다. C++ 코드에서도 *기본 도구로 충분한 경우*가 대부분이지만, 극단적 경합 환경에서는 5~7장의 atomic·lock-free 도구로 내려가야 할 때가 있다.
