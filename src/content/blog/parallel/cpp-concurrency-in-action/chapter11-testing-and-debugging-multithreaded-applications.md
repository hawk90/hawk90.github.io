...
│                                                             │
│  Data Race                                                  │
│  └─ 동시에 같은 메모리 접근 (적어도 하나는 쓰기)             │
│                                                             │
│  Race Condition                                             │
│  └─ 실행 순서에 따라 결과가 달라짐                          │
│                                                             │
│  Deadlock                                                   │
│  └─ 스레드들이 서로 대기하며 영원히 멈춤                    │
│                                                             │
│  Livelock                                                   │
│  └─ 스레드들이 계속 작업하지만 진행이 없음                  │
│                                                             │
│  Starvation                                                 │
│  └─ 특정 스레드가 영원히 자원을 못 얻음                     │
│                                                             │
│  Priority Inversion                                         │
│  └─ 높은 우선순위 스레드가 낮은 우선순위에 의해 차단        │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                             │
│  컴파일 시 계측 코드를 삽입하여 런타임에 데이터 레이스 탐지  │
│                                                             │
│  지원: GCC, Clang, MSVC (최근)                             │
│                                                             │
│  탐지:                                                      │
│  - Data race                                                │
│  - 락 순서 위반                                             │
