...
slug: "parallel/cpp-concurrency-in-action/chapter01-hello-concurrent-world"
date: 2026-05-06T01:00:00
description: "동시성과 병렬성의 차이, C++11 std::thread 첫 사용, 동시성을 사용해야 할 이유."
tags: [cpp, Concurrency, "std::thread"]
...
**싱글 코어 (태스크 전환)** — 한 순간엔 A 또는 B만 실행:

```text
Core 0:  [A1][B1][A2][B2][A3][B3]...     ← 시간 축
```

**멀티 코어 (하드웨어 동시성)** — 같은 순간에 A와 B 모두 실행:
...
| 연도 | 클럭 | 코어 | 패러다임 |
|------|------|------|---------|
| 2000 | 1 GHz | 1 | 단일 스레드 최적화 |
| 2005 | 3 GHz | 2 | 병렬화 시작 |
| 2010 | 3 GHz | 4 | 병렬화 필수 |
| 2020 | 3 GHz | 8+ | 병렬화 기본 |
| 2025 | 5 GHz | 24+ (P+E 혼합), 서버 96+ | 병렬화 일반화 |
...
- `master process` — 설정 로드, worker 관리
  - `worker process #1` — 단일 스레드, event loop (`epoll_wait` → 수만 개의 active 커넥션을 비동기 처리)
...
1. **thread 만들고 join 잊음**
   ...
   - C++20 `std::jthread`는 자동 join

2. **detach()가 안전한 선택이라는 오해**
...
4. **`hardware_concurrency()` = 정확한 코어 수라는 오해**
   - 단지 *힌트* (0 반환 가능)
   - 하이퍼스레딩 포함 (논리 코어)
   - 컨테이너 / VM에선 호스트 값일 수도
