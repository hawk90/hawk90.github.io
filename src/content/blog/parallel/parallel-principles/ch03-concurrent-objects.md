...
slug: "parallel/parallel-principles/ch03-concurrent-objects"
...
**객체:** X(queue, SC만 만족), Y(queue, SC만 만족).
...
그러나 합쳐서 보면 두 deq가 *모두 empty*를 반환할 수도, *모두 성공*할 수도 있고, 순환 의존이 생긴다 — 일관된 전역 순서 없음.
...
| 모델 | 허용 재정렬 | 예 |
|------|-------------|-----|
| Sequential Consistency (SC) | 모든 load/store가 단일 전역 순서, 프로그램 순서 유지 | 이론적 모델, 없는 것에 가까움 |
| Total Store Order (TSO) | store→load 재정렬만 허용 (store buffer) | x86, x86-64 |
| Partial Store Order (PSO) | store→store도 재정렬 허용 | 옛 SPARC |
| Weak / Release | 거의 모든 재정렬 허용 (fence로 막아야 함) | ARM, PowerPC, RISC-V |
| C++/Java | Data-race-free → SC 결과 보장 (DRF-SC), `atomic + memory_order`로 재정렬 제어 | C++, Java |
...
| 모델 | 의미 |
|------|------|
| Linearizability | 실시간 순서 + 합성적 (이번 챕터) |
| Sequential Consistency (객체 차원) | 프로그램 순서, 실시간 순서 무시 (이번 챕터) |
| Quiescent Consistency | 조용할 때만 일관성 (이번 챕터) |
| Strict Serializability (DB) | 트랜잭션 단위 linearizability |
| Causal Consistency | 인과 관계만 보존 (실시간 무시) |
| Eventual Consistency | 결국 같아짐 (DynamoDB, NoSQL) |
...
mutex는 메모리 일관성을 강제(happens-before)한다. 그러나 객체 일관성은 별도 — *알고리즘 로직이 맞아야* 한다. 예 — dequeue가 empty 체크 후 pop 사이에 다른 스레드가 enq 가능한가?

**혼동 2 — "`memory_order_seq_cst`만 쓰면 linearizable"**

메모리 차원의 SC가 객체 차원의 linearizable을 *자동 보장하지 않는다*. 알고리즘 자체가 잘못되었을 수 있다.

**혼동 3 — "C++는 SC 메모리 모델이다"**

DRF (data race free)일 때만 SC 결과를 보장한다. `atomic + relaxed`를 쓰면 SC가 깨진다. 객체의 linearizability는 메모리 모델 위에서 *별도로* 증명해야 한다.
