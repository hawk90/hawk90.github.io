...
slug: "parallel/parallel-principles/ch11-concurrent-stacks-and-elimination"
...
스택은 큐와 달리 *push와 pop의 짝이 자주 만난다*. LIFO 특성상 가장 최근의 push가 다음 pop의 대상이기 때문이다. 동시 환경에서 이 만남은 두 의미를 가진다. 첫째, 두 연산이 같은 변수(top)를 두고 *경쟁*한다. 모든 스레드가 한 꼭대기에서 다투는 형국이다. 둘째, 만남 자체가 *우회의 기회*다. push와 pop이 서로 만나면 스택을 *안 거치고* 서로 상쇄될 수 있다. 이 챕터의 핵심 통찰은 *경합을 완화하는 게 아니라 우회한다*는 발상에 있다.
...
![Elimination 인과 — 경합이 늘면 CAS 실패가 늘어나는 한편 elimination 만남도 늘어 처리량이 유지된다](/images/blog/parallel-principles/diagrams/ch11-elimination-feedback.svg)
...
| 자료구조 | 시나리오 | elimination 결과 |
|----------|----------|-----------------|
| Stack (LIFO) | `push(D)`와 `pop()` 만남 | `pop`이 D 받음 — *OK* (최신이 다음) |
| Queue (FIFO) | `enqueue(D)`와 `dequeue()` 만남 | `dequeue`가 D 받음 — *명세 위반* (가장 오래된 원소가 먼저였어야) |
