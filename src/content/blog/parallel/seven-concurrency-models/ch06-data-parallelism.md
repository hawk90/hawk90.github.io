...
Data parallel:
  Core 1: 같은 작업 (data[0..N/3])
  Core 2: 같은 작업 (data[N/3..2N/3])
  Core 3: 같은 작업 (data[2N/3..N])
```
...
```
일반 명령:        SIMD 명령:
ADD a, b         ADD [a0,a1,a2,a3], [b0,b1,b2,b3]
                 → 4개 더하기 한 사이클에
```
...
Global work-items: 전체 작업 단위 (예: 백만 개)
  └─ Work-group 1: 256개 work-items 그룹
       └─ Local memory 공유, barrier 가능
  └─ Work-group 2: ...
  └─ ...
```
...
GPU 메모리는 다층 — 성능 차이가 큼.

```
Global memory:  GB 단위, 느림 (수백 cycles)
Local memory:   KB 단위, 빠름 (work-group 공유)
Private memory: 수십 바이트, 매우 빠름 (work-item 전용)
Constant:       읽기 전용, 캐시됨
```
...
```
1. *GPU = 게임 / ML 전용*이라는 좁은 시각
   - 이미지 / 신호 처리 / 시뮬레이션
   - 데이터 분석에도 활용

2. *CUDA만 쓰면 됨*이라는 단순화
   - PyTorch / JAX가 더 실용적
   - 직접 CUDA는 극한 최적화 시만

3. *분기 = OK*라는 오해 (GPU)
   - warp divergence로 성능 폭락
   - 가능한 한 branchless

4. *데이터 전송 비용 무시*
   - GPU memory ↔ CPU memory 매우 느림
   - 한 번 올리면 GPU에서 *오래* 작업
...
□ SIMD와 GPU의 차이?
□ Work-item / Work-group 의미?
□ GPU 메모리 계층과 최적화?
□ Warp divergence가 성능에 미치는 영향?
□ Reduce를 GPU에서 구현하는 방법?
```
