...
slug: "parallel/seven-concurrency-models/ch01-threads-and-locks"
...
스레드와 락은 거의 모든 백엔드 서버, 게임 엔진, 모바일 앱이 *이미 쓰고 있는* 기술입니다. Redis가 단일 스레드 모델로 유명한 이유, Kafka 프로듀서가 락 경합을 줄이려 배치를 묶는 이유, Linux 커널이 spinlock과 mutex를 구분하는 이유는 모두 이 장의 개념 위에 있습니다. 락의 한계를 모르면 다음 6개 모델(함수형, Actor, CSP, STM, 데이터 병렬, 람다)이 *왜* 등장했는지 이해할 수 없습니다.
...
여러 사람이 한 화장실을 쓰려면 문에 잠금이 필요합니다. 한 명이 들어가면 문을 잠그고, 다른 사람은 밖에서 기다리고, 나오면 다음 사람이 들어가는 식입니다. `synchronized`는 정확히 이 잠금 메커니즘입니다. 코드의 한 구간에 *한 번에 한 스레드만* 들어오게 하는 도구입니다.

책의 첫 예제는 카운터입니다. 여러 스레드가 같은 정수를 증가시키는 가장 단순한 시나리오입니다. 아래 코드는 *겉보기에* 멀쩡합니다. 그러나 실제로 돌려 보면 결과가 매번 다르게, 그리고 *기대보다 작게* 나옵니다.
...
회의실 칠판에 누가 글씨를 썼는데, 다른 방에 있는 사람은 그 글씨를 못 볼 수 있습니다. CPU의 캐시도 비슷합니다. 한 코어가 변수를 바꿔도 다른 코어는 자기 캐시의 *오래된* 값을 계속 볼 수 있습니다. `volatile`은 *모두가 같은 화이트보드를 보게* 강제하는 도구입니다. 한 명이 쓰면 모두에게 즉시 보입니다.
...
좁은 외나무다리를 떠올려 봅시다. 차 두 대가 다리 양쪽 끝에서 동시에 진입했습니다. 둘 다 *상대가 비켜 줄 때까지* 기다리고, 둘 다 비켜 주지 않습니다. 영원히 멈춥니다. 데드락(deadlock)이 정확히 이 상황입니다. 두 스레드가 *서로의 락*을 기다리며 둘 다 진행하지 못합니다.
...
회의실을 예약하는 상황을 생각해 봅시다. `synchronized`는 *예약될 때까지 무작정 줄 서는* 방식입니다. 도중에 자리를 비울 수도, 다른 일을 보러 갈 수도 없습니다. `ReentrantLock`은 "5분만 기다려 보고 안 되면 다른 회의실로 가자"라거나 "지금 당장 비어 있는지만 확인하자"가 가능합니다.
...
은행 창구가 한 줄로 모두를 받는 대신 *입금/출금*과 *대출 상담* 줄을 나누면, 입금 손님이 도착했을 때 대출 상담 줄에 있는 사람을 깨우지 않아도 됩니다. `Condition`이 같은 일을 합니다. 락 위에 *여러 개의 대기 줄*을 만들어, 정확히 필요한 줄만 깨웁니다.
...
암벽을 오를 때 양손 중 한 손만 떼고 다음 잡이로 옮기듯, 리스트를 탐색할 때 *현재 노드의 락*을 잡은 채 *다음 노드의 락*을 잡고, 그 다음에 현재 노드의 락을 푸는 방식입니다. 항상 *두 손 중 하나는* 잡고 있으니 안전하고, 동시에 *리스트 전체*를 막지 않습니다.
...
동전을 던졌더니 앞면이 나왔습니다. 잠시 후 다시 봤더니 또 앞면입니다. 그런데 그 사이 누가 동전을 *다른 동전*으로 바꿔치고 다시 던졌을 수 있습니다. 결과는 같은 "앞면"이지만, *사이에 일어난 일*은 다릅니다. CAS의 ABA 문제도 똑같습니다. 변수 값이 A → B → A로 돌아왔을 때, "값이 같으니 변한 게 없다"고 판단하지만 실제로는 그 사이 *다른 작업*이 끼어들었을 수 있습니다.
...
직접 못을 박아도 집은 지어집니다. 그러나 보통은 검증된 못과 공구를 씁니다. `java.util.concurrent`도 같은 발상입니다. 락과 큐를 *직접* 짜는 대신, 수십만 시간 검증된 표준 도구를 가져다 씁니다. 거의 모든 경우 더 빠르고 더 안전합니다.
...
택배 회사가 *주문이 올 때마다* 직원을 새로 뽑지 않는 것과 같습니다. 정해진 수의 직원을 고용해 두고, 주문은 큐에 쌓아 둡니다. 일이 비면 직원은 다음 주문을 꺼냅니다. `ExecutorService`는 정확히 이 모델입니다. 스레드를 *재사용*하므로 매번 생성/소멸 비용이 들지 않고, 동시 스레드 수가 폭주하지도 않습니다.
...
회사의 한 팀이 큰 보고서를 받았다고 합시다. 팀장은 분량을 나눠 팀원에게 나눠 주고, 각자의 결과를 모아 다시 합칩니다. 팀원이 일을 빨리 끝내면 *바쁜 동료의 일을 가져다* 돕습니다. Fork/Join이 정확히 이 구조입니다. 큰 작업을 *분할(fork)*하고 *결과를 합치며(join)*, 노는 워커가 바쁜 워커의 일을 *훔쳐(steal)* 옵니다.
...
식당의 주방과 홀을 떠올려 봅시다. 주방은 음식을 *만들어 패스 창에 올리고*, 홀 직원은 *창에서 꺼내 손님에게 가져갑니다*. 창이 가득 차면 주방은 잠시 멈추고, 비면 홀 직원이 기다립니다. `BlockingQueue`가 이 *패스 창* 역할을 합니다. 양쪽이 서로의 속도를 신경 쓰지 않고, 큐가 알아서 흐름을 조절합니다.
...
레고 블록 두 개가 각각 안전하다고 해서, 둘을 붙인 결과도 안전하다는 보장은 없습니다. 락도 똑같습니다. 두 개의 thread-safe한 객체를 한 작업에서 같이 쓰면, 그 *조합*은 thread-safe하지 않을 수 있습니다. 이것이 락이 다른 모든 동시성 모델의 출발점이자, 또한 *떠나는 이유*입니다.
...
이 장의 개념들이 실제 어디서 쓰이는지 몇 가지만 짚어 봅니다.

- **Redis** — 단일 스레드 이벤트 루프로 *대부분의 락을 없앤* 시스템입니다. 명령이 직렬화되므로 race 자체가 발생하지 않습니다. 다만 백그라운드 RDB 저장, AOF rewrite 같은 보조 스레드는 여전히 atomic 연산과 짧은 mutex를 씁니다.
- **Kafka Producer** — 내부적으로 `ConcurrentHashMap`과 비슷한 batch accumulator를 둡니다. 여러 애플리케이션 스레드가 같은 토픽으로 메시지를 넣을 때 락 경합을 줄이려고 파티션별 deque + CAS 기반 append를 결합합니다. j.u.c.의 직접 응용 사례입니다.
- **Linux 커널 spinlock** — `synchronized`처럼 *블록*하지 않고 *바쁘게 회전*하며 락을 기다립니다. 임계 영역이 *나노초 단위*로 짧고 컨텍스트 스위치 비용이 더 클 때 유리합니다. 인터럽트 핸들러 안에서는 잠들 수 없으므로 mutex 대신 spinlock이 필수입니다.
- **JVM의 `ConcurrentHashMap`** — Java 8에서 락 스트라이핑 + CAS + bin 단위 락(`synchronized(node)`)을 섞어 씁니다. 읽기는 거의 락 없이, 쓰기는 충돌하는 bin만 잠급니다. 이 장에서 다룬 도구가 *조합되어* 한 자료구조에 들어간 사례입니다.
- **PostgreSQL Buffer Pool** — 페이지마다 LWLock(lightweight lock)을 두고, 핫 페이지는 *atomic refcount + spinlock*으로 일반 락의 비용을 회피합니다. ABA 회피를 위한 epoch 카운터까지 포함합니다.
