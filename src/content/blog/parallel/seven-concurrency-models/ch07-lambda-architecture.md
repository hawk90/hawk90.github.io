...
```text
Map     : (Hello,1) (world,1) (Hello,1) (there,1) (world,1) (is,1) (wide,1)
Shuffle : Hello → [1,1]   world → [1,1]   there → [1]   is → [1]   wide → [1]
...
Lambda Architecture가 다른 분산 처리와 가장 다른 점은 *재계산성(recomputability)*입니다. 마스터 데이터셋이 불변이기 때문에 batch view 생성 코드를 바꾸면 *처음부터 다시 돌리면* 됩니다. 운영 중 발견된 집계 버그는 다음 두 단계로 고칠 수 있습니다.
...
```text
query(URL)
  = batch_view[URL]      // 어제 자정까지 누적 PV
  + speed_view[URL]      // 그 이후 실시간 증분
```
