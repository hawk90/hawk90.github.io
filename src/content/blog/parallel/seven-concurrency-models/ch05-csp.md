...
slug: "parallel/seven-concurrency-models/ch05-csp"
...
CSP를 한 줄로 직관화하면 이렇습니다. **채널은 두 사람 사이의 컨베이어 벨트**입니다. 한쪽 끝에서 누가 물건을 올려놓는지 다른 쪽은 모릅니다. 다른 쪽 끝에서 누가 물건을 집어가는지 올려놓은 사람도 모릅니다. 양쪽은 *벨트만* 신경 씁니다. 이 무지(無知)가 결합도를 낮춥니다.

같은 무지의 다른 표현은 **익명 우편함**입니다. Actor 모델은 *받을 사람을 지정해서 편지를 보내는* 방식입니다. "철수에게 보낸다"고 명시합니다. CSP는 *공동 우편함에 익명으로 넣어두는* 방식입니다. 누군가 가져가지만, 보낸 사람은 누가 가져갈지 모릅니다. 두 방식의 차이가 시스템의 결합 양상을 결정합니다.

이 차이가 *파이프라인* 설계에 특히 잘 맞는 이유는 명확합니다. 공장 조립 라인을 떠올리면 됩니다. 1번 공정의 작업자는 자기 앞에서 부품이 올라오면 가공해서 다음 컨베이어에 올려놓습니다. 2번 공정 작업자가 누구인지, 몇 명인지, 얼마나 빠른지는 알 필요가 없습니다. *벨트가 사이를 매개*하기 때문입니다. 작업자가 한 명이든 다섯 명이든, 1번 공정 코드는 똑같습니다. 채널이 정확히 같은 역할을 합니다.

이 비유를 한 발 더 밀고 가면 다음 매핑이 나옵니다.

| 일상의 비유 | core.async 대응 |
|---|---|
| 컨베이어 벨트 | `chan` |
| 손에서 손으로 직접 전해주기 (둘이 만나야 함) | unbuffered channel — rendezvous |
| 사물함 (먼저 넣어두면 나중에 꺼내감) | buffered channel |
| 사물함이 꽉 차면 *새로 온 짐을 버린다* | `dropping-buffer` |
| 사물함이 꽉 차면 *오래된 짐을 꺼내 버린다* | `sliding-buffer` |
| 공장 조립 라인 (단계마다 다음으로 넘김) | `pipeline` |
| 방송국 송출탑 (한 신호를 여러 수신기가 동시에 수신) | `mult` |
| 잡지 구독 (토픽별로 받는 사람이 다름) | `pub` / `sub` |
| 응급실 진료 우선 순위 (먼저 도착하는 환자부터) | `alts!` |
| 시계가 울리는 알람 (시간이 되면 신호) | `timeout` |

특히 `alts!`의 응급실 비유가 직관에 도움이 됩니다. 응급실에는 여러 입구로 환자가 들어옵니다. 의사는 "다음 한 명"을 받습니다. 누가 어느 입구에서 들어왔는지는 처리 후에 알면 됩니다. `alts!`도 똑같습니다. 여러 채널을 늘어놓고 *가장 먼저 준비되는 채널 하나*를 받습니다. 이 동작 하나로 timeout, polling, multiplex가 모두 표현됩니다.

## go 블록과 parking의 직관

`go` 블록을 처음 만나면 한 가지 의문이 생깁니다. *Java thread가 비싼데 어떻게 수만 개의 go 블록을 띄울 수 있는가?* 답은 **parking**입니다.

비유로 풀면 이렇습니다. 회사에 회의실(thread)이 열 개 있다고 합시다. 그런데 미팅(go 블록)은 백 개를 잡아야 합니다. 평범하게 짜면 한 미팅이 회의실 하나를 *전세*냅니다. 그러면 열 한 번째 미팅부터는 회의실이 빌 때까지 기다려야 합니다. core.async는 다르게 합니다. 미팅이 *상대방을 기다리는 시간 동안* 회의실을 비워줍니다. 그동안 다른 미팅이 그 회의실을 빌려 씁니다. 기다리던 미팅에 상대가 도착하면, 빈 회의실 아무 곳에서나 이어서 진행합니다. 회의실 열 개로 백 개의 미팅이 *겹쳐* 돌아갑니다.

이것이 parking입니다. *go 블록이 채널을 기다릴 때 dispatch thread를 점유하지 않고 놓아준다*는 의미입니다. 다른 go 블록이 그 thread를 *빌려서 자기 일을 함*. 채널 이벤트가 도착하면 *다른 thread*가 원래 go 블록의 상태를 이어서 실행합니다. 같은 코드가 thread를 갈아타며 진행하는데, 작성자는 그 사실을 의식할 필요가 없습니다. 매크로가 상태 머신을 만들어 주기 때문입니다.

이 모델 덕분에 *thread는 적게, 동시성은 많게* 라는 균형이 가능합니다. 책이 강조하는 dispatch thread pool의 크기(보통 코어 수 + 작은 상수)는 이 가정 위에서 결정됩니다. 모든 go 블록이 *친절하게 park만 한다면* 적은 thread로 충분합니다. 반대로 한 go 블록이 진짜로 thread를 점유하면 (예: `<!!`을 잘못 사용) 회의실 하나가 *전세* 처리되어 전체 처리량이 무너집니다.

## Actor vs CSP — 한 번 더, 직관 수준에서

5.1과 5.14에 형식적인 비교가 있지만, 직관 수준에서 한 번 더 짚습니다.

- **Actor**는 *주소록*을 가진 사람들의 모임입니다. "철수에게 메시지를 보낸다"가 일차 동작입니다. 받는 사람의 정체를 알아야 보낼 수 있고, 그 사람의 mailbox가 메시지를 보관합니다.
- **CSP**는 *공용 게시판* 또는 *공용 컨베이어*입니다. "이 채널에 값을 올린다"가 일차 동작입니다. 그 값을 누가 받을지는 모릅니다. 채널 자체가 *제3의 객체*로 존재합니다.

이 차이의 실용적 귀결은 두 가지입니다.

첫째, **테스트 용이성**. CSP 시스템은 *채널 그래프*를 그릴 수 있습니다. 한 노드의 입력 채널과 출력 채널만 분리해 두면, 그 노드를 *데이터 한 줄짜리 테스트*로 검증할 수 있습니다. Actor는 상대 actor의 reference에 결합되어 있어 격리가 더 까다롭습니다.

둘째, **분산 친화성**. Actor 모델은 자연스럽게 노드를 가로지릅니다. "철수"가 다른 머신에 있어도 같은 코드가 동작합니다. CSP의 채널은 보통 *프로세스 내부* 추상이라, 노드를 가로지르려면 별도 통신층(NATS, Kafka 등)이 필요합니다. 책이 4장과 5장의 결을 다르게 펼치는 이유가 여기 있습니다.
...
![Actor vs CSP — direct addressing vs channel-mediated](/images/blog/seven-concurrency-models/diagrams/ch05-actor-vs-csp.svg)
...
기본 도입은 단순합니다. 아래 코드는 *채널 하나*에 한쪽 go 블록이 "hello"를 올려놓고, 다른 go 블록이 같은 채널에서 꺼내 출력하는 *최소 rendezvous*를 시연합니다. 두 go 블록은 서로의 존재를 코드 어디에도 언급하지 않습니다. 채널만 공유합니다.
...
![alts! multiplexer — recv, send, timeout, default all unified](/images/blog/seven-concurrency-models/diagrams/ch05-alts-select.svg)
...
![web crawler channel graph — worker pool + thread pool + results channel](/images/blog/seven-concurrency-models/diagrams/ch05-web-crawler.svg)
...
책이 다루는 core.async 너머의 실제 시스템에서 CSP 아이디어가 어떻게 살아 있는지 짧게 둘러봅니다. 모두가 "채널이 first-class"라는 정신을 공유합니다.

**Go의 goroutine + channel.** Go 언어는 CSP를 *언어의 기본 어휘*로 끌어들였습니다. `go f()`로 가벼운 프로세스를 띄우고, `ch := make(chan T)`로 채널을 만들고, `<-ch`와 `ch <- v`로 값을 주고받습니다. core.async의 매크로가 하는 일을 Go는 컴파일러와 런타임이 합니다. 책의 Day 1 ping-pong, Day 2 worker pool 패턴이 Go에서는 거의 한 줄로 옮겨갑니다. Docker, Kubernetes, etcd 같은 클라우드 인프라가 이 모델 위에 서 있는 이유가 *fan-out / fan-in*이 자연스럽기 때문입니다.

**Kubernetes의 informer pattern.** Kubernetes 컨트롤러는 API 서버의 변경을 *workqueue* (Go 채널 위에 얹힌 큐)로 받아 처리합니다. 한 informer가 변경 이벤트를 broadcast하면, 여러 컨트롤러가 같은 스트림을 *각자의 채널*로 구독합니다. 책의 5.12절 `mult`와 같은 모양입니다. 컨트롤러는 자기가 누구로부터 이벤트를 받는지 모른 채 자기 일을 합니다.

**NATS의 subject-based pub/sub.** NATS는 분산 메시지 브로커지만, 어휘는 core.async의 `pub`/`sub`와 거의 같습니다. 발행자는 *subject*를 지정해서 메시지를 보내고, 구독자는 subject 패턴(`orders.>`, `metrics.cpu.*`)으로 받습니다. 발행자와 구독자가 서로의 존재를 모릅니다. 5.12절의 토픽 라우팅 비유 "잡지 구독"이 그대로 분산 환경으로 확장된 형태입니다.

**Walmart Labs의 Clojure 백엔드.** 책 출간 즈음 Walmart Labs는 블랙 프라이데이 트래픽을 Clojure + core.async 기반 서비스로 처리한 경험을 공개했습니다. 외부 API 호출(재고, 가격, 추천)을 *각각의 채널*로 모델링하고, `alts!`로 가장 빠르거나 가장 신뢰할 만한 응답을 골라 사용했습니다. timeout 채널 한 줄로 "n ms 안에 답이 안 오면 캐시된 값 사용"이 표현되었습니다. 책의 Day 2 패턴이 그대로 production에 적용된 사례입니다.

| 시스템 | 모델 위치 | core.async 대응 |
|---|---|---|
| Go goroutine + channel | 언어 차원 | `go` + `chan` |
| Kubernetes informer | 컨트롤 플레인 | `mult` |
| NATS subject | 분산 메시지 | `pub` / `sub` |
| Walmart Labs API fan-out | 서비스 백엔드 | `alts!` + `timeout` |

공통점은 *흐름을 first-class로 다룬다*는 점입니다. 어디서 시작되어 어디로 가는지를 채널(혹은 그 일반화)로 명시하면, 시스템의 구조가 코드에서 직접 읽힙니다.
